...
modules/trade/internal/
  ...
  exchange/
    ...
    okx/
  infra/
    persistence/     Repository、UnitOfWork、Inbox、Outbox；不暴露数据库实现
    messaging/       Trade Topic 映射和 Outbox Relay，复用 packages/jetstream
    clock/           交易所时钟偏差
```
...
`domain` 不依赖 GORM、tRPC、NATS、Binance 或 OKX SDK。`persistence` 是 Trade 内部持久化端口的实现包名，SQLite/GORM 只是部署配置和依赖注入时选择的技术，不进入领域接口、目录名或类型名。
...
Trade 直接复用公共 `packages/messagepb` 和 `packages/jetstream`。`infra/messaging` 不重复封装 NATS 连接、发布、消费、ACK、重投和编解码，只实现 Trade 特有的 Topic/Payload 映射、Outbox Relay、Inbox 事务衔接与消费者路由。初始 Topic：
