...
modules/trade/internal/
  ...
  exchange/
    ...
    okx/
  infrastructure/
    sqlite/          Repository、UnitOfWork、Inbox、Outbox
    eventbus/        Publisher、Consumer、Topic 编解码
    clock/           交易所时钟偏差
```
...
`domain` 不依赖 GORM、tRPC、NATS、Binance 或 OKX SDK。
...
Trade 使用 `packages/messagepb` 和 `packages/jetstream`，初始 Topic：
