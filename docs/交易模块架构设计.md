...
Trade 采用模块化单体：一个进程、一个权威数据库和清晰的领域边界。订单、成交、执行计划、Saga、账本、余额、仓位、调仓与 Inbox 都由 Trade 自己保存。Trade 不维护 Outbox，也不通过 EventBus 给自己发事件。
...
```mermaid
flowchart LR
  U["用户或策略"] --> GW["Admin 浏览器控制面"]
  GW --> RPC["Trade RPC"]
  RPC --> APP["Application Commands"]
  STRATEGY["Strategy Outbox"] --> BUS["MooX EventBus"]
  BUS --> APP["Rebalance Consumer"]
  APP --> DOMAIN["Trade Domain"]
  DOMAIN --> STORE["Trade Store"]
  STORE --> DB["Trade Database"]
  STORE --> WAKE["Local Wake"]
  WAKE --> WORKERS["Execution / Rebalance / Reconciliation Workers"]
...
Strategy 在自己的事务中写 Outbox，并发布唯一的 `trade.rebalance.requested` 命令。Trade 在一个数据库事务中记录 Inbox 和调仓计划，重复投递由 `event_id`、业务幂等键和 Inbox 共同保护。

Trade 的后续状态推进只依赖数据库和进程内 Wake。Timer 负责启动恢复和遗漏兜底，不承担正常路径。Trace 信息从 EventMessage 恢复到消费者 Context。
...
改单不是内存中的连续函数，而是持久化 Saga。恢复扫描覆盖初始撤单、结果未知、替代单已创建和替代单提交未知等状态。替代订单沿用普通订单内核，由本地 Worker 提交；Saga 不会再次直接提交已经打开的订单。

## 调仓架构

调仓规划器只生成确定性草案，不访问数据库、EventBus 或交易所。规划结果保存算法名称、版本、市场快照、规则版本、legs 和依赖关系。

`trade.rebalance.requested` 创建调仓计划并立即唤醒首批 legs；订单和成交状态提交后继续唤醒后续依赖 legs。恢复循环只修复中断，不作为正常调仓驱动器。

## 可观测性与健康

Prometheus 指标使用 `moox_trade_` 前缀，覆盖调仓命令、交易所提交、成交、未知订单、对账差异、Consumer、私有流、操作延迟和调仓结果。标签只使用有限枚举，不包含订单号、通道 ID 或 Symbol。

健康检查同时验证：

- Trade Store 可访问。
- EventBus 调仓 Consumer 当前连接正常。
- 活动订单需要的每个私有流均已鉴权并订阅。
- 未知订单和活动订单数量可观测。
...
立即对账由 RPC 写入本地工作状态并唤醒 Worker，必须遵守请求中的 Space、账户和通道边界。Saga 检查接口只返回当前 Space 的记录。
...
| EventBus、本地 Wake 与定时器 | EventBus 只接收 Strategy 调仓命令，本地 Wake 主驱动 | 进程内状态不绕 EventBus；定时器仅恢复遗漏工作。 |
