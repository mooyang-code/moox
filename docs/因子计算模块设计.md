...
- 触发源是 Storage 的 `moox.storage.time_series.rows_changed.v1` JetStream 事件；事件只含 key，因子模块负责回读 K 线窗口。**部署前提：storage 的 `eventbus.type` 必须配置为 `nats`（默认 `memory` 只在单进程内可见）。**
...
| 集成点 | 依赖 | 说明 |
| --- | --- | --- |
| 变更事件 | NATS JetStream stream `MOOX_STORAGE`，subject `moox.storage.time_series.rows_changed.v1` | durable pull consumer `factor_calc`，与 ViewBuilder/Archive 消费互不影响 |
...
flowchart LR
  ...
  Storage -- "rows_changed 事件" --> NATS["NATS JetStream"]
  NATS --> Factor
...
flowchart TB
  ...
  Access -- "time_series.rows_changed.v1<br/>(仅含 key)" --> JS["NATS JetStream<br/>MOOX_STORAGE"]
...
1. collector 在每分钟 bar 收盘后写入 500 symbol 的 K 线，Access 发布约 500 条 `rows_changed` 事件（含批量写入时的合并）。
...
1. **防自触发死循环**（决定性理由）：`WriteTimeSeriesRows` 成功后 Access 必然发布 `rows_changed` 事件，且事件只含 key、不含列信息。若写回 K 线 Dataset，trigger 无法区分"collector 写了新 K 线"与"自己刚写回的因子值"，会无限自触发。独立 Dataset 后 trigger 只需按 `dataset_id` 白名单过滤。
...
- Consumer：durable pull consumer `factor_calc`，`FilterSubject = moox.storage.time_series.rows_changed.v1`，`AckWait = 60s`，`MaxDeliver = 5`，`DeliverPolicy = New`（首次部署不回放历史，存量数据靠补算命令初始化）。
...
截面计算必须等"该 bar 的时序因子基本到齐"，采用水位线（watermark）机制。**就绪信号统一使用因子结果 Dataset 的 `rows_changed` 事件**（而非进程内计数器）：所有实例写回因子值都会触发该事件，截面执行者订阅它并统计到齐率——单机与多实例分片部署共用同一套机制，分片对截面层完全透明。

```text
截面执行者（cross_section_leader）订阅因子结果 Dataset 的
moox.storage.time_series.rows_changed.v1 事件：
