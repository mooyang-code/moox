...
| 结果写回 | `storage.PrimaryStore/UpsertFields` | 字段级 patch 语义：同 key 只更新携带字段 |
...
flowchart TB
  subgraph storage["Storage"]
    PrimaryStore["PrimaryStore<br/>UpsertFields / ReadTimeSeriesRows"]
  ...
  Engine -- 因子值 --> SIO
  SIO -- "UpsertFields<br/>(独立因子 Dataset)" --> PrimaryStore
...
1. **防自触发死循环**（决定性理由）：`UpsertFields` 成功后 PrimaryStore 会发布 rows-upserted 事件。若写回 K 线 Dataset，因子会再次消费自己的输出并无限自触发。独立 Dataset 后 trigger 只需按输入 `dataset_id` 白名单过滤。
...
├── config/
│   ├── app.yaml                # storage/eventbus/python/worker/调度参数
...
└── internal/
    ...
    ├── trigger/                # EventBatcher、pending inbox 与 replay
    │   └── eventconsumer/      # EventBus 绑定、解码、ACK 与重连
...
- **Go 侧**：使用 `github.com/apache/arrow-go/v18`。storageio 回读的 `FieldValue` 字段直接构建 `arrow.Record`（每列一个 `array.Builder`，double 列 `Float64Builder`、时间列 `TimestampBuilder(ms, UTC)`），`ipc.NewWriter` 写入帧 payload。K 线缓存（见下节）可直接以 Arrow 列存驻留，回读后零转换发送。
...
| 写回 | 500 次 UpsertFields × 100 字段 × 5 行 | 可按 symbol 批量合并为单请求多行 |
...
1. **EventBus 可达性**：Storage 与 Factor 必须通过各自的 `eventbus` 凭据连接同一
   `moox-eventbus` 部署；事件名、Stream 与 Subject 由 `packages/events` Registry
   统一解析，不在业务配置中重复声明。
