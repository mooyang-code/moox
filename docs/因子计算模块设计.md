...
| 集成点 | 依赖 | 说明 |
| --- | --- | --- |
| 变更事件 | NATS JetStream stream `MOOX_STORAGE`，subject `moox.storage.fields_changed.v1.>` | durable pull consumer `factor_calc`，与 ViewBuilder/Archive 消费互不影响 |
...
flowchart TB
  ...
  PrimaryStore -- "fields_changed.v1.<shard>" --> JS["NATS JetStream<br/>MOOX_STORAGE"]
...
- Consumer：durable pull consumer `factor_calc`，`FilterSubject = moox.storage.fields_changed.v1.>`，`AckWait = 60s`，`MaxDeliver = 5`，`DeliverPolicy = New`（首次部署不回放历史，存量数据靠补算命令初始化）。
- 消息体：`DatasetFieldsChanged`（protojson），携带变更字段与 `spaceId/datasetId/subjectId/freq/dataTime` 定位信息。
