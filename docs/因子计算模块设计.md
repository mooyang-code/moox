...
- 触发源是 Storage 的 `moox.storage.rows_committed.time_series.v1.<shard>` JetStream 事件；事件携带提交序列与行快照，因子模块负责回读 K 线窗口。
...
| 集成点 | 依赖 | 说明 |
| --- | --- | --- |
| 变更事件 | NATS JetStream stream `MOOX_STORAGE`，subject `moox.storage.rows_committed.time_series.v1.<shard>` | durable pull consumer `factor_calc`，与 ViewBuilder/Archive 消费互不影响 |
...
flowchart LR
  ...
  Storage -- "rows_committed 事件" --> NATS["NATS JetStream"]
  NATS --> Factor
...
flowchart TB
  ...
  PrimaryStore -- "rows_committed.time_series.v1.<shard>" --> JS["NATS JetStream<br/>MOOX_STORAGE"]
...
1. collector 在每分钟 bar 收盘后写入 500 symbol 的 K 线，PrimaryStore 发布 `rows_committed` 事件（含批量写入时的合并）。
...
- Consumer：durable pull consumer `factor_calc`，`FilterSubject = moox.storage.rows_committed.time_series.v1.>`，`AckWait = 60s`，`MaxDeliver = 5`，`DeliverPolicy = New`（首次部署不回放历史，存量数据靠补算命令初始化）。
- 消息体：`TimeSeriesRowsCommitted`（protojson），`keys[]` 含 `spaceId/datasetId/subjectId/freq/dataTime`。
- 消费即 Ack：事件只是"触发信号"，去抖与任务化在内存完成；即便进程崩溃丢失少量触发，由对账 timer 兜底（见调度器章节）。这避免了慢消费导致的 JetStream 重投风暴。
...
事件 keys[] 逐条处理：
  ...
  3. 入去抖桶 bucket[(space, dataset, subject, freq)]：
       ...
       bucket.deadline 未设置则设为 now + event_batch_window（默认 2s）
...
- **触发分片**：trigger 在 binding 过滤之后追加分片过滤（`hash(subject_id) % total == index` 或显式 symbol 白名单）。K 线变更事件只含 key、量小（约 500 条/bar），每实例全量接收、本地过滤，代价可忽略。每实例使用独立 durable consumer（`factor_calc_{instance_id}`），互不干扰。
...
截面计算必须等"该 bar 的时序因子基本到齐"，采用水位线（watermark）机制。**就绪信号统一使用因子结果 Dataset 的 `rows_committed` 事件**（而非进程内计数器）：所有实例写回因子值都会触发该事件，截面执行者订阅它并统计到齐率——单机与多实例分片部署共用同一套机制，分片对截面层完全透明。

```text
截面执行者（cross_section_leader）订阅因子结果 Dataset 的
moox.storage.rows_committed.time_series.v1.<shard> 事件：
...
| M2 触发链路 | NATS durable 订阅 + 去抖 + scheduler（分片/supersede/重试）+ K 线窗口缓存 | collector 实时写 1m K 线，因子列自动跟进；固定分片 worker 实测可并发，500 symbol 事件风暴下 bar 内完成 |
