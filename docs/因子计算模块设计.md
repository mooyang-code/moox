...
flowchart TB
  ...
  subgraph factor["moox-factor"]
    Trigger["trigger<br/>Consumer / 过滤 / 去抖聚合"]
...
└── internal/
    ...
    ├── trigger/                # NATS Consumer、事件过滤、去抖聚合
...
| M2 触发链路 | NATS Consumer + 去抖 + scheduler（分片/supersede/重试）+ K 线窗口缓存 | collector 实时写 1m K 线，因子列自动跟进；固定分片 worker 实测可并发，500 symbol 事件风暴下 bar 内完成 |
