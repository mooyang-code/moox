...
当前架构不依赖旧库迁移。演示、测试或重新初始化环境时，优先删除运行态数据，先用各模块 CLI 重建 schema，再按 `examples/e2e/README.md` 通过管理台/API 重新生成数据。Storage 元数据 seed 通过 `moox-storage-cli import-seed` 导入。
