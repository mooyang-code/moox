...
- **连接池管理**：GORM 连接池配置
- **Schema 初始化**：Admin 启动时自动应用内嵌 `schema/admin.sql`
...
// 初始化数据库（SQLite），会自动应用内嵌 admin schema
...
```go
// Admin 的表结构由 modules/admin/schema/admin.sql 内嵌进二进制。
// Initialize 会打开 ./data/admin.db 并自动应用 admin schema。
err := dbManager.Initialize(&cfg.Database)
```
...
当前架构不依赖旧库迁移。演示、测试或重新初始化环境时，优先删除运行态数据并按 `examples/e2e/README.md` 通过模块启动、`moox-cli metadata import`、管理台/API 重新生成数据。
