...
流水中的 rows 是主存写入成功后可用于物化的行内容，至少包含本次写入列。ViewBuilder 稳态路径直接使用 rows patch DuckDB/Bleve，不再为了补齐 join 视图而回读 PrimaryStore；新建、版本变更和手动 Rebuild 才通过 Access 读接口按 `query_window` 或 `backfill_window` 回扫。当前派生消费者包括 view 角色的 ViewBuilder 和 archive 角色的归档运行时；Bleve 搜索是 view 内部能力，不作为独立 search 服务部署。
