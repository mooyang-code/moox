...
事件中的 rows 是变更提示，不要求携带完整行。派生消费者收到事件后通过 Access 读接口回读最新完整行，再覆盖写入 DuckDB、Bleve 或其他派生存储，使重放和重试保持幂等。当前派生消费者包括 view 角色的 ViewBuilder 和 archive 角色的归档运行时；Bleve 搜索是 view 内部能力，不作为独立 search 服务部署。
