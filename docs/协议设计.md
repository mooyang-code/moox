...
| `rows.proto` | 用户侧事实数据读写入口 |
| `store.proto` | PrimaryStore 到 DataShard 的内部执行协议 |
| `view.proto` | DuckDB/Bleve 等 View 读模型 |
| `rows_committed.proto` | storage 领域事件 |
...
- 内部主存协议统一使用 `ShardKey`、`ShardRow`、`ShardTarget`。
...
DataShard 是 PrimaryStore 内部的物理分片执行协议：
...
```text
ShardKey(space_id, dataset_id, kind, key, version)
ShardRow(key, columns, attributes)
ShardTarget(space_id, dataset_id, node_id, device_id, engine, endpoint, device_table)
```

PrimaryStore 根据 `PrimaryStoreRoute` 解析出 `ShardTarget`，再把同批 rows 按 target 分组写入 DataShard。跨 target 写入不提供全局原子性；已经成功写入的 target 不因后续 target 失败而回滚。
...
`RebuildTimeSeriesView` 与 `RebuildRecordView` 是异步控制面接口，只负责受理重建任务并返回 `rebuild_id`。后台任务必须通过 PrimaryStore 读接口回读主存，不能绕过 PrimaryStore 直接请求主存 target。
...
事件中的 rows 是变更提示，不要求携带完整行。派生消费者收到事件后通过 PrimaryStore 读接口回读最新完整行，再覆盖写入 DuckDB、Bleve 或其他派生存储，使重放和重试保持幂等。当前派生消费者包括 view 角色的 ViewBuilder 和 archive 角色的归档运行时；Bleve 搜索是 view 内部能力，不作为独立 search 服务部署。
