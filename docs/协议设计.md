...
`SearchRecordRows` 面向 Record + Bleve。请求以 View 为入口，使用全文 query、filters、sorts、keys 或 numeric revision range 查询 Record 派生索引，返回 `RecordRow` 和 View columns。
...
Record committed event 携带 PrimaryStore 原子提交的完整行、source_id 和 commit_seq。NATS 仅用于低延迟通知，ViewBuilder 的稳定快照和连续 journal replay 才是重建/补偿依据。
