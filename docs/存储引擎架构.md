...
| 角色 | 职责 |
| --- | --- |
| `access` | 面向用户的写入和权威读取入口；校验列契约、解析路由、写 PrimaryStore，并发布 rows_updated 写入流水。 |
| `primary` | 拥有 Pebble PrimaryStore RPC；可按路由和配置部署多个 primary 服务。 |
| `view` | 注册 DataView，消费写入流水、批量聚合 rows 并 patch DuckDB/Bleve；Create/Rebuild 时才通过 Access 回扫窗口。 |
| `archive` | 注册 archive timer，消费写入流水，通过 Metadata/Access RPC 读取元数据和事实数据；后续写 Parquet 冷归档。 |
...
| `view.backfill_window` | 必填；Create/Rebuild 未指定 `query_window` 时的回扫窗口。 |
| `view.batch_size` | 单批最多聚合的写入流水行数。 |
| `view.batch_wait_ms` | 未达到批大小时的最长等待时间。 |
| `view.max_workers` | 保留兼容；当前热路径固定单 processor 保序，不再作为并发吞吐旋钮。 |
...
AccessService.WriteTimeSeriesRows / WriteRecordRows
  ...
  -> eventbus.Bus             PublishTimeSeriesRowsUpdated / PublishRecordRowsUpdated
  -> services/view/builder    异步按写入流水 patch DuckDB / Bleve 派生结果
```
...
- **跨 target 非原子**：Access 按路由把同一批 rows 分组写入多个 `PrimaryStoreTarget`。`PrimaryStoreTarget` 是内部执行目标，不是事务边界；某 target 失败不回滚已成功 target，且会为每个已成功写入分组发布 rows updated 写入流水，让派生侧追上成功部分。

### 事件总线

- `core/eventbus` 是上层业务写入流水抽象，发布 `TimeSeriesRowsUpdated` / `RecordRowsUpdated`。
...
写入流水携带 `message_id`、`written_at`、`space_id`、`dataset_id` 和本次成功写入的 `rows`。稳态派生消费者直接使用 rows patch 本 dataset 贡献列，不再按 key 回读 PrimaryStore；跨 dataset join 依赖各 dataset 后续流水逐步补齐半行。Backfill/Rebuild 仍通过 Access 扫描有效窗口内的事实数据。

部署形态：

- 仓库默认 eventbus 协议为 `memory`，只支持同进程订阅，且仍异步派生，**不提供写后立即可搜/可查 View 契约**。
- 生产环境使用 NATS 时，`infra/eventbus.SubscriberBus` 分别订阅 `${prefix}.time_series.rows_updated.v1` 与 `${prefix}.record.rows_updated.v1`。NATS transport 会基于 `consumer_name + subject kind` 派生不同 durable consumer，避免两个 subject 共用 durable 名冲突。
- 实时消费者只处理启动后新产生的写入流水。历史补仓、断档追数和读模型重建统一通过 View rebuild 完成，避免独立 view/archive 进程启动后回放大量历史事件。
- 事件 subject 统一前缀默认 `moox.storage`，时序写入流水默认 `moox.storage.time_series.rows_updated.v1`，记录写入流水默认 `moox.storage.record.rows_updated.v1`；NATS stream 可用 `moox.storage.>` 前缀通配，便于扩展 View/Archive 等派生事件。
- ViewBuilder handler 解析并入队成功后即返回 success，让 NATS Ack 与 DuckDB/Bleve 物化完成解耦；物化失败通过内部日志/指标暴露，并依赖 backfill/Rebuild 修复读模型。
...
- Access 的**数据面读路径**（路由解析、写入校验、Search 列解释、视图查询暴露 `active_result`）走 snapshotcache 快照，默认每 10s 刷新一次。因此新建路由/列契约后，写读路径可见前存在短暂延迟，运行链路验证时需要等待快照刷新。
