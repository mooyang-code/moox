...
默认事件总线协议是 NATS。仓库默认配置启用 `eventbus.embedded.enabled: true`，在 storage 进程内启动 JetStream，本地无需额外启动 `nats-server`；分布式部署可关闭内嵌 broker，让多个节点连接同一个独立 NATS。`memory` 只用于极简单进程测试，且仍是异步总线，不提供写后立即可查派生结果的契约。ViewBuilder 配置项包括：
...
- 单进程本地运行可使用默认 `roles: [access, view]`、`primary.service_name: ""` 与内嵌 NATS；`access` 会同时暴露本地 `PrimaryStoreService`，ViewBuilder 通过 AccessService 回读并写 DuckDB/Bleve。
- 本地单进程或临时验证可显式设置 `eventbus.type: memory`、`view.access_service_name: ""`。该模式不需要 NATS，但派生仍异步；验证脚本不沉淀为项目自有单元测试。
