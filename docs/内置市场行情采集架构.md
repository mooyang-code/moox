...
| 来源数据集 | 保存各 Provider 带 tag 的最小标准化事实；内部类型为 `ProviderDataset`，角色为 `provider_data` | `provider_equity_kline` |
...
`provider_data` 和 `unified_data` 是 Dataset 角色，不是两套 Storage API。两类时序数据都通过 Storage PrimaryStore `UpsertFields` 写入，但写入者和权限不同：

| 角色 | 写入者 | 写入内容 | 读取者 |
| --- | --- | --- | --- |
| `provider_data` | `ProviderDatasetWriter` | 各 Provider 以 `provider:<id>` 区分的最小标准化事实 | KlinePipeline、Quality Resolver、内部排障工具 |
| `unified_data` | `UnifiedDatasetWriter` | Quality Resolver 裁决后的统一事实 | Market Module、Factor、Trade、Web |

Provider 只返回 `ProviderKlineBatch`，不选择 Dataset，也不直接调用 Storage。
`KlinePipeline` 根据 Market Descriptor 和 Provider binding 选择共享来源 Dataset
并生成 `provider:<provider_id>` tag；`QualityResolver` 只能通过
`UnifiedDatasetWriter` 写统一数据集。普通查询路径不读取来源数据集。

当前 Binance Collector 由数据源实现直接构造 Storage 行并写目标 Dataset。迁移到本架构时，应把取数、来源写入、质量裁决和统一写入拆开；Provider 层只保留协议访问、解析和最小标准化。

### 来源数据集写入

Schema、频率和生命周期相同的 Provider 来源使用同一个来源 Dataset，并由一个不透明
`series_tag=provider:<provider_id>` 区分。例如：

```text
stock_cn/provider_equity_kline
  provider:tdx
  provider:tencent
  provider:ifeng
```

Provider 独有 Schema 或生命周期确实不同时才拆 Dataset。来源数据使用统一
`subject_id`，Provider symbol 作为列保存：
...
`space_id + dataset_id + subject_id + freq + data_time + series_tag` 执行字段级幂等
Upsert。`series_tag` 只承载一条序列的身份，Storage 不解析 `provider:`；其他业务
属性保存为 Field/Attribute。

### 统一数据集写入

来源数据写入成功后，`QualityResolver` 按本次受影响的
`subject_id + freq + data_time` 读取该时间点的全部 Provider tag，生成统一数据：
...
`ROW_WRITE_MODE_REPLACE` 解决整行残留问题，但不提供基于 `revision` 的条件更新。
第一阶段必须由 Collector/CloudNode 为
`market_id + instrument_type + subject_id + freq + target_series_tag + time_window`
提供统一裁决租约，保证同一输出序列范围同时只有一个写入者。统一 K 线的
`target_series_tag` 为空。以后允许重叠采集任务时，应增加按 Subject 分区的单写者
队列，或者为 Storage 增加 CAS 写入，避免低优先级结果覆盖高优先级结果。

统一数据集保存当前胜出结果。来源冲突、裁决变更和修订原因写入追加式 `kline_quality_event` Record Dataset；原始 Provider 响应如需长期审计，再使用独立归档能力，不塞入时序来源数据集。

## SCF 执行模型

> 本节的 Market Module/JobItem 是后续多 Provider 扩展的设计草案，不得覆盖当前 Binance
> 实时 K 线实现。当前 K 线采用每函数一个 Timer Trigger 和静态 Environment 任务快照：
> Collector 负责分片/DNS 协调，SCF 负责一次抓取和一次 Storage 聚合写入；Symbol 快照与
> 缺口补采才使用有界 Invoke。若后续将本节 JobItem 落地，必须另写执行计划并重新评估成本。
...
moox-collector-crypto-scf
  MOOX_SPACE_ID=crypto
```

这种部署方式保持每个函数固定 `MOOX_SPACE_ID`，并通过 `space_id + job_type` durable
参与竞争消费，也让 Provider 限频、SCF 并发、凭据和故障影响按市场隔离。新增 Market
Module 时可复用代码包，只增加函数配置和节点声明。
...
- 来源数据集和统一数据集按包含 `series_tag` 的完整时序 key 幂等写入，超时重试
  不产生重复事实。
...
`subject_id`、`freq`、`data_time` 和 `series_tag` 是时序行键。统一 K 线默认使用空
tag；来源序列使用 `provider:<provider_id>`。所有价格均为不复权价格；复权因子和
复权结果属于独立 Dataset 或派生结果。
...
crypto/instruments
crypto/provider_spot_kline
crypto/provider_perpetual_kline
crypto/spot_kline
crypto/perpetual_kline
crypto/market_coverage
crypto/kline_quality_event
```

统一数据集和共享来源数据集各绑定一个 `kind=internal` 的逻辑聚合 DataSource，例如
`moox_stock_cn`、`stock_cn_provider_ingress`。物理 `tdx`、`tencent`、`binance`
DataSource 仍管理 Provider 配置和凭据，但不直接绑定同一个共享 Dataset；具体来源
由 `series_tag` 与 `source_provider` Field 保留。来源数据集使用
`dataset_role=provider_data`，统一数据集使用 `dataset_role=unified_data`，追加式
质量记录使用 `dataset_role=quality_event`，coverage 区间记录使用
`dataset_role=coverage_state`。
...
```text
market_id + data_type + unified_dataset + subject_id + freq + target_series_tag
```
