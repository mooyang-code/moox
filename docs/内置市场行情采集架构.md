...
`ProviderDatasetWriter` 只写已经闭合且完成基础解析的 K 线。它按 `space_id + dataset_id + subject_id + freq + dimensions + data_time` 幂等写入。实现阶段先为 Storage 增加 `ROW_WRITE_MODE_REPLACE`，让同一来源后续修正同一根 K 线时原子替换整行并更新 `fetched_at`；现有 `MERGE` 继续作为默认模式。这样 Provider 撤回可选字段时不会残留旧值。
