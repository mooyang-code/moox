...
| Feed | Market Module 暴露的强类型数据接口 | `instrument`、`calendar`、`kline` |
...
| Provider Client | SCF | 按单出口 IP RateLimitPolicy 请求外部接口并解析响应 |
...
每个 stock Timer 函数的最大实例并发固定为 1、异步自动重试为 0，并仅按本函数出口 IP 执行 Feed 的 token bucket、并发 semaphore、请求 timeout 和 429 cooldown。Fetcher 内不得隐藏 HTTP 重试；fallback 必须重新取得候选 Feed 的本地令牌。控制面负责 Group 分布和执行优先级，不汇总或发放跨全部函数的 Provider 请求 permit。
...
Realtime、Backfill 和 GapRepair 使用不同 BatchKind。Realtime 始终优先；历史任务受 `batch_bar_limit`、`max_concurrency`、每函数单 IP 的 `rate_budget_ratio`、Provider 历史范围和每次最大 Bars 约束。同一 Subject/分钟同时只属于一个 BatchKind，Backfill 和 GapRepair 必须避开 Realtime 最近窗口及正在执行的桶。无水位时不得再隐式使用 `now-1h` 代替策略。
...
高级过滤可以指定 `exchange_ids` 和 `subjects`，但不能指定 Provider 或 Provider 来源 Dataset。普通用户也不需要指定逻辑 K 线 Dataset；Market Module 根据 Feed Descriptor 解析目标。
...
`market.yaml` 保存 Fetcher Spec、Provider 权重、单 IP RateLimitPolicy、`timer_function_count=N`、路由和 HistoryPolicy。`metadata.seed.yaml` 保存 Storage 逻辑契约。密钥不进入这两类文件，通过环境变量或 Secret 管理。
