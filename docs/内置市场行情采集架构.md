...
`stock_cn.timer_function_count=N` 是发布配置，初始建议 200，但不是代码常量。`ActiveInstrumentSet` 变化只更新 Assignment；只有 `ceil(active_subject_count/N)` 超过压测安全容量时，才由运维调整 N 并重新发布，第一期不做运行时自动扩缩容。Instrument 的有界 Invoke shard 只在完整 generation 的所有 shard 到齐后切换 active set，避免独立抓取结果混代。
