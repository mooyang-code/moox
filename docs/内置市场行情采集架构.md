...
本文描述 MooX 行情采集的目标架构。文中的 Market Module、多 Provider 聚合、Canonical Dataset、完整性修复和 `moox-cli init` 内置市场初始化尚未实现。当前实现仍以 Binance `symbol`、`kline` 采集为主，现状见 [采集任务管理](采集任务管理.md)。
...
- 多 Provider 分片采集、质量校验、Canonical K 线生成和缺口修复。
- 通过 Storage Access 读取已经持久化的 Canonical K 线。
...
2. **Provider 只负责取数。** Provider 不依赖 Storage，不管理任务状态，也不决定 Canonical 数据。
3. **一根 K 线来自一个 Provider。** 不把一个来源的 OHLC 和另一个来源的成交量拼成一根 K 线，也不平均多个来源的价格。
4. **采集结果先落来源层，再进入 Canonical 层。** 来源证据和业务统一数据分开保存。
...
| Landing Dataset | 保存单个 Provider 最小标准化事实的内部 Dataset | `landing_tdx_equity_kline` |
| Canonical Dataset | 质量裁决后供业务统一读取的 Dataset | `equity_kline` |
...
股票、ETF 和指数属于同一个模块，但使用不同 Canonical Dataset 和独立 Provider 路由策略。它们的停牌、成交量、可交易性和字段要求不同，不应写入同一 Dataset。
...
现货、永续和交割合约应使用不同 Canonical Subject，例如：
...
flowchart TD
    ...
    Market --> Read["Storage Access 读取 Canonical Dataset"]
    ...
    P1 --> Landing["Provider Landing Dataset"]
    P2 --> Landing
    P3 --> Landing
    Landing --> Quality["Normalize + Quality Resolver"]
    Quality --> Canonical["Canonical Dataset"]
    Canonical --> Read
    Coverage["Coverage Reconciler"] --> Canonical
    Coverage --> Planner
```
...
| Symbol Policy | 维护 Canonical Subject 与各 Provider symbol 的映射 |
...
```text
解析逻辑任务
  -> 读取 Canonical 水位和近期缺口
  -> 生成 Provider 分片
  -> 拉取并分页
  -> 过滤未闭合 K 线
  -> 写 Landing Dataset
  -> 执行质量裁决
  -> 幂等写 Canonical Dataset
  -> 返回逐标的执行摘要
```

第一阶段在同一个 JobItem 中顺序完成 Landing 和 Canonical 写入。Landing 成功而 Canonical 失败时，JobItem 返回失败；重试依赖两侧幂等写入恢复。规模扩大后，可以把 Canonical 生成改为事件驱动。
...
同一 Canonical K 线的 OHLCV 必须来自同一个 Provider：

- 不平均不同 Provider 的价格。
- 不按字段拼接多个 Provider。
- Provider 独有字段保留在 Landing Dataset。
- 缺失公共必填字段的候选行不能进入 Canonical Dataset。
...
Canonical 行记录 `source_provider` 和 `revision`。低优先级来源先填补缺口、高优先级来源后来恢复时，可以修订 Canonical 行，但必须留下版本和冲突证据。
...
| 层次 | 行为 |
| --- | --- |
| 增量采集 | 从 Canonical 最新水位继续，并回看少量已闭合 K 线 |
...
Canonical Dataset 绑定一个 `kind=internal` 的 MooX DataSource，例如 `moox_stock_cn`。Provider Landing Dataset 分别绑定物理 DataSource，例如 `tdx`、`tencent` 或 `ifeng`。
...
`stock_cn.kline` 是逻辑读取接口，只从 Storage Canonical Dataset 读取：
...
Market Module 根据 Subject 的 Instrument Type 选择 Dataset；查询包含多种 Instrument Type 时，分别调用 Storage Access，再统一合并返回。业务层不直接访问 Pebble，也不读取 Landing Dataset。
...
高级过滤可以指定 `exchange_ids` 和 `subjects`，但不能指定 Provider Landing Dataset。普通用户也不需要指定 Canonical Dataset；Market Module 根据 Feed Descriptor 解析目标。
...
```text
market_id + data_type + canonical_dataset + subject_id + freq
```
