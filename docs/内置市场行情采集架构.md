...
Provider 独有字段可以保留在对应来源数据集，但不能直接进入统一数据集。来源数据集保存可参与裁决的最小标准化事实，不保存原始 HTTP 或 TCP 响应。

`ProviderDatasetWriter` 只写已经闭合且完成基础解析的 K 线。它按 `space_id + dataset_id + subject_id + freq + dimensions + data_time` 幂等写入。同一来源后续修正同一根 K 线时，写入完整的非空列集合并更新 `fetched_at`，避免普通部分写入保留旧值。当前 Storage 不支持用空值清除已经存在的列；如需表达字段撤回，应先为 Storage 增加行替换或显式清除语义。
...
第一阶段由一个 JobItem 拥有确定的 Subject 分片和时间窗口。JobItem 可以顺序请求主 Provider 和质量抽样 Provider，分别写入来源数据集，最后只执行一次统一裁决。

当前 Storage 对相同时序 key 和 `data_time` 执行合并式写入，尚未提供基于 `revision` 的条件更新。第一阶段必须由 Collector/CloudNode 为 `market_id + instrument_type + subject_id + freq + time_window` 提供统一裁决租约，保证同一范围同时只有一个写入者。以后允许重叠采集任务时，应增加按 Subject 分区的单写者队列，或者为 Storage 增加 CAS 写入，避免低优先级结果覆盖高优先级结果。
...
统一数据集绑定一个 `kind=internal` 的 MooX DataSource，例如 `moox_stock_cn`。来源数据集分别绑定物理 DataSource，例如 `tdx`、`tencent` 或 `ifeng`。来源数据集使用 `dataset_role=provider_data`，统一数据集使用 `dataset_role=unified_data`，追加式质量记录使用 `dataset_role=quality_event`。
