...
### Provider Landing 与 Canonical Dataset

多 Provider 市场采集仍遵守“一个 Dataset 只绑定一个 DataSource”：

- Provider Landing Dataset 绑定物理来源，例如 `tdx`、`tencent`、`ifeng`。
- Canonical Dataset 绑定 `kind=internal` 的 MooX 市场 DataSource，例如 `moox_stock_cn`。
- 质量裁决器从多个 Landing Dataset 选择一根合法 K 线，再写入 Canonical Dataset。
- 普通业务、Factor、Trade 和 Web 只读取 Canonical Dataset。

以 `stock_cn` 为例：

```text
landing_tdx_equity_kline       -> data_source_id=tdx
landing_tencent_equity_kline   -> data_source_id=tencent
equity_kline                   -> data_source_id=moox_stock_cn
```

Canonical Dataset 的每一根 K 线仍然来自一个 Provider。系统不平均多个来源的价格，也不按字段拼接多个来源。Canonical 行通过 `source_provider`、`quality_status` 和 `revision` 保留来源和裁决结果。
