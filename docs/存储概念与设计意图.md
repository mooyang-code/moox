...
更细的表结构见 `存储目标架构与元数据.md`，PB 接口见 `协议设计.md`，服务角色与部署方式见 `存储服务架构与部署.md`。
...
- 在线主存、分析查询、文本检索和冷备归档职责分离，但由同一套元数据解释。文本检索是 view 角色内部能力，不作为独立 search 服务部署。
- 所有事实数据访问都先进入 PrimaryStore Service。View 和 Archive 需要回读事实数据时，也通过 PrimaryStore/Metadata RPC 完成。
- Primary、View 和 Archive 都可以独立部署；其中只有 Primary 需要按 Subject 做在线主存水平切分。
...
PrimaryStore 是唯一公开事实数据访问入口；Primary、View 和 Archive 不直接暴露给业务写入方。
```
...
Collector 的目标架构把系统提供的标准市场数据注册为用户可见的内置 Space：
...
这些 Space 是稳定的市场数据目录，不是 `moox_system` 一类内部运维空间，也不是用户临时研究空间。它们固定携带：
...
内置 Space 及其 DataSource、Field、Dataset 和 DatasetColumn 契约由 `moox-cli init` 创建并校验。普通用户可以读取数据、在 Dataset 契约支持范围内调整采集策略和创建派生 View，但不能删除或改名系统内置契约。

不同 Space 之间当前不能通过一个 Storage View 直接组合 Dataset。跨市场分析由 Factor、Trade 或其他上层服务分别读取多个 Space 后完成。
...
多 Provider 市场采集仍遵守“一个 Dataset 只绑定一个 DataSource”：

- 来源数据集绑定物理来源，例如 `tdx`、`tencent`、`ifeng`，内部角色为 `provider_data`。
- 统一数据集绑定 `kind=internal` 的 MooX 市场 DataSource，例如 `moox_stock_cn`，内部角色为 `unified_data`。
- 质量裁决器从多个来源数据集选择一根合法 K 线，再写入统一数据集。
- 普通业务、Factor、Trade 和 Web 只读取统一数据集。

以 `stock_cn` 为例：

```text
tdx_equity_kline       -> data_source_id=tdx,            dataset_role=provider_data
tencent_equity_kline   -> data_source_id=tencent,        dataset_role=provider_data
equity_kline           -> data_source_id=moox_stock_cn,  dataset_role=unified_data
```

统一数据集的每一根 K 线仍然来自一个 Provider。系统不平均多个来源的价格，也不按字段拼接多个来源。统一数据行通过 `source_provider`、`quality_status` 和 `revision` 保留来源和裁决结果。

完整的 Market Module、查询和补洞设计见 [内置市场行情采集架构](内置市场行情采集架构.md)。
...
多 Dataset View 的一行由统一的 ViewRowKey 组合，不会因为附属 Dataset 增加 Subject 而扩展行域。例如：
...
最终 View 行：
  BTCUSDT + 1m + 2026-07-18T12:00:00Z
    close
    volume
    momentum
    volatility
...
当 A 的 `close` 变化时，ViewBuilder 只发送 A 所拥有列的 `MERGE`，ViewIndex 复用已有行中的 B 列，不回读未变化的 B。只有目标行不存在时才批量读取全部来源并以完整 `REPLACE` 恢复；附属来源缺失时对应列为 Null。
...
## PrimaryStore、PrimaryStore、Device 与 PrimaryStoreRoute

`PrimaryStore Service` 是唯一公开数据访问入口。Collector、FactorCalculator、管理台、CLI 和其他业务调用方都先访问 PrimaryStore，再由 PrimaryStore 按请求语义转发到内部服务。

元数据管理和服务侧读取分开：管理台、CLI 和控制面任务直接通过 SQLite 元数据 store 做 CRUD；PrimaryStore 的校验、路由、Search 列解释等服务读路径使用独立 metadata cache。cache 启动加载快照，后续刷新由 snapshotcache 组件负责，元数据写入不会同步更新 cache。
...
- PrimaryStore 只关心“把这批事实数据交给哪个 PrimaryStore 节点”。
- PrimaryStoreRoute 解析结果会携带目标节点的 endpoint，PrimaryStore 的内部 PrimaryStore client 据此连接正确节点。
...
```text
1. 本文：先理解概念和设计取舍
2. 存储目标架构与元数据.md：看架构、数据流和表结构
3. 协议设计.md：看接口协议和调用边界
