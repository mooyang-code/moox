...
> **状态：第一阶段迁移已完成。** `go.work` 管理 9 个 Go module，所有模块已迁入 `modules/`，`skills/moox` 和 `scripts/` 已建立，根 `Makefile` 已统一构建入口。Gin 依赖已移除，全部统一为 tRPC 框架。
...
当前 workspace 包含 9 个 module（7 个业务模块 + 2 个 proto 生成代码包）：

```
modules/account          → github.com/mooyang-code/moox/modules/account
...
modules/order            → github.com/mooyang-code/moox/modules/order
...
| `modules/order` | 订单、成交、交易通道和账户交易操作 |
