...
| Egress result/non-empty/distinct IP count | `moox_collector_market_egress_functions`; publish CLI emits the authoritative one-time gate | CLI publishes the bounded gate result when metrics EventBus is configured; Monitor checks freshness and exact count equality | Closed in source. The release gate remains authoritative; production status remains pending until a real publish report is consumed. |
...
ObserveEgressGate: `modules/cli/internal/command/collector.go` publish gate
```
