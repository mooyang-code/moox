...
定义查询结果粒度
定义查询窗口
记录当前 active_result / active_view_version
记录正在 warming 的 building_result / building_view_version
记录构建状态
```

`c_query_window` 表示 View 支持在线组合查询的最近时间范围。派生索引按这个窗口和 rotation 配置做有界回扫和重建。

View 派生索引统一使用 active/building 双索引生命周期。`active_result` 是当前可读索引 ID，`building_result` 是正在构建或 warming 的索引 ID。DuckDB 表和 Bleve 索引目录都实现 `ViewIndexEngine`（`internal/core/viewindex`），上层生命周期只调用 `Prepare`、`Write`、`Stat`、`Remove`；查询仍分别走 `QueryTimeSeriesRows` 的 DuckDB 路径和 `SearchRecordRows` 的 Bleve 路径。

唯一调度入口是 view_builder 上的 `op=rotate`：负责 schema 抢占重建、容量轮换、stale/failed warming 清理、ready switch，以及未引用物理索引删除。独立的 pending rebuild / cleanup / retry_failed 调度路径已删除，手动 `RebuildTimeSeriesView` / `RebuildRecordView` RPC 也已删除。schema、列来源、过滤条件、粒度、查询窗口等 View 定义变化会递增 `view_version`；系统默认抢占式重建新的 building 索引，旧 active 继续服务直到 warming Complete 原子切换指针。新字段只有在 building 索引切换为 active 后才可查询。

DuckDB 和 Bleve 索引都是可重建的近期派生结果，不是完整事实存储；完整历史以 PrimaryStore KV/Pebble 为准。物理索引使用确定性的 `a` / `b` 双 slot，旧 timestamp 命名或其它非 a/b 派生索引可直接丢弃，并由 `op=rotate` 从 PrimaryStore 重建。DuckDB 的 `__latest` helper 暂时保留，待 rotation 稳定且 K 线查询性能复核后再单独移除。
...
DuckDB 和 Bleve 都只保存近期派生 View 索引。系统按事件增量写入 active 索引和合法 building 索引；定时 `op=rotate` 从 PrimaryStore 扫描 View 的查询窗口，构建新的 a/b slot。
...
4. `Prepare(indexID)` 新建 inactive a/b slot。
5. 按粒度键关联附属 Dataset 的列，写入宽表并校验。
6. `BeginViewBuild` 发布 `building_result`，让后续增量双写进入 warming 索引。
7. 从 PrimaryStore 分页 backfill，ready 后 `CompleteViewBuild` 切换 `active_result`。
8. 等待 `remove_grace` 后通过 engine `Remove` 删除旧 active 或 orphan 索引。

系统不在线 `ALTER` 当前 active 索引。新增字段或因子时，后台新建 building 索引并切换；旧 active 在 Complete 前持续服务，避免读空窗。`storage.view.rotation.enabled=false` 是完整 kill switch：跳过 schema warming、容量轮换、ready switch 和 orphan sweep；已发布的 building 索引仍只按 `BuildingIndexWritable` 规则接收增量写入。
