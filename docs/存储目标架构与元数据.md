...
- `PrimaryStoreRoute` 只负责 Pebble 在线事实主存的水平切分，并路由到 PrimaryStore Service 节点。`PrimaryStoreTarget` 是 PrimaryStore 内部解析后的执行目标，不暴露给用户协议，也不要求派生消费者理解。
...
modules/storage/
  ...
  infra/               # 具体底层实现
    ...
    metadata/sqlite/
    transport/
  services/            # 可独立部署或独立调度的角色
    primary/            # PrimaryStore + Metadata，统一事实读写入口
    primary/           # PrimaryStore Service，Pebble 在线事实主存
    view/              # DataView、ViewBuilder、DuckDB 和 Bleve
      builder/         # 行变更消费和增量物化
      search/          # Record View 的 Bleve 索引和搜索
    archive/           # Archive runtime，Parquet 事实冷备
```

`services/primary` 是唯一公开事实数据访问入口。它负责协议编排、鉴权、校验、元数据解释、PrimaryStoreRoute 解析、请求转发和统一错误码。View 与 Archive 需要回读完整事实行时，也通过 PrimaryStore 的读接口完成，不直接访问 `PrimaryStoreTarget` 或底层分片。`services/primary`、`services/view` 和 `services/archive` 是可独立部署或独立调度的内部执行角色；`services/view/search` 是 view 内部的 Bleve 能力，不是独立部署服务。单进程部署下 PrimaryStore 默认使用 LocalClient 访问 PrimaryStore，只有显式配置 `storage.primary.service_name` 时才走远程 PrimaryStore Service。`core` 只放领域抽象，`infra` 只放 SQLite、Pebble、DuckDB、Bleve、Parquet、NATS 等具体实现。
...
View 派生索引统一使用 `a`/`b` 双槽。`active_index_id` 是当前可读槽位；非活跃槽位的构建信息保存在独立的 `t_view_index_builds`。DuckDB 和 Bleve 都实现 `ViewIndexEngine`，但只有 `view_index` owner 能打开物理文件；builder/query 通过内部 ViewIndex RPC 调用 `Prepare`、`Write`、`Stat`、`Remove` 和查询接口。

唯一调度入口是 view_builder 上的 `op=maintain`：负责 schema 抢占、容量和保留范围切换、租约接管、游标续跑、追平、激活和 orphan 清理。schema、列来源、过滤条件、粒度或保留窗口变化会递增 `view_version` 并立即废弃旧构建声明。旧活动槽位持续服务到 `ActivateViewIndex` CAS 成功；新字段在新槽位激活前返回 `VIEW_NOT_READY`。
...
PrimaryStore 会按 PrimaryStoreRoute 把同批 rows 分组写入一个或多个 `PrimaryStoreTarget`。`PrimaryStoreTarget` 是内部执行目标，不是跨节点事务边界；跨 `PrimaryStoreTarget` 写入不承诺全局原子性，也不回滚已经成功写入的目标。若后续目标失败，已经成功写入的 rows 仍会发布对应的 TimeSeries 或 Record 事件，便于派生侧追上成功部分。

主存写入完成后发布 `TimeSeriesRowsChangedEvent` 或 `RecordRowsChangedEvent`。事件中的 rows 是变更提示，不要求携带完整行；Search、View 等派生消费者收到事件后通过 PrimaryStore 读接口回读最新完整行，再覆盖写入 DuckDB、Bleve 或其他派生结果，保证重试消费是幂等的。消费者不直接请求 `PrimaryStoreTarget`，也不理解分片、路由或底层设备细节。派生索引始终异步生成，不提供写后立即可搜契约；主链路只负责主存写入和事件发布。生产环境可把 eventbus 配置为 NATS，并通过 `subject_prefix` 和 `consumer_name` 配置统一 subject 前缀与 durable consumer。
...
`PrimaryStoreTarget` 表示 PrimaryStore 已完成路由后的主存执行目标。它包含目标 `node_id`、Pebble `device_id`、内部 `device_table` 和 PrimaryStore 节点 `endpoint`，只在 PrimaryStore 到 PrimaryStore 的内部调用中使用，不对普通用户暴露。它不是事务对象，storage 不在多个 `PrimaryStoreTarget` 之间提供原子提交。
