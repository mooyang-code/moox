...
如果是第一次阅读本系统，建议先读 `存储概念与设计意图.md`。该文档解释 Space、DataSource、Subject、Dataset、Field、Factor、View 等概念的边界，以及这些取舍背后的设计意图。服务角色、模块关系和单机/多机部署见 `存储服务架构与部署.md`。
...
- storage 事件 subject 使用统一前缀，默认 `moox.storage`；时序行变更事件默认 `moox.storage.time_series.rows_committed.v1`，记录行变更事件默认 `moox.storage.record.rows_committed.v1`，NATS 可用 `moox.storage.>` 订阅整组 storage 事件。
- 所有用户侧和业务侧事实数据访问都必须先进入 PrimaryStore Service。Collector、FactorCalculator、管理台和 CLI 不直接访问 PrimaryStore、DataView、Archive 或底层设备。
- `PrimaryStoreRoute` 只负责 Pebble 在线事实主存的水平切分，并路由到 PrimaryStore Service 节点。`ShardTarget` 是 PrimaryStore 内部解析后的执行目标，不暴露给用户协议，也不要求派生消费者理解。
...
flowchart TD
    ...
    subgraph PrimaryStore["PrimaryStore Service<br/>唯一公开数据访问入口"]
        API["API Handler"]
    ...
    subgraph ViewSvc["派生服务，可独立部署"]
        VS["View Service<br/>DuckDB + Bleve"]
        AS["Archive Runtime<br/>Parquet"]
    end
...
`primarystore` 是唯一公开事实数据访问入口。它负责协议编排、鉴权、校验、元数据解释、PrimaryStoreRoute 解析、请求转发和统一错误码。View 需要回读完整事实行时，也通过 DataShard 的读接口完成，不直接访问底层 Pebble。单进程部署下 PrimaryStore 默认使用 LocalClient 访问固定 `shard_id` 的 DataShard，只有显式配置 `storage.primary.service_name` 时才走远程 DataShard Service。
...
`View` 是用户查询入口。每个 View 都会异步构建一份派生查询索引；TimeSeries View 由 DuckDB 承载，Record View 由 Bleve 承载，但 View 概念本身不暴露底层表名或索引目录。
...
定义查询结果粒度
定义 retention_window
记录当前 active_index_id / active_view_version / active schema / coverage
投影当前持久化 ViewIndexBuild 租约和游标
```

`c_retention_window` 是派生索引的硬边界；未显式设置时，TimeSeries 按 View 的固定 `filter_json.freq` 选择维护配置中的窗口，Record 使用 Record 保留窗口且要求版本可规范化为时间。

View 派生索引统一使用 `a`/`b` 双槽。`active_index_id` 是当前可读槽位；非活跃槽位的构建信息保存在独立的 `t_view_builds`。DuckDB 和 Bleve 都实现 `ViewIndexEngine`，但只有 `view` owner 能打开物理文件；builder/query 通过内部 ViewIndex RPC 调用 `Prepare`、`Write`、`Stat`、`Remove` 和查询接口。

唯一调度入口是 view 上的 `op=maintain`：负责 schema 抢占、容量和保留范围切换、租约接管、游标续跑、追平、激活和 orphan 清理。schema、列来源、过滤条件、粒度或保留窗口变化会递增 `view_version` 并立即废弃旧构建声明。旧活动槽位持续服务到 `ActivateViewIndex` CAS 成功；新字段在新槽位激活前返回 `VIEW_NOT_READY`。

DuckDB 和 Bleve 索引都是可重建的近期派生结果，不是完整事实存储；完整历史以 PrimaryStore KV/Pebble 为准。每个 TimeSeries View 槽位是一份独立 `.duckdb` 文件且只有 `view_rows` 表；每个 Record View 槽位是独立 Bleve 目录。删除旧槽位会先拒绝新引用、等待在途查询结束、关闭句柄，再整库删除并立即回收空间。
...
```text
build_start_time = snapshot_end - view.c_retention_window
build_end_time = now
```

### PrimaryStoreNode

`PrimaryStoreNode` 表示 PrimaryStore Service 节点，也就是一个 Pebble 在线事实主存服务实例或服务组入口。PrimaryStore Service 根据 PrimaryStoreRoute 把事实写入和事实读取分发到不同 PrimaryStoreNode。
...
元数据控制面直接读写 SQLite，用于管理台、CLI 和控制面任务。PrimaryStore 的写入校验、PrimaryStoreRoute 解析、Search 索引列解释等服务读路径使用独立的 metadata cache。cache 启动时加载快照，后续刷新调度由 snapshotcache 组件负责；storage 不在元数据写入时手动刷新或更新 cache，也不要求“刚写入的元数据立刻被服务读路径读到”。

事实写入链路不负责写入 DatasetSubject 绑定关系；应用层、管理台或 CLI 可按自身业务需要独立维护 DatasetSubject，用于“列出 dataset 下有哪些 subject/record”、派生重建枚举等控制面场景。PrimaryStore 写入校验只校验 Dataset、列契约以及必要的 key 字段；TimeSeries 与 Record 数据只要携带合法 key 和已登记列即可写入。服务侧允许等 snapshotcache 刷新后再让读路径看到新绑定。派生索引补偿和重建统一由 view 的 `op=maintain` 完成，后台回扫仍通过 PrimaryStore 读路径从 PrimaryStore 获取完整事实行。
...
PrimaryStore 会按 PrimaryStoreRoute 把同批 rows 分组写入一个或多个 `ShardTarget`。`ShardTarget` 是内部执行目标，不是跨节点事务边界；跨 `ShardTarget` 写入不承诺全局原子性，也不回滚已经成功写入的目标。若后续目标失败，已经成功写入的 rows 仍会发布对应的 TimeSeries 或 Record 事件，便于派生侧追上成功部分。

主存写入完成后发布 `TimeSeriesRowsCommitted` 或 `RecordRowsCommitted`。事件中的 rows 是变更提示，不要求携带完整行；Search、View 等派生消费者收到事件后通过 PrimaryStore 读接口回读最新完整行，再覆盖写入 DuckDB、Bleve 或其他派生结果，保证重试消费是幂等的。消费者不直接请求 `ShardTarget`，也不理解分片、路由或底层设备细节。派生索引始终异步生成，不提供写后立即可搜契约；主链路只负责主存写入和事件发布。生产环境可把 eventbus 配置为 NATS，并通过 `subject_prefix` 和 `consumer_name` 配置统一 subject 前缀与 durable consumer。

### View 派生索引构建

DuckDB 和 Bleve 都只保存近期派生 View 索引。系统按事件增量写入活动槽位，以及版本、引擎和 schema 均匹配的 `BUILDING/CATCHING_UP` 槽位；定时 `op=maintain` 从 PrimaryStore 扫描保留窗口并构建非活跃槽位。

构建流程：

1. 读取 View 的 `primary_dataset_id`、Dataset 列表和 ViewColumn 列集合。
2. 捕获 `snapshot_end`，计算 `build_start_time = snapshot_end - c_retention_window`。
3. `ClaimViewIndexBuild` 先以 `PREPARING` 状态持久化 buildID、inactive indexID、schema 和 owner 租约。
4. owner 执行 `Prepare(indexID)`，再 CAS 到 `BUILDING`。
5. 从 PrimaryStore 分页回扫；每页写成功后持久化 cursor、coverage、entries 并续租。附属 Dataset 按批读取，禁止逐行 N+1 回读。
6. 回扫到 snapshot 后进入 `CATCHING_UP`，在固定 source end 上重扫 overlap 区间。
7. 校验 schema、覆盖范围、最小条数和 lag，CAS 到 `READY` 后调用 `ActivateViewIndex` 原子写入活动指针和 active schema。
8. 等待 `remove_grace`，由 owner 在引用排空后整库删除旧槽位或 orphan。

系统不在线 `ALTER` 当前活动索引。新增字段或因子时，新 buildID 抢占旧构建，旧活动槽位继续服务，旧构建立即停止双写。`storage.view.maintenance.enabled=false` 是完整 kill switch，不执行构建、激活或 orphan 清理。
...
```text
t|space|dataset|subject|freq|version|dimhash|row_id
...
写入协议不提供用户删除能力，不返回行变更明细，也不返回旧值。写入成功只表示在线事实主存已接受请求。若同一批请求被路由到多个 `ShardTarget`，成功目标不会因为后续目标失败而回滚。DuckDB、Bleve 和 Parquet 的派生结果由异步任务处理。
...
`SearchRecordRows` 使用 Space + View 维度，查询 `active_index_id` 指向的 Bleve 槽位。查询经 ViewIndex owner 完成，不走 PrimaryStoreRoute，也不 fan-out 到 Pebble 分片。进入索引的字段由 ViewColumn 决定；字段形态变化递增 `view_version`，新槽位激活后才对用户可见。`filters` 和 `sorts` 在 Bleve 内下推，不替代跨 Dataset 的组合分析查询。
...
`ShardTarget` 表示 PrimaryStore 已完成路由后的主存执行目标。它包含目标 `node_id`、Pebble `device_id`、内部 `device_table` 和 PrimaryStore 节点 `endpoint`，只在 PrimaryStore 到 PrimaryStore 的内部调用中使用，不对普通用户暴露。它不是事务对象，storage 不在多个 `ShardTarget` 之间提供原子提交。
...
c_grain_keys_json
c_filter_json
c_engine              // duckdb / bleve
c_retention_window
c_active_index_id
c_view_version
c_active_view_version
c_active_columns_json
c_active_schema_hash
c_indexed_from
c_indexed_to
c_status
c_attrs_json
...
```text
c_space_id / c_view_id
c_build_id / c_index_id / c_engine
c_target_view_version / c_state
c_owner_id / c_lease_expires_at
c_cursor_json / c_snapshot_end
c_coverage_start / c_coverage_end
c_entries_written
c_schema_hash / c_columns_json
c_started_at / c_updated_at / c_finished_at / c_error
```

每个 View 最多一条构建行。所有状态更新都要求 buildID、owner、预期状态和未过期租约匹配；激活在一个 SQLite 事务内复制活动指针、schema 和 coverage，并删除构建行。
...
c_node_id
c_name
c_engine              // pebble / duckdb / bleve / archive
c_endpoint
c_config_json
c_status
c_attrs_json
...
最终目标是保留旧 xData 的简单性，同时支持 PrimaryStore 统一入口、PrimaryStore 多实例、Pebble 主存、DuckDB View 宽表、Bleve 文本索引和 Parquet 事实归档。
