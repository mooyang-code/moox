...
- storage 事件 subject 使用统一前缀，默认 `moox.storage`；时序行变更事件默认 `moox.storage.time_series.rows_changed.v1`，Record committed event 默认 `moox.storage.record.rows_committed.v1`，NATS 可用 `moox.storage.>` 订阅整组 storage 事件。
- 所有用户侧和业务侧事实数据访问都必须先进入 Access Service。Collector、FactorCalculator、管理台和 CLI 不直接访问 PrimaryStore、DataView、Archive 或底层设备。
...
modules/storage/
  ...
  services/            # 可独立部署或独立调度的角色
    access/            # Access + Metadata，统一事实读写入口
...
`services/access` 是唯一公开事实数据访问入口。它负责协议编排、鉴权、校验、元数据解释、PrimaryStoreRoute 解析、请求转发和统一错误码。View 与 Archive 需要回读完整事实行时，也通过 Access 的读接口完成，不直接访问 `PrimaryStoreTarget` 或底层分片。`services/primary`、`services/view` 和 `services/archive` 是可独立部署或独立调度的内部执行角色；`services/view/search` 是 view 内部的 Bleve 能力，不是独立部署服务。单进程部署下 Access 默认使用 LocalClient 访问 PrimaryStore，只有显式配置 `storage.primary.service_name` 时才走远程 PrimaryStore Service。`core` 只放领域抽象，`infra` 只放 SQLite、Pebble、DuckDB、Bleve、Parquet、NATS 等具体实现。
...
sequenceDiagram
    ...
    C->>S: WriteTimeSeriesRows / UpsertRecordRows
...
事实写入链路不负责写入 DatasetSubject 绑定关系；应用层、管理台或 CLI 可按自身业务需要独立维护 DatasetSubject，用于“列出 dataset 下有哪些 subject/record”、派生重建枚举等控制面场景。Access 写入校验只校验 Dataset、列契约以及必要的 key 字段；TimeSeries 与 Record 数据只要携带合法 key 和已登记列即可写入。服务侧允许等 snapshotcache 刷新后再让读路径看到新绑定。派生索引补偿和重建统一由 view_builder 的 `op=maintain` 完成，后台回扫仍通过 Access 读路径从 PrimaryStore 获取完整事实行。

事实写入语义收敛为 TimeSeries 的 `key + data_time` 和 Record 的逻辑 key patch。Record 每次 mutation 由 PrimaryStore 分配不可变 revision，并在同一 Pebble 事务内更新 CURRENT、history、commit journal 和幂等记录。
...
主存写入完成后发布 `TimeSeriesRowsChangedEvent` 或携带完整行的 `RecordRowsCommittedEvent`。Record View 通过稳定 snapshot + `(after_commit_seq, through_commit_seq]` journal replay 重建，NATS 只提供低延迟触发；发布失败不回滚已提交 mutation。消费者不直接请求 `PrimaryStoreTarget`。
...
事实行按 TimeSeries 的 `data_time` 或 Record 的逻辑 key + server revision 存储。Record mutation 会同时写 CURRENT、HISTORY 和 commit journal；未携带的列不会被置空或删除。
...
```text
RecordKey(space_id, dataset_id, record_id)
internal_key = record_id
revision = server-assigned uint64
```
...
rh|space|dataset|record_id|revision
rc|space|dataset|record_id
j|commit_seq
```

TimeRange 固定为闭区间 `[start_time, end_time]`；HISTORY 使用数值 RevisionRange。
...
`ReadTimeSeriesRows` 使用 `TimeSeriesKey + TimeRange` 读取时序数据。`ReadRecordRows` 默认读取 CURRENT，显式 HISTORY 时使用数值 RevisionRange。
...
c_active_coverage_start
c_active_coverage_end
c_status
c_attrs_json
c_ctime
c_mtime
