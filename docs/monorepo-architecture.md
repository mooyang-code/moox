...
根目录新增 `build/`，存放跨模块编译、发布和打包脚本。根 `Makefile` 只做入口转发，不承载复杂逻辑。
...
1. 建立根目录骨架：`go.work`、`modules/`、`build/`、`skills/`、`configs/`、`deployments/`、`var/`。
...
8. 建立根 `Makefile` 与 `build/` 脚本，统一测试、构建和发布入口。
