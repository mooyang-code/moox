...
本轮使用 Colima 的 `moox-cross-builder` 容器取得了宿主机之外的可复现执行结果：`stockcn/tencent`、`markets`、`serverless/market_data` 定向测试，以及 `marketmanifest`、`marketcalendar`、`routeprobe` 公共库测试均通过；对应 `go vet` 通过。宿主机上遗留的不可中断测试进程不作为测试结果依据。
