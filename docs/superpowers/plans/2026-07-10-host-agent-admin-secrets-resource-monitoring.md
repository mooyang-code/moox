...
- 告警规则决策已补充：Monitor 启动时加载 enabled Host rules 到 `trpc.group/trpc-go/trpc-database/localcache`，消费链路只读内存缓存；规则变更主动失效并刷新，后台周期刷新失败时保留上一份有效快照。
...
| 告警规则缓存 | Host 告警消费路径禁止逐条查询 SQLite；使用 `trpc.group/trpc-go/trpc-database/localcache` 缓存规则快照，配置变更主动失效，后台周期刷新失败保留上一份快照。 |
