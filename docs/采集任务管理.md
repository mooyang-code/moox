...
> Collector 每分钟只协调任务、ActiveInstrumentSet、任务分片和公共 DNS 环境变量；SCF 不回调
> Collector/Admin，也不发布实时 Completion。每个函数独立保存一份环境变量；crypto 每函数最多
> 30 个标的，stockcn 按发布配置 N、Group 安全容量和完整 4KB 校验决定分片大小，
> 64MB/15s，聚合后一次写 Storage。stockcn 全市场 Instrument 快照由独立的每日 Timer SCF 执行；显式
> Backfill/GapRepair、
> 出口探针和人工 E2E 才使用 `InvokeFunction`。`crypto` 和 `stockcn` 共用 MarketProvider、Fetcher/Spec、InstrumentPipeline
...
`moox.toml` 中的 `function_count` 只计算实时 Timer 函数。若只手工创建 Timer 函数，Instrument
快照和显式 Backfill/GapRepair 会明确告警“无 active market fetcher nodes”，不会伪装成已完成。

`moox-collector` 是采集任务、短时 SCF 批次和补采重试的唯一控制面。行情采集不再使用常驻
SCF、CloudNode JobItem 或 SCF 心跳；一次 Timer 函数调用只处理一个有界批次，并在 15 秒内返回。

### Timer 节点与 Invoke 辅助节点

`moox-fetcher-<space>-timer-*` 是实时 Kline 节点：腾讯 Timer Trigger 到点后直接触发函数，
函数从自身 Environment 读取标的分片并写入 Storage。`moox-fetcher-stockcn-instrument-*` 是独立的每日
Instrument snapshot Timer 节点，不携带 Kline 分片。`moox-fetcher-crypto-market-invoke-*` 和
`moox-fetcher-stockcn-invoke-*` 是每个启用地域保留 1 个的辅助节点，没有 Timer，不会自动参与每分钟采集，只由
`InvokeFunction` 按需调用。

辅助节点用于显式 K 线 Backfill/GapRepair、出口探针和人工 E2E。保留地域级辅助节点是为了让这些任务也能使用对应地域的公网出口；
它们不计入 `function_count`，也不会因为闲置而产生每分钟的 Timer 运行时长。删除辅助节点不会影响实时 Kline Timer
或每日 Instrument snapshot Timer，但会使上述按需任务无法执行。

## 服务边界

| 服务 | 默认地址 | 职责 |
| --- | --- | --- |
| `moox-collector` | `127.0.0.1:11402` | CollectionTask、TaskInstance、BatchInvocation、RetryItem 与调度 |
...
```mermaid
flowchart LR
  Task["CollectionTask + Instrument Dataset"] --> Collector["Collector Timer 协调"]
  ...
  Collector -. "仅补采/探针" .-> Invoke["有界 InvokeFunction"]
  Collector --> InstrumentTimer["独立每日 Instrument Timer"]
  InstrumentTimer --> SCF
```

1. 用户创建并启用采集任务。
2. Collector 每分钟扫描启用规则和完整 ActiveInstrumentSet，按固定 N 个 Group 协调；crypto 保持每函数最多 30 个标的，stockcn 按压测安全容量和 4KB Environment 校验分片。
...
5. `BatchInvocation`、Completion Consumer 和 RetryItem 仅服务显式 Backfill/GapRepair 等有界 Invoke 路径；stockcn 的 Instrument snapshot Timer
   直接执行完整目录刷新，
   不作为实时 K 线健康真值。
...
8. Completion Consumer 只事务性更新有界 Invoke 的批次、TaskInstance 新鲜度和 RetryItem。
   实时 Timer 的 429、5xx、网络错误和临时 Storage 错误由下一周期重试覆盖；Collector 不自动扫描
   Storage 内部时间桶，Monitor freshness 只根据最新已提交 K 线事实告警。
...
Instrument 任务必须指定数据源、市场类型和 RECORD Dataset，并且始终从交易所读取全量活跃
...
K 线任务必须关联 Instrument Dataset，标的不再在 K 线任务中重复配置：
...
`stockcn` 的 K 线任务使用 `provider=stockcn_multi`、`market_type=equity`、
`symbol_dataset_id=dataset_stockcn_instruments` 和 `target_dataset_id=dataset_stockcn_equity_kline`。新浪、腾讯、
东方财富分别负责分片中的部分标的，当前 Provider 失败时最多顺序 fallback 一次；所有成功来源
仍写同一个 `dataset_stockcn_equity_kline`，`series_tag=default` 和 `source_provider` 字段保证同一分钟单行幂等。
...
这两个对象是 `CollectionTask` 的内容：外层 `data_type`、`provider`、`market_type` 写入任务；只有 `collect_params` 的对象序列化后写入 `CollectionTask.collect_params`。不要把外层字段放进 `collect_params`。

### K 线重采样任务

`kline_resample` 复用现有 `CollectionTask` 和 `TaskInstance`，不新增配置表。数据源选择一个已经生效的
...
```json
{
  ...
  "collect_params": {
    ...
    "target_dataset_id": "dataset_spot_kline_resample_5m",
    "target_frequency": "5m",
    "alignment": "epoch_utc",
...
通过覆盖区间和 active-task 排他检查避免同一 Subject/分钟并发来源覆盖。Backfill 从任务的
HistoryPolicy 覆盖下界向当前水位推进；GapRepair 只处理调用方明确给出的、位于覆盖区间内的缺口，
并受 `gap_repair_lookback`、批次大小、并发和速率预算约束。Collector 不执行周期性 Storage 缺口审计或自动补洞；
Monitor freshness 只观察指定 View series 的最新 output `data_time`；View input 与 output
`commit_timestamp` 只用于定位路由/写入链路，不把单个内部桶缺失误判为故障。Storage RowKey Upsert
允许少量重复调用，不会生成重复 K 线。

## 调度、监控与排障

管理台通过 `/api/admin/collectmgr/CreateTask` 创建任务，也可以读取
`GetTaskInstanceList`。Collector timer 是唯一行情执行入口，统一生成短时批次；已没有
`ScheduleTasks` 或旧 JobItem 调度接口。

Monitor 以 Storage 最新数据时间和提交时间作为实时业务真值；有界 Invoke 可额外消费 Completion 快照用于中文诊断。
短时函数未被调用时没有心跳是正常状态，不能用 `scf:heartbeat` 判定 Fetcher 是否在线。

### A股 1m Canary 与低基数指标

`stockcn/dataset_stockcn_equity_kline` Canary 使用 `modules/collector/config/markets/stockcn/calendar.yaml`
...
4. 显式 Backfill/GapRepair 再检查 `BatchInvocation`、`RetryItem` 和 `MarketFetchBatchCompleted` 事件。
5. 最后检查 Storage 的最新已收盘 K 线与 View。

`moox-cli collector function probe-egress` 会同步调用每个已部署 Fetcher：crypto 检查 Binance，
stockcn 检查新浪、腾讯、TDX 和东方财富的连通性。stockcn 的 expected/result/non-empty/distinct 公网 IP
统计只用于诊断，不再作为启用 Timer 和业务任务的门禁；A 股 Provider 按出口 IP 限频，不设置跨全部
函数的 Provider 总请求配额。探针不写 Storage、不发布完成事件。
...
Storage 数据、Monitor 30 秒 tRPC 检查以及连续两次 stale 结果的告警/恢复行为。实时验收证据使用函数名、Timer event 时间、CLS
