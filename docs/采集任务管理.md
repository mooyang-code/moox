...
> 64MB/15s，聚合后一次写 Storage。stockcn 全市场 Instrument 快照由独立的每日 Timer SCF 执行；缺口补采、
...
辅助节点用于 K 线缺口补采、出口探针和人工 E2E。保留地域级辅助节点是为了让这些任务也能使用对应地域的公网出口；
...
5. `BatchInvocation`、Completion Consumer 和 RetryItem 仅服务缺口补采等有界 Invoke 路径；stockcn 的 Instrument snapshot Timer
   直接执行完整目录刷新，
   不作为实时 K 线健康真值。
