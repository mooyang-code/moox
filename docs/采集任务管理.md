...
> 64MB/15s，聚合后一次写 Storage。stock_cn 全市场 Instrument 快照由独立的每日 Timer SCF 执行；缺口补采、
> 出口探针和人工 E2E 才使用 `InvokeFunction`。`crypto_market` 和 `stock_cn` 共用 MarketProvider、Fetcher/Spec、InstrumentPipeline
...
函数从自身 Environment 读取标的分片并写入 Storage。`moox-fetcher-stock-cn-instrument-*` 是独立的每日
...
5. `BatchInvocation`、Completion Consumer 和 RetryItem 仅服务缺口补采等有界 Invoke 路径；stock_cn 的 Instrument snapshot Timer
   直接执行完整目录刷新，
   不作为实时 K 线健康真值。
...
stock_cn 检查新浪、腾讯和东方财富的连通性。stock_cn 的 expected/result/non-empty/distinct 公网 IP
