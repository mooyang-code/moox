...
其中 `PollJobItems` / `ReportJobItemStatus` 维护 CloudNode JobItem 执行状态和结果，`ReportTaskStatus` 维护 collector 任务实例状态。
...
SCF runtime 不直接读取 collector 数据库。它通过 CloudNode JobItem 协议获取任务，并通过后台网关回报任务状态。storage 写入走直连 storage access，以降低转发流量和延迟。
