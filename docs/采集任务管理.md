...
`stockcn` 的 K 线 Rule 使用 `provider=stockcn_multi`、`market_type=equity`、
...
通过覆盖区间和 active-task 排他检查避免同一 Subject/分钟并发来源覆盖。Backfill 从 Rule 的
...
统计只用于诊断，不再作为启用 Timer 和业务 Rule 的门禁；A 股 Provider 按出口 IP 限频，不设置跨全部
