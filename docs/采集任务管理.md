...
> 本文描述当前已经实现的 Collector 控制面和 SCF 执行链路。多 Provider 聚合、`stock_cn` 等 Market Module、Canonical Dataset、完整性补洞和内置市场初始化属于目标架构，见 [内置市场行情采集架构](内置市场行情采集架构.md)；在对应代码落地前，不应把目标设计当作当前运行能力。
...
用户规则只表达 `market_id`、Feed、Instrument Type、频率和历史范围。Market Module 在内部选择 Provider、生成分片、写 Provider Landing Dataset、执行质量裁决，并把业务统一读取的数据写入 Canonical Dataset。
...
- `stock_cn.kline` 等查询接口只读 Storage Canonical Dataset，不在查询链路调用外部 Provider。
