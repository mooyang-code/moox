...
SCF、CloudNode JobItem 或 SCF 心跳；一次函数调用只处理一个有界批次，并在 15 秒内返回。
...
Symbol Rule 必须指定数据源、市场类型和 RECORD Dataset。`exchange` 从交易所读取全量活跃
USDT 现货快照；`manual` 仅采集用户指定的标的，最多 20 个。全量快照不填写 `symbols`：
...
真实环境依次验证：函数发布配置回读、出口探针、全量 Symbol 快照的真实 Storage 数据、
