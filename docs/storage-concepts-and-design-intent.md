...
- adapter 服务可以独立部署，让一个 storage 接入层把数据分散到多个 adapter 处理。
...
## StorageNode、Device 与 StorageRoute

`StorageNode` 表示存储代理节点，也就是一个 adapter 服务实例或服务组入口。它包裹一组底层 Device，对外提供统一的读写能力。

`Device` 表示底层具体存储组件，例如 Pebble、DuckDB、Bleve 或 Parquet 归档目录。

`StorageRoute` 只负责在线主存的水平切分，并路由到 StorageNode，不直接路由到 Device。

设计意图：

- storage 接入层只关心“把这批数据交给哪个 adapter”。
- adapter 内部再决定使用哪个 Pebble 设备。
- 这样可以让 adapter 独立部署，形成一个简单的分布式系统。
- DuckDB、Bleve、Parquet 不走在线写入路由，而是消费 Pebble 主存变更。
