...
| 上传到 COS | `modules/cloudnode/internal/rpc/package.go` + `internal/store` | 生成预签名 URL、校验 COS 对象并落库 |
| 管理台 API | `modules/admin` 网关 | 只做鉴权和转发 |
| SCF 运行时包构建 | `modules/collector` / `scripts/build/build-collector-scf-package.sh` | 生成 collector SCF zip |
...
c_status
c_error_message
c_is_deleted        # INTEGER 存储，协议/前端按 bool 语义使用
```
...
构建脚本只查询腾讯云中固定的 `moox/moox-application` 资源，不会在打包阶段创建或修改 CLS。资源不存在或索引未启用时会终止构建，并提示先使用 MooX Skill 自动建立或修复资源。
...
- 批量部署云函数节点时，cloudnode 根据节点、代码包和云账户调用 `internal/providers/tencentscf`。
