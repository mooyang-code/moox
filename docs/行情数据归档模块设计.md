...
flowchart LR
    ...
    Storage -->|"commit"| Event["moox.storage.rows_committed.time_series.v1.<shard>"]
...
| --- | --- |
| Stream | `MOOX_STORAGE` |
| Subject | `moox.storage.rows_committed.time_series.v1.<shard>` |
...
CLI 通过 Storage Metadata 枚举 DatasetSubject 和频率，通过 Storage PrimaryStore 分页读取行，然后写入同一 journal 和物化链路。`--subject`、`--freq`、`--start` 和 `--end` 均可选，但扩大范围前 CLI 必须输出预估分区数并要求显式 `--confirm`。
