...
| Space ID | Dataset ID |
| --- | --- |
| `stockcn` | `dataset_stockcn_equity_kline`, `index_kline` |
| `stockhk` | `ah_stock_kline_daily` |
| `stockus` | `stock_kline_daily` |
| `crypto` | `dataset_spot_kline_1h`, `dataset_perpetual_kline_1h` |
...
flowchart LR
    ...
    Storage -->|"committed upsert"| Event["moox.event.storage.dataset.rows.upserted.v2.<space>.<dataset>"]
...
| --- | --- |
| Stream | `MOOX_STORAGE` |
| Subject | `moox.event.storage.dataset.rows.upserted.v2.<space>.<dataset>` |
...
archive:
  ...
  sources:
    stockcn:
      datasets: [dataset_stockcn_equity_kline, index_kline]
    stockhk:
      datasets: [ah_stock_kline_daily]
    stockus:
      datasets: [stock_kline_daily]
    crypto:
      datasets: [dataset_spot_kline_1h, dataset_perpetual_kline_1h]
...
df = pd.read_parquet(
    "data/archive/crypto/dataset_spot_kline_1h/1h/BTC-USDT/"
    "series_tag=venue%3Abinance/"
