...
Storage 已有 `internal/services/archive` 和 `internal/infra/device/parquet` 原型。现有实现通过定时器回读 Storage，以随机归档 ID 生成文件，并把每个业务列拆成一条长表记录。它适合通用事实快照，不符合本设计的三项要求：
...
flowchart LR
    ...
    Journal --> Materializer["Parquet Materializer"]
    Materializer --> Local["Local Parquet Archive"]
...
modules/archive/
  ...
  internal/
    ...
    materializer/           # 分区合并、排序、去重和 Parquet 原子替换
    registry/               # ArchiveFile Metadata RPC
    cosstore/               # COS 上传、校验和重试
    reconcile/              # Storage Access 回填和水位修复
    health/                 # ready/live 与运行指标
```

模块直接复用公共的 `packages/jetstream`、`packages/messagepb` 和 `modules/storage/proto/gen`。它不复制 NATS 连接、MooX Message 包装或 Storage protobuf 契约。
...
当 UTC 进入新月后，物化器对上月所有 dirty partition 执行最终物化和校验，并将文件标记为 sealed。sealed 表示普通在线数据不再修改该文件，不表示拒绝迟到数据或人工回填。任何合法修订都可重新打开该月，生成新 generation，并重置 COS 校验状态。
...
6. 启动 COS 重试和后台校验循环。
7. journal 可写、物化器可用且 NATS 已连接后才将 readiness 设为 true。
...
- 不同 materializer worker 数下的顺序 IO、磁盘队列和归档延迟。
