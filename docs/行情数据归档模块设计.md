...
flowchart LR
    ...
    Storage -->|"committed upsert"| Event["moox.storage.dataset.rows.upserted.v1.<space>.<dataset>"]
...
| Consumer | `moox_archive_kline_v1`，由 Archive 创建并持有 |
...
`dimensions_json` 是时序行身份的一部分；无维度时固定为 `{}`。
...
archive:
  ...
  eventbus:
    ...
    stream: MOOX_STORAGE
    consumer: moox_archive_kline_v1
    fetch_batch: 128
    fetch_max_wait: 1s
