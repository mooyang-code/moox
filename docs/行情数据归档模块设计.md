...
本文定义 MooX 行情数据归档模块的目标架构。`series_tag` Parquet v2 改造尚未编码
完成。归档模块使用纯 Go 实现，作为独立的 `modules/archive` 运行，消费 Storage
提交成功后发布的 NATS JetStream 行变更事件，将已闭合 K 线按市场、数据集、频率、
标的和月份物化为本地 Parquet 文件。本地归档永久保留；COS 是可选的远程副本，
不参与在线写入和本地读取。
...
文件名使用五个固定分区字段，并以双下划线 `__` 分隔。双下划线是保留分隔符；
分区字段中的字面 `__` 必须 percent-encode。文件名可以恢复 Space、Dataset、
Subject、频率和月份，但 `series_tag` 是文件内行身份，不进入路径或文件名。
...
│   │   └── 1h/
│   │       └── BTC-USDT/
│   │           ├── crypto__spot_kline_1h__BTC-USDT__1h__202606.parquet
│   │           └── crypto__spot_kline_1h__BTC-USDT__1h__202607.parquet
│   └── perpetual_kline_1h/
│       └── 1h/
│           └── BTC-USDT-SWAP/
│               └── crypto__perpetual_kline_1h__BTC-USDT-SWAP__1h__202607.parquet
├── stock_cn/
├── stock_hk/
└── stock_us/
```
...
再按固定命名规则定位文件。扫描散落的原子文件时，读取器从文件名恢复分区身份，
再从 Parquet 系统列读取并校验行级 `series_tag`。
...
| `series_tag` | UTF-8 string | 非空；默认序列固定为 `""`，不解析 |
...
assert not df.duplicated(["candle_begin_time", "series_tag"]).any()
```
...
1. 四个内置 Market Space 的启用 K 线 Dataset 能按约定路径生成月分区 Parquet，
   文件名可独立恢复 Space、Dataset、Subject、频率和月份，行内 `series_tag`
   保持不变。
