...
```text
{archive_root}/{space_id}/{dataset_id}/{freq}/{subject_id}/{YYYYMM}.parquet
```
...
月份使用归一化后的 UTC `data_time` 计算。时间范围查询先计算涉及的 `YYYYMM`，再读取对应文件。
...
```text
{prefix}/{space_id}/{dataset_id}/{freq}/{subject_id}/{YYYYMM}.parquet
```
...
df = pd.read_parquet("data/archive/crypto_binance/spot_kline/1m/BTC-USDT/202607.parquet")
...
1. 四个内置 Market Space 的启用 K 线 Dataset 能按约定路径生成月分区 Parquet。
