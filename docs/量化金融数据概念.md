...
`data_node_id -> service_target`。当前 Schema v6 不保留独立拓扑表、路由 seed 或
在线迁移；TimeSeries 行身份额外包含一个不透明 scalar `series_tag`。
```

`series_tag` 不拆分 name/value，也不进入 Dataset metadata。精确空 tag 是默认序列；
范围 selector 不设置 tag 才查询全部序列。相同 Schema、频率和生命周期的加密货币行情
使用共享 Dataset，以 `venue:binance`、`venue:okx` 区分。
