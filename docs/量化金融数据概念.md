...
| `PrimaryStore` | 唯一公开数据访问入口，负责校验、元数据解释、路由和请求转发。 |
...
- 写入事实数据：使用 `PrimaryStore.MergeTimeSeriesRows` 或 `PrimaryStore.MergeRecordRows`，入口是 `Dataset`。
...
PrimaryStore 读写接口通过 `PrimaryStoreRoute` 访问 PrimaryStore。`SearchRecordRows` 查询 Search Service 中的集中式索引，不走 PrimaryStoreRoute，也不 fan-out 到多个 Pebble 分片。`QueryTimeSeriesRows` 查询 View Service 中的物化结果。
