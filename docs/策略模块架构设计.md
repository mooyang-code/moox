...
策略开发者只编写 `strategy.yaml` 和 `strategy.py`。Python 不访问 Storage、Trade、账户、网络或文件，也不知道当前处于观察、回测、模拟还是实盘环境。完整 Python 契约见 [Strategy Python 策略接入手册](策略模块Python策略接入手册.md)。
