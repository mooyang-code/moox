...
当前仓库已提供独立 `moox-strategy` 服务和 CLI、Python SDK/worker、SQLite schema 与状态提交、RunOnce/查询/运维 RPC、快照 hash、确定性回放、Observe/Paper 端口、事务 Outbox、JetStream relay、健康检查和管理台。构建、release 与 `deploy-moox.sh` 标准部署链路均已接入，默认服务端口为 RPC `11430`，健康检查 `11431`。以下仍是明确的能力边界：

- 管理台已提供概览、运行中策略、运行详情、目标、状态、健康和绩效查询，以及暂停、恢复和执行模式操作；仍坚持“只展示和运维，不在线编辑策略源码”。
- Outbox relay 已连接生产 JetStream，并在断线时保留未发布的 `trade.rebalance.requested`、重连后继续发布。
- Strategy 不消费 Storage 事件；运行调度使用本地数据库和 Timer，并通过 Storage RPC 读取所需数据。
...
flowchart LR
  ...
  Factor --> Storage
  Strategy --> Storage
  Strategy --> EventBus["EventBus"]
  EventBus --> Trade["Trade"]
  Admin["Admin 浏览器控制面"] --> Strategy
...
- 按本地调度点通过 Storage RPC 查询所需数据，不依赖 Storage 事件唤醒。
...
当前 Factor 不发布“某一 Bar 的全市场因子已完成”事件。V1 Strategy 由本地调度器发起运行，并通过 View 查询确认目标 Bar 已物化。数据完整性检查不能被简单的唤醒事件替代。
...
Admin 只做代理和服务发现，不实现 Strategy 业务。EventBus 只传输 Strategy 发给 Trade 的调仓命令；Strategy Outbox 负责发布恢复，业务真相仍保存在 Strategy SQLite 与 Trade 数据库中。

### 前端管理台边界

前端管理台**不提供策略 Python 源码输入或在线编辑入口**。策略源码由后端导入/发布接口提交为带版本号、源码 hash 和依赖信息的策略包，再进入 Strategy。管理台可以展示源码 hash、版本和发布信息，但不直接修改已发布源码。

管理台面向运行运维，至少提供以下视图：

- **运行策略列表**：策略 ID、版本、Binding、Space、运行模式（observe/paper/live）、启停状态、最近运行时间、最近数据 revision、状态 revision、worker 健康、最近错误和延迟。
- **策略详情**：当前理论目标、组合目标、实际持仓、目标与实际偏差、最近运行输入/输出 hash、源码 hash、数据版本和状态变更记录。
- **表现面板**：累计收益、区间收益、日收益、最大回撤、净值曲线、换手率、费用、波动率、胜率及运行成功率。指标必须带 `performance_source` 标识：回测使用虚拟成交，observe 使用理论目标，paper 使用模拟成交，live 使用 Trade 对账后的真实成交。
- **运行与风险操作**：暂停/恢复 Binding、切换 observe/paper、查看失败运行和执行未知状态；不提供版本回滚操作，也不允许在页面上修改正在运行版本的代码。

策略性能不是 Python worker 直接上报的结果。Strategy 记录决策和目标，Paper/Trade 记录成交与持仓，Performance 服务或查询层按统一版本、数据 revision 和执行来源聚合指标，避免把回测收益误显示为实盘收益。
...
└── internal/
    ...
    ├── outbox/                  # Trade 调仓命令的 Outbox relay
    ├── rpc/                     # StrategyMgr
...
| `t_strategy_outbox` | 发往 Trade 的调仓命令 | `message_id` |
...
V1 使用本地数据库保存 Binding 和逻辑调度点，由 Strategy Timer 触发到期运行。运行时按 Binding 的 View、Space、freq 和标的范围通过 Storage RPC 读取数据。进程重启后根据数据库状态恢复遗漏调度点，不消费 Storage 事件。
...
Strategy 只定义 `StrategyTask` 和 `StrategyOutput` 的业务 payload；Strategy 现有 worker 管理、控制面和业务运行时保持独立。本次 Factor runtime 约束不迁移、不删除 Strategy 实现，也不把 Python runtime 依赖扩散到其他业务服务。
...
4. 对每个启用的 paper/live ExecutionBinding 生成一条可执行的 `trade.rebalance.requested`，并写入同一事务的 Outbox；observe 不产生调仓命令。

### `hold`

Go 只保存 StrategyRun 和 `next_state`，CAS 推进状态，继续引用旧 TargetWeights；不创建调仓命令。旧目标尚未完成时，执行层继续收敛。
...
Backtest 在 Strategy 内使用历史快照生成虚拟成交。实时 `paper/live` 不在 Strategy
计算目标数量：Strategy 只发布固定资金和完整目标权重，Trade 使用公开 instrument
价格与规则、当前仓位生成确定性目标数量，并负责反向先平后开和订单恢复。
...
- 使用 Trade 中的持久化模拟账户、账本、订单和仓位。
- 与 Live 使用相同触发、数据快照、状态、组合和风控链路。
- 只允许绑定模拟通道，由 Trade 本地即时成交处理器推进，不允许调用或回退到真实通道。

### LiveTradeExecution

1. 从 ExecutionBinding 取得账户、真实通道、固定执行资金和报价资产。
2. 将 GroupTargetWeights、数据 revision 和稳定幂等键写入
   `trade.rebalance.requested` Outbox。
3. Trade 校验通道和交易规则，读取公开价格与本地当前仓位并生成 FullTarget 调仓计划。
4. Trade 持久化 run，执行真实订单，并通过私有流和 REST 持续对账。
...
使用 work queue Stream：

```yaml
name: MOOX_TRADE
subjects: ["moox.trade.rebalance.requested.v1.>"]
max_age: 168h
```
...
| --- | --- | --- |
| `moox.trade.rebalance.requested.v1.<space>.<execution_binding>` | `RebalanceRequested` | 请求 Trade 按账户、通道、固定资金和完整目标权重调仓。 |

Strategy 不创建 Consumer。调仓命令只从事务 Outbox relay 发布；RPC 或 SQLite 事务中不直接发布 NATS。Trade 创建并持有 `trade_rebalance_v1` Consumer，并用 Inbox 对 `event_id` 幂等。

## RPC 与 CLI

`StrategyMgr` 由每台服务器的 Node Gateway 按 SysDeploy 路由暴露。当前 RPC `11430`，健康检查 `11431`，Prometheus metrics `12930`，均只监听 loopback；标准部署和健康脚本使用这些固定端口。

主要 RPC：

```text
CreateStrategy / RunOnce / GetEngineStatus
ListRunningStrategies / GetStrategyOverview
ListStrategyRuns / GetStrategyRun / ListStrategyTargets
GetStrategyStateSummary / GetStrategyHealth / GetStrategyPerformance
PauseBinding / ResumeBinding / SetExecutionMode
```

目标架构中的 Definition/Binding/Group 完整 CRUD、ResetState、Backtest Job 和 execution retry 尚未进入当前 RPC；新增时必须继续遵守状态 CAS、审计和幂等边界。`ResetState` 属于高风险操作，必须暂停 Binding，记录操作者、旧 revision 和原因，并从新 revision 恢复，不能静默删除状态。
...
`validate` 校验 `strategy.yaml` 和 Python 源码并输出 source hash；`run-once` 使用真实 worker 协议执行一次纯计算。数据库初始化由服务启动时自动应用嵌入 schema，不提供独立 `init` 命令。

## 配置

当前 `config/app.yaml` 的运行配置为：

```yaml
python_bin: python3
worker_path: ./pyworker/worker.py
database: ./data/strategy.sqlite
live_enabled: false
workers: 4
instance_id: strategy-1
eventbus:
  urls: [nats://127.0.0.1:4222]
  credential_file: ~/.config/moox/eventbus/strategy-eventbus.yaml
  relay_interval: 1s
...
标准部署把数据库和虚拟环境改写到部署目录的 `data/strategy`，打包共享 `python-runtime`，并创建只包含声明依赖的 venv。启动时 worker 握手或 EventBus 发布探针失败即判为未就绪，不能降级到本机其他 Python 环境。
...
- Python worker 握手成功。
- EventBus 连接和发布探针正常。
- Outbox 状态可查询；响应明细包含 pending 数量和最老消息年龄。

Strategy 不依赖 Storage trigger；paper/live 调仓通过 Outbox 发往 Trade。tRPC Prometheus exporter 监听 `127.0.0.1:12930`；下列 `moox_strategy_` 领域指标属于目标契约，尚未全部实现：
...
- 真实 SQLite 事务、Outbox relay 和重启恢复。
- 进程内 NATS Server 的发布去重和断线恢复。
- View 物化延迟、数据缺口、late revision 和 reconcile。
- Python worker 超时、崩溃、stderr 和版本加载失败。
- 多 Binding 与每个 ExecutionBinding 的调仓命令幂等。
...
1. **已完成**：`go.work` 已包含 `modules/strategy` 和 `modules/strategy/proto/strategygen`。
2. **已完成**：根 `Makefile`、`scripts/build.sh` 和 proto target 已包含 server/CLI。
3. **已完成**：EventBus 拓扑、Strategy 发布身份、事务 Outbox、重连 relay 和凭据路径已接入；Strategy 按设计不消费 Storage 事件。
4. **已完成**：Admin SysDeploy 已注册 `moox_strategy` / `strategymgr`，RPC `11430`、health `11431`；前端按 capability 隐藏不可达入口。
5. **已完成**：构建、发布和标准部署链路已接入二进制、配置、策略包、Python runtime、启停、健康检查、保留式升级和 `--no-strategy`，并有实际编译部署 E2E。
6. **部分完成**：Strategy 到 Trade 的稳定调仓命令、归因字段和幂等消费已接入；真实交易所 live 远端验收仍需单独完成，默认 `live_enabled: false`。
7. **已完成**：模块 README、架构总览、发布部署说明和端口表已同步；本文继续区分当前实现与目标架构。

## 交付顺序

1. **M1：契约与 run-once，已完成**。策略包、Python SDK、worker 握手与模块缓存、输出校验和 CLI。
2. **M2：回测，核心完成**。HistoricalReplayer、隔离状态和 BacktestExecution 已实现；Backtest Job RPC 和管理台尚未提供。
3. **M3：实时运行与 Paper，核心完成**。本地调度、readiness、状态 CAS、Outbox 和 paper 调仓命令已实现。
4. **M4：策略组合，核心完成**。固定资金聚合、组合风控、ObserveExecution 和多周期成员目标已有领域实现。
5. **M5：Live Trade，部分完成**。ExecutionBinding、调仓命令和 Trade 幂等接收已实现；真实交易所提交、结果查询和目标偏差对账仍需目标环境联调，默认禁用 live。
6. **M6：管理面与运维，大部分完成**。RPC、Node Gateway 路由、管理台、健康、构建、发布、标准部署和编译部署 E2E 已完成；领域指标和真实 live 远端验收仍随 M5 完成。
