...
`modules/strategy` 是独立 Go 服务。它读取 Storage 中的行情与因子宽表，调用版本化 Python 策略生成目标权重，聚合多个策略的资金分配，执行组合级风控，并把最终目标交给回测、模拟或实盘执行端口。

策略开发者只编写 `strategy.yaml` 和 `strategy.py`。Python 不访问 Storage、Trade、账户、网络或文件，也不知道当前处于回测、模拟还是实盘环境。完整 Python 契约见 [Strategy Python 策略接入手册](策略模块Python策略接入手册.md)。
...
本文描述目标架构，不把尚未存在的能力写成现状。当前仓库需要补齐以下前置条件：

- 尚无 `modules/strategy`、Strategy proto、数据库和 EventBus stream。
...
- Storage 时序行支持通用 attributes，但 Factor 当前写回没有记录列级 source hash、computed_at 或数据可见时间。
- Factor 当前没有全市场 Bar 完成事件；一次运行以 per-symbol 任务为粒度。
- Trade `TargetPosition` 只有 `symbol/quantity`，Rebalance 缺少 Strategy 归因和查询接口，完成事件仍使用 `BytesValue`。
...
flowchart TB
  ...
  subgraph strategy["moox-strategy"]
    ...
    Allocator["strategy group allocator"]
    Risk["risk gate"]
  ...
  State --> Decision
  Decision --> State
  Decision --> Allocator --> Risk --> Intent
  Intent --> Backtest
  Intent --> Paper
  Intent --> Live
...
└── internal/
    ...
    ├── decision/                # 输出校验、状态 CAS 和决策提交
    ├── allocation/              # 策略组合固定资金聚合
    ├── risk/                    # 组合目标门禁
    ├── execution/               # backtest/paper/live ExecutionPort
...
账户、执行模式和硬风控放在 StrategyGroup，不放在 Python 包中。一个策略版本可创建多个 Binding，各自持有独立状态。

### StrategyGroup

StrategyGroup 表示共享账户或虚拟账户的一组策略：
...
`next_state` 是完整替换快照。Decision 使用读取时的 `state_revision` 做 CAS；旧状态上的晚到响应不能覆盖新状态。状态只描述策略决策记忆，不能保存“订单已成交”或实际账户仓位。
...
decision_time + data_cutoff + data_revision
...
稳定幂等键由 `binding_id + strategy_version + trigger_bar_time + live_namespace` 生成。晚到数据 revision 不自动改写已接受实盘决策；显式回放使用新的 namespace。
...
```text
execution_id
group_id + group_target_revision
mode
account_id + channel_id
decision_time + execution_latency
target_weights_hash
...
| `t_strategy_groups` | 账户/虚拟账户与组合风控 | `group_id` |
| `t_strategy_states` | Binding 当前状态快照 | `binding_id` |
| `t_strategy_runs` | 每次决策运行事实 | `decision_id`、逻辑调度点唯一键 |
| `t_strategy_target_weights` | Binding 完整目标权重 | `(decision_id, instrument_id)` |
...
| `t_strategy_execution_requests` | 执行意图和下游关联 | `execution_id`、`idempotency_key` |
...
6. `available_at <= data_cutoff <= decision_time`。
...
HistoricalReplayer 不消费 NATS。它按 Binding 的交易日历和 schedule 生成与实时相同的 DecisionTask，并查询固定 `data_revision` 下当时可见的数据。历史回放必须保留当时的 universe、上市/下架状态、因子版本和 available_at，禁止幸存者偏差和修订数据泄漏。
...
- DecisionTask 有稳定 `decision_id`；重复事件返回已有结果。
...
`StrategyOutput` 包含 `decision`、`targets`、`next_state` 和可选 `debug_info`。协议细节以接入手册为准。
...
- 单任务超时后 kill 并重启 worker，使用同一 DecisionTask 重试。
- worker 崩溃不丢状态；状态只在 Go 事务提交后推进。
- 源码更新必须发布新版本，不对已启用版本热改文件。
- stderr 由 Go 收集并关联 `decision_id`，stdout 只用于帧协议。
...
| SQLite 事务失败 | 不发布事件，可使用同一 decision_id 重试。 |
...
三个端口接收同一种 GroupTargetWeights 和执行参数。

三个端口都调用 Strategy 内的共享 `TargetQuantityBuilder`，但各自提供账户与市场快照。该服务只负责按权益、价格、lot size 和预算把权重转成确定性目标数量。真实账户的目标减当前、反向先平后开和订单恢复仍由 Trade 负责；模拟执行器按同一公开语义实现并通过一致性测试校验。

### BacktestExecution

- 使用隔离的虚拟账户、余额、持仓和成交记录。
- 最早成交时间满足 `execution_time >= decision_time + execution_latency`。
...
1. 从 Trade 获取账户余额、当前仓位和交易规则快照。
2. 使用 GroupTargetWeights、账户权益和市场价格生成确定性目标数量。
3. 持久化 snapshot ID、rules version 和稳定幂等键。
4. 调用 Trade `CreateRebalance(target_mode=FULL)`。
5. 保存 Trade run ID，消费或查询调仓结果，持续对账目标偏差。
...
| --- | --- | --- |
| `moox.strategy.decision.accepted.v1` | `StrategyDecisionAccepted` | Binding 接受新完整目标。 |
| `moox.strategy.group_target.ready.v1` | `StrategyGroupTargetReady` | 组合目标通过聚合和风控。 |
| `moox.strategy.execution.requested.v1` | `StrategyExecutionRequested` | 请求 backtest/paper/live 执行。 |
...
| Durable | Filter |
| --- | --- |
| `strategy_group_target_v1` | `moox.strategy.decision.accepted.v1` |
...
标签只使用有限枚举，不放 strategy_id、binding_id、decision_id、instrument_id 或账户 ID。业务 ID 放日志和 TraceContext。
...
- state revision CAS、decision/execution 幂等键。
...
- Backtest 与实时 replay 在相同输入下产生一致 decision hash。
...
7. Paper 与 Live 输入相同时产生相同 GroupTargetWeights。
...
- 参考工程为专有软件。本模块只吸收“目标驱动、策略与执行分离、回测与实盘共核”等通用领域思想，采用独立协议、目录和实现。
...
1. **M1：契约与 run-once**。策略包、registry、Python worker、输出校验和 CLI。
2. **M2：回测**。HistoricalReplayer、隔离状态、BacktestExecution 和结果产物。
3. **M3：实时决策与 Paper**。Storage trigger、readiness、状态 CAS、Outbox 和虚拟账户。
4. **M4：策略组合**。固定资金聚合、组合风控和多周期成员目标。
5. **M5：Live Trade**。Trade 协议改造、FULL 调仓、结果查询和目标偏差对账。
6. **M6：管理面与运维**。RPC、Admin 转发、指标、告警、发布和远端验证。

每个阶段都必须保留同一 Python 协议和 DecisionEngine；禁止先写一套回测策略逻辑，再另写实盘适配逻辑。
