...
- 当前首版尚未提供完整前端管理台；管理台建设应遵循“只展示和运维，不在线编辑策略源码”的边界。
...
前端管理台**不提供策略 Python 源码输入或在线编辑入口**。策略源码由 Git/CI 或受控发布流程生成带版本号、源码 hash 和依赖锁定的策略包，再通过 `ImportStrategy`/发布接口进入 Strategy。管理台可以展示源码 hash、版本、发布人和代码仓库链接，但不直接修改已发布源码。
...
- **运行与风险操作**：暂停/恢复 Binding、切换 observe/paper、查看失败运行和执行未知状态、回滚到已验证版本；不允许在页面上修改正在运行版本的代码。
