...
- Outbox relay 已连接生产 JetStream，并在断线时保留未发布事实、重连后继续发布；Storage 更新事件的 durable trigger consumer 和 Trade live client 仍未接入。
- Storage 当前事件主题是 `moox.storage.rows_committed.time_series.v1.<shard>`。
...
- 消费 `moox.storage.rows_committed.time_series.v1.<shard>` 作为实时触发信号。
...
Storage trigger 和 Live Trade 尚未接入，因此当前 readiness 不伪造这两项依赖检查。tRPC Prometheus exporter 监听 `127.0.0.1:12930`；下列 `moox_strategy_` 领域指标属于目标契约，尚未全部实现：
...
3. **已完成**：EventBus 拓扑、strategy 发布身份、事务 Outbox、重连 relay 和凭据路径已接入；Storage trigger consumer 仍待实现。
...
6. **部分完成**：Trade 已保留 Strategy 归因字段；真实 live 提交、结果未知查询和稳定事件仍需跨模块联调，默认 `live_enabled: false`。
...
3. **M3：实时运行与 Paper，部分完成**。readiness、状态 CAS、Outbox 和虚拟账户已实现；Storage trigger consumer 尚未接入。
4. **M4：策略组合，核心完成**。固定资金聚合、组合风控、ObserveExecution 和多周期成员目标已有领域实现。
5. **M5：Live Trade，未完成**。ExecutionBinding 和归因模型已预留；真实 Trade 提交、结果查询和目标偏差对账尚未联调，默认禁用 live。
