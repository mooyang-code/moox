...
| `view` | `trpc.moox.storage.DataView`，以及对应 HTTP 服务；view_builder 上的 `view.timer?op=rotate` | DuckDB、Bleve | 消费行变更事件，通过 Access 回读最新事实行，构建 TimeSeries DuckDB View 和 Record Bleve 索引，并通过统一 rotate 生命周期切换 active/building 索引 |
...
`QueryTimeSeriesRows` 和 `SearchRecordRows` 都只读取当前 `active_result` 指向的派生索引。旧 active 会持续服务到 warming 索引完成并 `CompleteViewBuild` 切换，因此 schema 或容量轮换期间没有读空窗。新字段在新索引切换为 active 前不可查询；完整历史仍以 PrimaryStore KV/Pebble 为准。
...
实时派生消费者只处理进程启动后新产生的行变更事件。历史补仓、断档追数和读模型重建统一通过 view_builder 的 `op=rotate` 完成：schema 变化会抢占式 warming，新旧索引通过 active/building 双指针零空窗切换；不再提供 `RebuildTimeSeriesView` / `RebuildRecordView` 手动 RPC，也不通过事件消费者回放历史消息。
...
View 索引 lifecycle 计时器只应在承担 view_builder 职责的进程注册，调度参数统一使用 `op=rotate`。`op=rotate` 负责 schema 抢占重建、容量轮换、stale/failed warming 清理、ready switch 和未引用物理索引删除；独立 pending rebuild、cleanup、retry_failed 计时器已经删除。

相关业务配置位于 `storage.view.rotation`：
...
`rotation.enabled=false` 是完整 kill switch：跳过整个 rotate pass，不做 schema warming、容量轮换、ready switch 或 orphan sweep。DuckDB `__latest` helper 仍保留，待 rotation 稳定且 K 线查询性能复核后再作为后续任务删除。
