...
// TaskInstanceService 采集任务实例服务接口
...
func NewTaskInstanceService(instanceDAO dao.CollectorTaskInstanceDAO, taskConfigDAO dao.CollectorTaskConfigDAO, nodeDAO cloudnodedao.CloudNodeDAO, heartbeatDAO cloudnodedao.HeartbeatDAO) TaskInstanceService {
...
func (s *taskInstanceServiceImpl) CreateTaskInstance(ctx context.Context, instance *model.CollectorTaskInstance) error {
	// 验证必填字段
	if instance.TaskID == "" {
	...
	// 验证节点存在且在线
	node, err := s.nodeDAO.GetCloudNode(ctx, instance.NodeID)
	if err != nil {
	...
	// 检查节点状态
	nodeStatus, err := s.heartbeatDAO.GetNodeStatus(ctx, instance.NodeID)
	if err != nil {
		return fmt.Errorf("failed to get node status: %w", err)
	}
	if nodeStatus == nil || *nodeStatus != types.NodeStatusOnline {
...
func (s *taskInstanceServiceImpl) BatchCreateTaskInstances(ctx context.Context, taskID string, nodeAssignments map[string][]string) error {
	if taskID == "" {
		return fmt.Errorf("task ID is required")
	}
	...
	for nodeID, objects := range nodeAssignments {
		...
		// 检查节点状态
		nodeStatus, err := s.heartbeatDAO.GetNodeStatus(ctx, nodeID)
		if err != nil {
			return fmt.Errorf("failed to get node %s status: %w", nodeID, err)
		}
		if nodeStatus == nil || *nodeStatus != types.NodeStatusOnline {
			continue // 跳过离线节点
		}
...
func (s *taskInstanceServiceImpl) GetTaskInstance(ctx context.Context, instanceID string) (*model.CollectorTaskInstance, error) {
	if instanceID == "" {
...
func (s *taskInstanceServiceImpl) GetTaskInstancesByNode(ctx context.Context, nodeID string, status []int) ([]*model.CollectorTaskInstance, error) {
	if nodeID == "" {
...
func (s *taskInstanceServiceImpl) GetTaskInstancesByTask(ctx context.Context, taskID string, limit int) ([]*model.CollectorTaskInstance, error) {
	if taskID == "" {
...
func (s *taskInstanceServiceImpl) GetPendingInstances(ctx context.Context, nodeID string) ([]*model.CollectorTaskInstance, error) {
...
func (s *taskInstanceServiceImpl) GetRunningInstances(ctx context.Context, nodeID string) ([]*model.CollectorTaskInstance, error) {
...
func (s *taskInstanceServiceImpl) GetRecentInstances(ctx context.Context, hours int) ([]*model.CollectorTaskInstance, error) {
	if hours <= 0 {
		hours = 24 // 默认24小时
	}
...
func (s *taskInstanceServiceImpl) UpdateInstanceStatus(ctx context.Context, instanceID string, status int, result string) error {
	if instanceID == "" {
		return fmt.Errorf("instance ID is required")
	}
...
func (s *taskInstanceServiceImpl) CleanupOldInstances(ctx context.Context, days int) error {
	if days <= 0 {
		days = 30 // 默认保留30天
	}
...
func (s *taskInstanceServiceImpl) RetryFailedInstance(ctx context.Context, instanceID string) error {
	if instanceID == "" {
		return fmt.Errorf("instance ID is required")
	}
...
func (s *taskInstanceServiceImpl) CancelInstance(ctx context.Context, instanceID string) error {
	if instanceID == "" {
		return fmt.Errorf("instance ID is required")
	}
...
func (s *taskInstanceServiceImpl) GetInstanceLogs(ctx context.Context, instanceID string) (string, error) {
	if instanceID == "" {
		return "", fmt.Errorf("instance ID is required")
	}
...
func (s *taskInstanceServiceImpl) getStatusString(status int) string {
	switch status {
...
func (s *taskInstanceServiceImpl) generateInstanceID() string {
	return fmt.Sprintf("inst-%s-%d", uuid.New().String()[:8], time.Now().Unix())
}

// validateInstanceJSONFields 验证实例JSON字段
func (s *taskInstanceServiceImpl) validateInstanceJSONFields(instance *model.CollectorTaskInstance) error {
	// 验证TargetObjects
...
func (s *taskInstanceServiceImpl) mergeInstanceParams(instance *model.CollectorTaskInstance, taskConfig *model.CollectorTaskConfig) error {
	// 解析任务配置的采集参数
...
func (s *taskInstanceServiceImpl) GetTaskInstanceList(ctx context.Context, nodeID string, limit, offset int) ([]*model.CollectorTaskInstance, error) {
	if limit <= 0 {
		limit = 50 // 默认50条
	}
	if offset < 0 {
...
func (s *taskInstanceServiceImpl) UpdateTaskInstance(ctx context.Context, instanceID string, instance *model.CollectorTaskInstance) error {
	if instanceID == "" {
		return fmt.Errorf("instance ID is required")
	}
	if instance == nil {
...
func (s *taskInstanceServiceImpl) StartTaskInstance(ctx context.Context, instanceID string) error {
	return s.StartInstance(ctx, instanceID)
}

// StopTaskInstance 停止任务实例
func (s *taskInstanceServiceImpl) StopTaskInstance(ctx context.Context, instanceID string) error {
	if instanceID == "" {
		return fmt.Errorf("instance ID is required")
	}
