...
// TickerDistributor Ticker分配器
type TickerDistributor struct {
	base *BaseDistributor
}

// NewTickerDistributor 创建Ticker分配器
func NewTickerDistributor(base *BaseDistributor) *TickerDistributor {
	return &TickerDistributor{base: base}
}

// GetDataType 返回支持的数据类型
func (d *TickerDistributor) GetDataType() string {
...
func (d *TickerDistributor) GetTargetObjects(ctx context.Context, rule *dto.TaskRuleDTO) ([]string, error) {
...
func (d *TickerDistributor) GetMatchingNodes(ctx context.Context, rule *dto.TaskRuleDTO) ([]*cloudnodemodel.CloudNode, error) {
