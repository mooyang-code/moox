...
func (s *Server) jwtAuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		log.Infof("[FileServer] 文件访问请求: IP=%s, UserAgent=%s, Path=%s", clientIP, userAgent, c.Request.URL.Path)

		// 获取token参数
		token := c.Query("token")
		if token == "" {
			log.Warnf("[FileServer] 访问文件时缺少token: %s (IP: %s)", c.Request.URL.Path, clientIP)
		...
		if !s.isValidFilePath(filepath) {
			log.Warnf("[FileServer] 检测到可疑的文件路径: %s (IP: %s)", filepath, clientIP)
			s.respondWithError(c, http.StatusBadRequest, "非法文件路径", "ILLEGAL_PATH")
			return
		}

		// 验证JWT令牌（此时filepath已经是相对路径，与token中的路径格式一致）
		claims, err := authutils.ValidateFileDownloadToken(token, filepath)
		if err != nil {
			log.Warnf("[FileServer] JWT验证失败: %v (IP: %s, Path: %s)", err, clientIP, filepath)
		...
		log.Infof("[FileServer] JWT验证成功，用户: %s, 文件: %s (IP: %s)", claims.UserID, claims.FilePath, clientIP)
		c.Next()
	}
}
