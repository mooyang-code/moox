...
func (s *AuthServiceImpl) ChangePassword(ctx context.Context, req *pb.ChangePasswordReq) (*pb.ChangePasswordRsp, error) {
	...
	// 验证旧密码
	if !utils.ValidateEncryptedPassword(user.PasswordHash, user.Salt, req.Salt, req.Timestamp, req.OldPasswordHash) {
		return &pb.ChangePasswordRsp{
			RetInfo: &pb.RetInfo{
				Code: pb.EnumMooxErrorCode_NO_AUTH,
				Msg:  "旧密码错误",
	...
	// 解密新密码
	newPassword, err := utils.DecryptPassword(req.NewPasswordHash, req.Salt, req.Timestamp)
	if err != nil {
	...
	newPasswordHash := utils.HashPassword(newPassword, newSalt)
