...
func TestDatasetRowsUpsertedRoundTrip(t *testing.T) {
	in := &DatasetRowsUpserted{
		...
		Rows: []*RowUpsert{
			{Key: &RowKey{SpaceId: "crypto", DatasetId: "spot_kline", Kind: &RowKey_TimeSeries{TimeSeries: &TimeSeriesRowKey{SubjectId: "BTC-USDT", Freq: "1m", DataTime: "2026-07-23T00:00:00Z"}}}, Fields: []*FieldValue{{FieldId: "close", Value: &TypedValue{Value: &TypedValue_DoubleValue{DoubleValue: 101.25}}}}, Attributes: map[string]*TypedValue{"source": {Value: &TypedValue_StringValue{StringValue: "binance"}}}},
