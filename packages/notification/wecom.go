...
func NewWeComSenderWithTimeout(webhookURL string, timeout time.Duration) (*WeComSender, error) {
	if timeout <= 0 {
		return nil, errors.New("notification: WeCom timeout must be positive")
	}
...
func newWeComSender(webhookURL string, options ...weComOption) (*WeComSender, error) {
	...
	for _, option := range options {
		if option == nil {
			return nil, errors.New("notification: WeCom option must not be nil")
		}
...
func (s *WeComSender) Send(ctx context.Context, message Message) error {
	if s == nil {
		return errors.New("notification: WeCom sender is nil")
	}
	...
	if err != nil {
		return errors.New("notification: failed to encode WeCom request")
	}
	...
	if err != nil {
		return errors.New("notification: failed to create WeCom request")
	}
	...
	if err != nil {
		if contextErr := requestContext.Err(); contextErr != nil {
			return fmt.Errorf("notification: WeCom request failed: %w", contextErr)
		}
		return errors.New("notification: WeCom request failed")
	}
	defer response.Body.Close()

	responseBody, err := io.ReadAll(io.LimitReader(response.Body, maxResponseBytes+1))
	if err != nil {
		return errors.New("notification: failed to read WeCom response")
	}
	if len(responseBody) > maxResponseBytes {
		return fmt.Errorf("notification: WeCom response body exceeds %d byte limit", maxResponseBytes)
	}
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices {
		return fmt.Errorf("notification: WeCom returned HTTP %d", response.StatusCode)
	}
	...
	if err := json.Unmarshal(responseBody, &result); err != nil {
		return errors.New("notification: invalid WeCom response")
	}
	if result.ErrorCode != 0 {
		return fmt.Errorf("notification: WeCom returned errcode %d", result.ErrorCode)
	}
	return nil
}
...
func validateWebhookURL(rawURL string, allowHTTP bool) error {
	...
	if err != nil || parsed.Host == "" || parsed.User != nil || parsed.Fragment != "" {
		return errors.New("notification: invalid WeCom webhook URL")
	}
	if parsed.Scheme != "https" && !(allowHTTP && parsed.Scheme == "http") {
		return errors.New("notification: WeCom webhook URL must use HTTPS")
	}
	if !allowHTTP && !allowedWebhookHost(ChannelTypeWeCom, parsed.Hostname()) {
		return errors.New("notification: webhook host is not approved for WeCom")
	}
	return nil
}
