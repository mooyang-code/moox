...
func TestInstallServiceNameAddsStableField(t *testing.T) {
	...
	log.WithContextFields(ctx, "request_id", "req-2")
	log.InfoContext(ctx, "context request")

	require.Len(t, *base.entries, 3)
	...
	require.Equal(t, "factor", (*base.entries)[2][ServiceNameField])
	require.Equal(t, "req-2", (*base.entries)[2]["request_id"])
}
