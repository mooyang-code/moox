...
moox 当前包含 `cli`、`server`、`web-host` 等独立 Go module，同时目标系统还需要吸收 `xData-mini`、`data-collector`、`factor-calculator`、`order-center` 和 `account-center`。这些项目依赖差异较大，其中 `xData-mini/storage` 包含 RocksDB、DuckDB 等 CGO 依赖，直接合成单一 `go.mod` 会显著增加第一阶段迁移风险。
...
- 建立根 `build/`，集中管理测试、构建、发布和技能打包脚本。

**Non-Goals:**

- 第一阶段不强行合并为单 `go.mod`。
- 第一阶段不重写 PB 协议语义。
- 第一阶段不改变 RocksDB、DuckDB、Bleve、CSV 等存储组件职责。
...
业务代码统一放在 `modules/` 下，根目录保留跨模块资产，例如 `docs/`、`openspec/`、`skills/`、`build/`、`configs/`、`schema/` 和 `deployments/`。
...
`skills/moox` 是面向 Agent 的能力入口，`build/` 是面向工程构建和发布的脚本入口。二者放在根目录可以服务整个大仓，而不是绑定某个模块。
...
- 技能包和 CLI 发布产物耦合 -> `build/package-skill.sh` 必须显式打包 `skills/moox` 和全平台 `moox-cli`，避免隐式依赖构建副产物。

## Migration Plan

1. 新增根骨架：`go.work`、`modules/`、`build/`、`skills/moox`、`configs/`、`deployments/`、`var/`。
...
9. 建立根 `Makefile` 和 `build/` 脚本，统一测试、构建、发布和技能打包入口。
