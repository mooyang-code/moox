...
# moox-web-host 发布脚本 - 支持多平台部署（包括本地）
...
if [ "$IS_LOCAL" = true ]; then
    ...
    # 复制文件到临时目录（保持目录名为 web-host）
    cp -r "$PLATFORM" "$TEMP_DIR/web-host"
    
    # 切换到临时目录
    cd "$TEMP_DIR" || exit 1
    
    # 压缩目录（排除data文件夹）
    print_info "开始压缩 web-host 目录（排除 data 文件夹）..."
    if zip -r "$ZIP_FILE" "web-host" -x "web-host/data/*" > /dev/null 2>&1; then
...
if [ "$IS_LOCAL" = true ]; then
    ...
    # 创建目标目录
    mkdir -p ~/moox/web-host ~/moox/data
    
    # 备份旧版本（如果存在）
    if [ -d ~/moox/web-host ]; then
        print_info "备份旧版本..."
        rm -rf ~/moox/web-host.bak
        mv ~/moox/web-host ~/moox/web-host.bak
    fi
    
    # 复制新版本
    print_info "复制新版本到 ~/moox/web-host..."
    cp -r "$RELEASE_DIR" ~/moox/web-host
    
    # 处理后续部署步骤
    cd ~/moox/web-host || exit 1
    ...
    mkdir -p ~/bin
    ln -sf ~/moox/web-host/bin/$PROJECT_NAME ~/bin/$PROJECT_NAME
    ...
    print_info "工具目录: ~/moox/web-host"
    print_info "可执行文件: ~/bin/$PROJECT_NAME"
    print_info "共享数据目录: ~/moox/data"
    print_info ""
    print_info "使用方式: $PROJECT_NAME --help"
    print_info "启动服务: cd ~/moox/web-host && ./start.sh"
    print_info "停止服务: cd ~/moox/web-host && ./stop.sh"
    
else
    # 远程部署
    # 创建远程目录
    print_info "创建远程目录结构..."
    ssh "$REMOTE_SERVER" "mkdir -p ~/moox/web-host ~/moox/data"
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        print_info ""
        print_info "使用方式:"
        print_info "  ssh $REMOTE_SERVER '$PROJECT_NAME --help'"
        print_info "  ssh $REMOTE_SERVER 'cd ~/moox/web-host && ./start.sh'"
        print_info "  ssh $REMOTE_SERVER 'cd ~/moox/web-host && ./stop.sh'"
    else
