...
import (
	...
	"github.com/mooyang-code/moox/modules/control/internal/service/asynctask/dao"
...
func (s *AsyncTaskServiceImpl) AsyncJobCreate(ctx context.Context, tasks []TaskRequest) (string, error) {
	if len(tasks) == 0 {
	...
	for i, taskReq := range tasks {
		...
		taskModels[i] = &model.AsyncJobTask{
			...
			RequestParams: taskReq.RequestParams,
		}
	}
...
func (s *AsyncTaskServiceImpl) AsyncJobQuery(ctx context.Context, jobID string) (*JobQueryResult, error) {
	...
	// 3. 构建返回结果
	result := BuildJobQueryResult(job, tasks)
