...
func sanitizeTaskRequests(ctx context.Context, tasks []TaskRequest) ([]TaskRequest, error) {
	sanitized := make([]TaskRequest, len(tasks))
...
func sanitizeTaskRequest(ctx context.Context, task TaskRequest) (TaskRequest, error) {
	if task.TaskType != TaskTypeUploadFileToCOS {
		return task, nil
	}
	if strings.TrimSpace(task.RequestParams) == "" {
		return task, nil
	}

	var params map[string]interface{}
	if err := json.Unmarshal([]byte(task.RequestParams), &params); err != nil {
		return task, fmt.Errorf("invalid request_params for %s: %w", task.TaskType, err)
	}
	...
	if err != nil {
		return task, fmt.Errorf("invalid file_content for %s: %w", task.TaskType, err)
	}
