...
func TestRunKeepaliveProbeMarksSuccessfulSCFEventNodeOnline(t *testing.T) {
	...
	resp, err := service.GetNodeList(ctx, &NodeListRequest{Page: 1, PageSize: 20})
	requireNoError(t, err)
	if len(resp.Items) != 1 {
		t.Fatalf("expected one node, got %d", len(resp.Items))
	}
	if resp.Items[0].LastHeartbeat == nil {
		t.Fatalf("expected node list item to include last heartbeat")
	}
	if resp.Items[0].Status == nil || *resp.Items[0].Status != types.NodeStatusOnline {
		t.Fatalf("expected node status online, got %#v", resp.Items[0].Status)
	}
}
