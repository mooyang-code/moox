...
func TestRevealSecretUsesSignedServiceRoute(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.Equal(t, "/api/service/secret/RevealSecret", r.URL.Path)
	...
	secret, err := client.RevealSecret(context.Background(), "secret-1")
