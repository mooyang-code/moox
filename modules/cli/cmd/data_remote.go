...
func ensureRemoteDataset(ctx context.Context, storageURL string, options remoteImportOptions, rows []*pb.TimeSeriesRow) error {
	...
	for _, call := range calls {
		if err := postStorage(ctx, storageURL, "trpc.moox.storage.Metadata", call.method, call.req, call.rsp); err != nil {
			return err
		}
	}
	for columnName, valueType := range inferColumnTypes(rows) {
		if err := postStorage(ctx, storageURL, "trpc.moox.storage.Metadata", "UpsertDatasetColumn", &pb.UpsertDatasetColumnReq{Column: &pb.DatasetColumn{
