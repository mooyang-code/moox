...
func TestEventBusToMonitorHistoryFlow(t *testing.T) {
	...
	message := &messagepb.MooxMessage{ProtocolVersion: 1, MessageId: "monitor-metric-e2e-1", Topic: MetricTopic, Kind: messagepb.MessageKind_MESSAGE_KIND_SNAPSHOT, Producer: &messagepb.Producer{ServiceName: "fixture-service", InstanceId: "fixture-1", BootId: "boot-1", NodeId: "node-1", Version: "test"}, SpaceId: InternalMetricSpaceID, OccurredAt: now, PublishedAt: now, ContentType: MetricContentType, Payload: payload}
...
func fetchMetricsEventually(ctx context.Context, consumer *Consumer, timeout time.Duration) ([]*jetstream.Delivery, error) {
	deadline := time.NewTimer(timeout)
	defer deadline.Stop()
	for {
		fetchCtx, cancel := context.WithTimeout(ctx, time.Second)
		items, err := consumer.pull.Fetch(fetchCtx, 1)
		cancel()
		if len(items) > 0 {
			return items, err
		}
