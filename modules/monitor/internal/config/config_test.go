...
func TestMonitorConfigTRPCPort(t *testing.T) {
	...
	if cfg.Server.Service[1].Name != "trpc.moox.monitor.metrics.timer" || cfg.Server.Service[1].Port != 11415 {
		t.Fatalf("metrics timer service = %+v", cfg.Server.Service[1])
	}
}
