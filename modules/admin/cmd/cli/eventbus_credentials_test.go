...
func TestEventBusCredentialsExportAndRotate(t *testing.T) {
	...
	assert.Contains(t, yaml, "moox.strategy.output.accepted.v1.>")
	...
	assert.Contains(t, storageACL, "$JS.API.CONSUMER.INFO.MOOX_MARKET.storage_primary_kline_v1")
	assert.Contains(t, storageACL, "$JS.API.CONSUMER.MSG.NEXT.MOOX_MARKET.storage_primary_kline_v1")
	assert.Contains(t, storageACL, "$JS.ACK.MOOX_MARKET.storage_primary_kline_v1.>")
