...
func usersYAML(tokens map[string]string) string { // ACLs are deliberately subject-scoped; publisher roles never receive broad JetStream API access.
	return fmt.Sprintf("users:\n"+
		...
		"  - username: storage-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.storage.dataset.rows.upserted.v2.>\", \"$JS.API.STREAM.NAMES\", \"$JS.API.CONSUMER.INFO.*.storage_view\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.storage_view\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.storage_view.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.storage_view\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.storage_view\", \"$JS.ACK.MOOX_STORAGE.storage_view.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		...
		"  - username: collector-market-fetch-consumer\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.STREAM.NAMES\", \"$JS.API.CONSUMER.INFO.*.*\", \"$JS.API.CONSUMER.CREATE.MOOX_MARKET_FETCH.*\", \"$JS.API.CONSUMER.CREATE.MOOX_MARKET_FETCH.*.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_MARKET_FETCH.*\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_MARKET_FETCH.*\", \"$JS.ACK.MOOX_MARKET_FETCH.*.>\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.*\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.*.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.*\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.*\", \"$JS.ACK.MOOX_STORAGE.*.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
