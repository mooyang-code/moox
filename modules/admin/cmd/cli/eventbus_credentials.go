...
func usersYAML(tokens map[string]string) string { // ACLs are deliberately subject-scoped; publisher roles never receive broad JetStream API access.
	return fmt.Sprintf("users:\n"+
		...
		"  - username: monitor-hostmetrics-consumer\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.CONSUMER.INFO.MOOX_METRICS.monitor_hostmetrics_ingest_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_METRICS.monitor_hostmetrics_ingest_v1\", \"$JS.ACK.MOOX_METRICS.monitor_hostmetrics_ingest_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: monitor-metrics-consumer\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.CONSUMER.INFO.MOOX_METRICS.monitor_metrics_ingest_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_METRICS.monitor_metrics_ingest_v1\", \"$JS.ACK.MOOX_METRICS.monitor_metrics_ingest_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: storage-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.storage.dataset.rows.upserted.v1.>\", \"$JS.API.CONSUMER.INFO.MOOX_STORAGE.storage_view\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.storage_view\", \"$JS.ACK.MOOX_STORAGE.storage_view.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
