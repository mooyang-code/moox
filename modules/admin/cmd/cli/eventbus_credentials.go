...
func usersYAML(tokens map[string]string) string { // ACLs are deliberately subject-scoped; publisher roles never receive broad JetStream API access.
	return fmt.Sprintf("users:\n"+
		...
		"  - username: monitor-metrics-consumer\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.dlq.message.rejected.v1.>\", \"$JS.API.CONSUMER.INFO.MOOX_METRICS.monitor_metrics_ingest_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_METRICS.monitor_metrics_ingest_v1\", \"$JS.ACK.MOOX_METRICS.monitor_metrics_ingest_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: storage-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.storage.dataset.rows.upserted.v1.>\", \"moox.dlq.message.rejected.v1.>\", \"$JS.API.CONSUMER.INFO.MOOX_MARKET.storage_primary_kline_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_MARKET.storage_primary_kline_v1\", \"$JS.ACK.MOOX_MARKET.storage_primary_kline_v1.>\", \"$JS.API.CONSUMER.INFO.MOOX_STORAGE.storage_view\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.storage_view\", \"$JS.ACK.MOOX_STORAGE.storage_view.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: archive-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.CONSUMER.INFO.MOOX_STORAGE.moox_archive_kline_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.moox_archive_kline_v1\", \"$JS.ACK.MOOX_STORAGE.moox_archive_kline_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: cloudnode-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.cloudnode.job.execution.requested.v1.>\", \"$JS.API.CONSUMER.INFO.MOOX_CLOUDNODE_EXEC.>\", \"$JS.API.CONSUMER.CREATE.MOOX_CLOUDNODE_EXEC.>\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_CLOUDNODE_EXEC.>\", \"$JS.ACK.MOOX_CLOUDNODE_EXEC.>\", \"$JS.API.STREAM.INFO.KV_MOOX_CLOUDNODE_JOB_ACTIVE\", \"$JS.API.STREAM.MSG.GET.KV_MOOX_CLOUDNODE_JOB_ACTIVE\", \"$JS.API.CONSUMER.CREATE.KV_MOOX_CLOUDNODE_JOB_ACTIVE.>\", \"$JS.API.CONSUMER.DELETE.KV_MOOX_CLOUDNODE_JOB_ACTIVE.>\", \"$KV.MOOX_CLOUDNODE_JOB_ACTIVE.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: factor-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.CONSUMER.INFO.MOOX_STORAGE.factor_calc\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.factor_calc\", \"$JS.ACK.MOOX_STORAGE.factor_calc.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: strategy-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.strategy.output.accepted.v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n",
