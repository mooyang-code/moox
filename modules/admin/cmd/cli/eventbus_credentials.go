...
func ensureToken(ctx context.Context, d *dao.SecretDAO, key, role string) (string, error) {
	...
	value := ""
	if key == eventBusKeys["merge-eventbus"] {
		if legacy, ok := records["eventbus_factor_merge"]; ok {
			value = strings.TrimSpace(legacy.SecretValue)
		}
	}
	if value == "" {
		raw := make([]byte, 32)
...
func eventBusRoleFiles() map[string]string {
	return map[string]string{
		...
		"merge-eventbus":                  "merge-eventbus.yaml",
...
func usersYAML(tokens map[string]string) string { // ACLs are deliberately subject-scoped; publisher roles never receive broad JetStream API access.
	return fmt.Sprintf("users:\n"+
		...
		"  - username: factor-eventbus\n    password: %s\n    permissions:\n      publish: {allow: []}\n      subscribe: {allow: [\"_INBOX.>\", \"moox.factor.internal.catalog.snapshot\", \"moox.factor.internal.recalc.>\"]}\n      responses: {max_messages: 1, expires: 10s}\n"+
		"  - username: factor-engine-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.factor.internal.catalog.snapshot\", \"moox.factor.internal.recalc.>\", \"moox.event.observability.metrics.snapshot.reported.v1.>\", \"$JS.API.STREAM.NAMES\", \"$JS.API.CONSUMER.INFO.*.factor_view_ready_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_view_ready_v1.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.ACK.MOOX_STORAGE.factor_view_ready_v1.>\", \"$JS.API.CONSUMER.INFO.*.factor_source_subject\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_source_subject\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_source_subject.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.factor_source_subject\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.factor_source_subject\", \"$JS.ACK.MOOX_STORAGE.factor_source_subject.>\", \"$JS.API.CONSUMER.INFO.*.factor_dataset_rows_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_dataset_rows_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_dataset_rows_v1.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.factor_dataset_rows_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.factor_dataset_rows_v1\", \"$JS.ACK.MOOX_STORAGE.factor_dataset_rows_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
		"  - username: merge-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"$JS.API.STREAM.NAMES\", \"$JS.API.CONSUMER.INFO.*.merge_rows_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.merge_rows_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.merge_rows_v1.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.merge_rows_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.merge_rows_v1\", \"$JS.ACK.MOOX_STORAGE.merge_rows_v1.>\", \"$JS.API.CONSUMER.INFO.*.merge_collector_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.merge_collector_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.merge_collector_v1.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.merge_collector_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.merge_collector_v1\", \"$JS.ACK.MOOX_STORAGE.merge_collector_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
