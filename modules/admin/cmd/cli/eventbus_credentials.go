...
func usersYAML(tokens map[string]string) string { // ACLs are deliberately subject-scoped; publisher roles never receive broad JetStream API access.
	return fmt.Sprintf("users:\n"+
		...
		"  - username: factor-engine-eventbus\n    password: %s\n    permissions:\n      publish: {allow: [\"moox.factor.internal.catalog.snapshot\", \"$JS.API.STREAM.NAMES\", \"$JS.API.CONSUMER.INFO.*.factor_view_ready_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.API.CONSUMER.CREATE.MOOX_STORAGE.factor_view_ready_v1.>\", \"$JS.API.CONSUMER.DURABLE.CREATE.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.API.CONSUMER.MSG.NEXT.MOOX_STORAGE.factor_view_ready_v1\", \"$JS.ACK.MOOX_STORAGE.factor_view_ready_v1.>\"]}\n      subscribe: {allow: [\"_INBOX.>\"]}\n"+
