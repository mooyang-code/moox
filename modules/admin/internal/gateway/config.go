...
// GetServiceDetail 根据服务ID获取服务详情（address+path），供网关纯透传 forwardHTTP 使用。
func (c *Config) GetServiceDetail(serviceID string) (ServiceDetail, error) {
	serviceDetail, exists := c.Gateway.Services[serviceID]
	if !exists {
		return ServiceDetail{}, fmt.Errorf("服务 '%s' 未在配置文件中找到", serviceID)
	}
