...
func InitGatewayServices(s *server.Server, provider GatewayProvider, adminNodeID string, authorizers ...TradeSpaceAuthorizer) error {
	cfg := GetConfig()
	if cfg == nil {
	...
	if err := RegisterGatewayHTTPHandlers(s, provider, adminNodeID, authorizers...); err != nil {
