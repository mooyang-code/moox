...
// dtoToPackageInfo 将 pb.PackageDetail 转换为 packageInfo
func dtoToPackageInfo(dto *pb.PackageDetail) *packageInfo {
	if dto == nil {
		return nil
	}
	return &packageInfo{
		...
		Status:           int(dto.GetStatus()),
		COSBucket:        dto.GetCosBucket(),
		CloudAccountID:   dto.GetCloudAccountId(),
		COSPath:          dto.GetCosPath(),
		COSRegion:        dto.GetCosRegion(),
