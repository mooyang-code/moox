...
type TaskPlanner interface {
	...
	GetTargetObjects(ctx context.Context, rule *model.CollectorTaskRules) ([]string, error)

	// BuildTaskParams 为指定对象构建任务参数
	// object 为空字符串时表示不按对象拆分
	BuildTaskParams(ctx context.Context, rule *model.CollectorTaskRules, object string) (string, error)

	// GetMatchingNodes 根据规划策略获取匹配的节点列表
	GetMatchingNodes(ctx context.Context, rule *model.CollectorTaskRules) ([]*cloudnodemodel.CloudNode, error)
}
