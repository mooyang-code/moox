...
func DefaultDeployments() []Deployment {
	rows := []Deployment{
		...
		withExtra(deployment("storage_view", "storage", "http", defaultPublicHost, 20202, "trpc.moox.storage.DataView", "public", "moox-storage 数据视图 HTTP 服务"), `{"health_url":"http://127.0.0.1:20210/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view_builder", "storage", "http", "127.0.0.1", 20211, "", "internal", "moox-storage ViewBuilder 独立物化进程健康检查"), `{"health_url":"http://127.0.0.1:20211/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view_query", "storage", "http", "127.0.0.1", 20202, "trpc.moox.storage.DataView", "internal", "moox-storage DataView 独立查询进程"), `{"health_url":"http://127.0.0.1:20212/healthz","monitor_enabled":true}`),
