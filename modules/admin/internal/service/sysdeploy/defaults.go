...
func DefaultDeployments() []Deployment {
	rows := []Deployment{
		...
		withExtra(deployment("storage_metadata", "storage", "http", defaultPublicHost, 20200, "trpc.moox.storage.Metadata", "public", "moox-storage 元数据 HTTP 服务"), `{"health_url":"http://127.0.0.1:20210/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_access", "storage", "http", defaultPublicHost, 20201, "trpc.moox.storage.Access", "public", "moox-storage 数据写入/读取 HTTP 服务，SCF 采集写入优先直连"), `{"health_url":"http://127.0.0.1:20210/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view", "storage", "http", defaultPublicHost, 20202, "trpc.moox.storage.DataView", "public", "moox-storage 数据视图 HTTP 服务"), `{"health_url":"http://127.0.0.1:20212/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view_builder", "storage", "http", "127.0.0.1", 20211, "", "internal", "moox-storage View 独立进程中的物化职责健康检查"), `{"health_url":"http://127.0.0.1:20211/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view_query", "storage", "http", "127.0.0.1", 20202, "trpc.moox.storage.DataView", "internal", "moox-storage View 独立进程中的查询职责"), `{"health_url":"http://127.0.0.1:20212/healthz","monitor_enabled":true}`),
		withExtra(deployment("storage_view_index", "storage_rpc", "trpc", "127.0.0.1", 20104, "trpc.moox.storage.ViewIndex", "internal", "moox-storage View 索引文件唯一 owner"), `{"health_url":"http://127.0.0.1:20213/healthz","monitor_enabled":true}`),
		...
		withExtra(deployment("moox_strategy", "strategy", "http", "127.0.0.1", 11430, "trpc.moox.strategy.StrategyMgr", "internal", "交易策略运行、目标和绩效查询服务"), `{"health_url":"http://127.0.0.1:11431/healthz","monitor_enabled":true}`),
		withExtra(deployment("moox_archive", "archive", "http", "127.0.0.1", 11416, "", "internal", "事件归档和物化服务"), `{"health_url":"http://127.0.0.1:11416/healthz","monitor_enabled":true}`),
		withExtra(deployment("moox_hostagent", "hostagent", "http", "127.0.0.1", 11426, "trpc.moox.hostagent.HostAgentMgr", "internal", "主机指标采集代理"), `{"health_url":"http://127.0.0.1:11425/healthz","monitor_enabled":true}`),
		withExtra(deployment("moox_trade", "trade", "http", "127.0.0.1", 11210, "", "internal", "交易执行和账本服务"), `{"health_url":"http://127.0.0.1:11210/healthz","monitor_enabled":true}`),
