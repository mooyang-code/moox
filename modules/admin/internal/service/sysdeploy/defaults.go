...
func DefaultDeployments(nodeID string) []Deployment {
	rows := []Deployment{
		...
		withExtra(deployment("moox_factor", "factor", "http", "127.0.0.1", 11404, "trpc.moox.factor.FactorMgr", "internal", "因子计算服务，承载因子定义、绑定、补算与结果写回"), `{"health_url":"http://127.0.0.1:11414/readyz","health_kind":"readiness","monitor_enabled":true,"gateway_methods":["CreateFactor","UpdateFactor","GetFactor","ListFactors","SetFactorStatus","UpsertBinding","ListBindings","DeleteBinding","RecalcFactor","GetEngineStatus"],"gateway_callers":["admin-gateway","moox-cli"]}`),
