...
// 供统一 HTTP 转发层（dispatcher）使用，不依赖任何 HTTP 框架。
func InjectFromHeaders(ctx context.Context, headers map[string]string) context.Context {
	if headers == nil {
		return ctx
	}
	return WithSpaceID(ctx, headers["space_id"])
}
