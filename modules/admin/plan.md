     ...
     // 通过 Control API 代理的 REST API（端口 11000）
     POST   /api/admin/ssh/ListHosts           → 主机列表（分页）
     POST   /api/admin/ssh/CreateHost          → 创建主机
     POST   /api/admin/ssh/UpdateHost          → 更新主机
     POST   /api/admin/ssh/DeleteHost          → 删除主机
     POST   /api/admin/ssh/CreateSession       → 创建 SSH 会话（返回
     session_id）
     POST   /api/admin/ssh/DisconnectSession   → 断开 SSH 会话
     POST   /api/admin/ssh/ResizeWindow        → 调整终端大小
     POST   /api/admin/ssh/ExecCommand         → 执行命令
     POST   /api/admin/ssh/SftpList            → SFTP 目录列表
     POST   /api/admin/ssh/SftpMkdir           → SFTP 创建目录
     POST   /api/admin/ssh/SftpDelete          → SFTP 删除
     POST   /api/admin/ssh/GetOnlineSessions   → 在线会话列表
     POST   /api/admin/ssh/ForceDisconnect     → 强制断开
