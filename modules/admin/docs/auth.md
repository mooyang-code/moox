...
| 组件 | 职责 | 说明 |
|------|------|------|
| **Service** | 服务接口 | 实现 protobuf 定义的 AuthServiceService |
...
type Service interface {
    pb.AuthServiceService  // 实现 protobuf 生成的接口
    // 可以添加其他自定义接口
}
...
### 3.2 AuthServiceService 接口（protobuf 生成）
...
// 2. utils.ParseAccessToken(token, secretKey) 校验并解析 claims
...
下游 RPC 服务可通过 `utils.GetUserIDFromContext(ctx)` 获取当前用户 ID。
