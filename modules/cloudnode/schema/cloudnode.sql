...
CREATE TRIGGER IF NOT EXISTS update_cloud_accounts_mtime
AFTER UPDATE ON t_cloud_accounts
BEGIN
    UPDATE t_cloud_accounts SET c_mtime = CURRENT_TIMESTAMP WHERE rowid = NEW.rowid;
END;

CREATE TRIGGER IF NOT EXISTS update_cloud_packages_mtime
AFTER UPDATE ON t_cloud_function_packages
BEGIN
    UPDATE t_cloud_function_packages SET c_mtime = CURRENT_TIMESTAMP WHERE rowid = NEW.rowid;
END;
