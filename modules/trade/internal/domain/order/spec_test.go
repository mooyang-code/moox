	...
	tests := []struct {
	...
	}{
		{
			...
			mutate: func(spec *OrderSpec) {
				spec.OrderType = exchange.OrderTypeMarket
			},
		},
		{
			name:   "SPOT LIMIT GTC",
			market: exchange.MarketTypeSpot,
			mutate: func(spec *OrderSpec) {
				spec.OrderType = exchange.OrderTypeLimit
		...
		},
		{
			...
			mutate: func(spec *OrderSpec) {
				spec.OrderType = exchange.OrderTypeLimit
				spec.TimeInForce = exchange.TimeInForceIOC
				spec.LimitPrice = &price
			},
		},
		{
			...
			mutate: func(spec *OrderSpec) {
				spec.OrderType = exchange.OrderTypeLimit
				spec.TimeInForce = exchange.TimeInForceFOK
				spec.LimitPrice = &price
			},
		},
		{
...
func TestOrderSpecRejectsInvalidMatrixCombinations(t *testing.T) {
	...
	tests := []struct {
		...
		{"SPOT reduce-only", exchange.MarketTypeSpot, func(spec *OrderSpec) { spec.ReduceOnly = true }},
		{"MARKET limit price", exchange.MarketTypeSpot, func(spec *OrderSpec) {
			spec.OrderType = exchange.OrderTypeMarket
			spec.TimeInForce = exchange.TimeInForceUnspecified
		}},
		...
		{"LIMIT missing time in force", exchange.MarketTypeSpot, func(spec *OrderSpec) { spec.TimeInForce = exchange.TimeInForceUnspecified }},
		{"LIMIT unsupported time in force", exchange.MarketTypeSpot, func(spec *OrderSpec) { spec.TimeInForce = exchange.TimeInForce("POST_ONLY") }},
		...
		{"unsupported order type", exchange.MarketTypeSpot, func(spec *OrderSpec) { spec.OrderType = exchange.OrderType("STOP") }},
	}
