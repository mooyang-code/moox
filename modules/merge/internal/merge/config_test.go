...
func TestMergeProcessConfigHonorsRuntimeEnv(t *testing.T) {
	...
	require.True(t, ok)
	t.Setenv("MOOX_MERGE_DB_PATH", "/tmp/merge-runtime.db")
	t.Setenv("MOOX_MERGE_STORAGE_RPC_GATEWAY_TARGET", "ip://203.0.113.20:11003")
	t.Setenv("MOOX_MERGE_STORAGE_RPC_GATEWAY_NODE_ID", "storage")
	t.Setenv("MOOX_MERGE_STORAGE_RPC_KEY_ID", "merge")
	t.Setenv("MOOX_MERGE_STORAGE_RPC_HMAC_KEY_FILE", "/tmp/gateway-merge.key")
	t.Setenv("MOOX_MERGE_EVENTBUS_CREDENTIAL_FILE", "/tmp/merge-eventbus.yaml")
	...
	require.Equal(t, "/tmp/merge-eventbus.yaml", cfg.EventBus.CredentialFile)
...
func TestMergeAssemblerLoadsExampleSpotSwapDefinition(t *testing.T) {
	...
	cfg, err := LoadProcessConfig(filepath.Join(filepath.Dir(file), "..", "..", "config", "merge-app.yaml"))
	require.NoError(t, err)
	require.Equal(t, "merge-1", cfg.MergeID)
