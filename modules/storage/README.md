...
- **统一写入与读取**：所有数据访问都经由 Access 入口；时序数据使用 `TimeSeriesKey + TimeRange`，对象数据使用 `ObjectKey + VersionRange`，写入按事实键做列级更新。
- **多形态数据**：时序（K 线/tick/快照）、对象（公司/交易对资料）、事件、文档、通用表格，以及参数化因子结果。
- **全文检索**：把列标记为 `text_indexed` 即可同步到全文索引，支持 `text_query` 与结构化过滤。
- **物化视图**：登记 View 后由后台异步物化，`QueryView` 直接读取物化结果。
...
storage:
  ...
  eventbus:
    ...
    rows_changed_subject: moox.storage.fact.rows_changed.v1   # 行变更事件主题
```
...
| DataService | 18201 | 19104 | 事实数据读写 |
| QueryService | 18202 | 19105 | 视图/检索查询 |
...
| **DataSetSubject** | DataSet 与 Subject 的绑定，写入时据此做归属校验 | DataSet + Subject |
| **Field** | Space 级字段字典（`open`/`close`/`symbol`…），声明值类型、单位、示例 | Space |
| **Factor** | 参数化因子定义（算法 + 参数），结果可作为列来源 | Space |
| **DataSetColumn** | DataSet 的列契约，`origin_type` 指向 Field/Factor 等；`required`/`text_indexed` 等约束 | DataSet |
| **View** | 查询入口，必须指定 `primary_dataset_id`，按 `grain_keys` 异步物化 | Space |
| **ViewColumn** | View 对外暴露的列 | View |
| **StorageNode** | 在线事实主存节点（Pebble），`endpoint` 决定访问地址（`local`=同进程） | — |
| **Device** | 节点上的设备，`engine` ∈ `pebble`/`duckdb`/`bleve`/`parquet_archive`；路由只用 `pebble` 设备 | StorageNode |
| **StorageRoute** | 把 `(space, dataset[, subject/pattern])` 路由到 StorageNode，`hash_rule` 决定分片 | Space + DataSet + StorageNode |

> 命名注记：`StorageRoute` / `StorageNode` 为历史命名，语义已收窄为"仅路由 Pebble 在线主存切分"，不路由 DuckDB/Bleve/Parquet 派生设备。

枚举取值（在 seed 文件里用短名书写）：

- `data_kind`：`time_series` / `object` / `snapshot` / `event` / `document` / `table`
- `value_type`：`string` / `int` / `double` / `bool` / `time` / `json` / `bytes`
- `origin_type`（列来源）：`field` / `factor` / `dataset_column` / `expression` / `system`
...
# 对象在来源侧的外部代码：AR-USDT 在 binance 叫 ARUSDT
subject_symbols:
  - space_id: crypto
    subject_id: AR-USDT
    data_source_id: binance
...
dataset_columns:
  - space_id: crypto
    ...
    value_type: double
    required: true
    # text_indexed: true            # 标 true 的列会进全文索引（用于 SearchRows）
    status: active
...
```text
metadata seed 导入完成 (config/metadata.seed.yaml): spaces=1 data_sources=1 subjects=2 ... storage_routes=2
```
...
需要改动的配置：保持默认即可（`config/storage.yaml` 中 `primary.service_name=""`、`eventbus.type=memory`，`metadata.seed.yaml` 中 StorageNode `endpoint: local`）。
...
2. **主存走远程**：Access 节点的 `storage.primary.service_name` 必须非空，并在元数据里把 StorageNode 的 `endpoint` 指向真实主存地址。
...
storage:
  primary:
    service_name: trpc.storage.primary.PrimaryStoreService   # 走远程主存
  eventbus:
    type: nats
    nats_url: nats://10.0.0.9:4222
```
...
> StorageNode `endpoint` 支持 `ip://host:port`、`host:port`（自动补 `ip://`）或 tRPC 服务名（走服务发现）；`local`/留空表示同进程。
...
4. **分布式额外检查**：Access 节点日志无"primary store"连接错误；主存节点 `PrimaryStoreService` 端口可被 Access 节点 `telnet`/`nc` 通；NATS 上能看到 `moox.storage.fact.rows_changed.v1` 主题有消息。
...
Space、View（+ViewColumn）、DataSource、Subject（+SubjectSymbol）、DataSet（+DataSetSubject 绑定）、Field、Factor、DataSetColumn、StorageNode、Device、StorageRoute、ArchiveFile。

> CLI `-import-metadata` 即是对这些接口的批量封装。

### DataService — 事实数据读写（端口 18201 / HTTP 19104）

| RPC | 说明 |
| --- | --- |
| `WriteRows` | 写入事实数据行，按 `DataKey` 列级更新 |
| `ReadRows` | 低阶事实数据读取入口；新接入方优先使用时序/对象专用接口 |
| `WriteTimeSeriesRows` / `ReadTimeSeriesRows` | 写入/读取固定 `subject_id + freq` 下按 `data_time` 演进的时序数据 |
| `WriteObjectRows` / `ReadObjectRows` | 写入/读取对象数据，按 `object_id + version` 定位 |

### QueryService — 用户侧查询（端口 18202 / HTTP 19105）

| RPC | 说明 |
| --- | --- |
| `QueryView` | 查询已物化的视图；不存在返回 `VIEW_NOT_FOUND` |
| `SearchRows` | 数据集全文 + 结构化搜索 |
| `RebuildSearchIndex` | 异步重建全文索引，返回 `rebuild_id` |
