...
# xdata-storage 发布脚本 - 支持多平台部署（包括本地）
...
PROJECT_NAME="xdata-storage"
BINARY_NAME="xdata-storage"  # 实际的二进制文件名
REMOTE_SERVER=""
REMOTE_DIR=""
PLATFORM=""
IS_LOCAL=false
...
# 设置远程部署目录
REMOTE_DIR="~/xdata/storage"
REMOTE_DATA_DIR="~/xdata/data"
...
if [ "$IS_LOCAL" = true ]; then
    ...
    # 复制文件到临时目录（保持目录名为 storage）
    cp -r "$PLATFORM" "$TEMP_DIR/storage"
    
    # 切换到临时目录
    cd "$TEMP_DIR" || exit 1
    
    # 压缩目录（排除data文件夹）
    print_info "开始压缩 storage 目录（排除 data 文件夹）..."
    if zip -r "$ZIP_FILE" "storage" -x "storage/data/*" > /dev/null 2>&1; then
...
if [ "$IS_LOCAL" = true ]; then
    ...
    # 创建目标目录
    mkdir -p ~/xdata/storage ~/xdata/data
    
    # 备份旧版本（如果存在）
    if [ -d ~/xdata/storage ]; then
        print_info "备份旧版本..."
        rm -rf ~/xdata/storage.bak
        mv ~/xdata/storage ~/xdata/storage.bak
    fi
    
    # 复制新版本
    print_info "复制新版本到 ~/xdata/storage..."
    cp -r "$RELEASE_DIR" ~/xdata/storage
    
    # 处理后续部署步骤
    cd ~/xdata/storage || exit 1
    ...
    mkdir -p ~/bin
    ln -sf ~/xdata/storage/bin/$BINARY_NAME ~/bin/$PROJECT_NAME
    ...
    print_info "工具目录: ~/xdata/storage"
    print_info "可执行文件: ~/bin/$PROJECT_NAME"
    print_info "共享数据目录: ~/xdata/data"
    print_info ""
    ...
    # 创建远程目录
    print_info "创建远程目录结构..."
    ssh "$REMOTE_SERVER" "mkdir -p ~/xdata/storage ~/xdata/data"
