...
func TestResultTablePrefixDoesNotCollideForSanitizedSpaceIDs(t *testing.T) {
	...
	require.Contains(t, resultTableName("a-b", "view", time.Unix(1, 0).UTC()), hyphenPrefix)
	require.Contains(t, resultTableName("a_b", "view", time.Unix(1, 0).UTC()), underscorePrefix)
}
