...
func (s *Service) QueryTimeSeriesRows(ctx context.Context, req *pb.QueryTimeSeriesRowsReq) (*pb.QueryTimeSeriesRowsRsp, error) {
	...
	if err := s.validateTimeSeriesActiveSchema(ctx, viewMeta, req.GetColumnNames()); err != nil {
...
func (s *Service) SearchRecordRows(ctx context.Context, req *pb.SearchRecordRowsReq) (*pb.SearchRecordRowsRsp, error) {
	...
	if err := s.validateRecordActiveSchema(ctx, viewMeta, datasetID, req.GetColumnNames(), viewColumns); err != nil {
		if IsViewNotReadyError(err) {
...
func (s *Service) validateTimeSeriesActiveSchema(ctx context.Context, viewMeta *pb.View, requested []string) error {
	...
	activeNames := ViewColumnNames(viewMeta.GetColumns())
	if len(activeNames) == 0 {
		if s.views == nil {
			return errors.New("view store is required")
		}
		columns, _, _, err := s.views.QueryTimeSeriesRows(ctx, viewMeta.GetActiveResult(), &pb.QueryTimeSeriesRowsReq{
			Limit:     1,
		...
		activeNames = ResultColumnNames(columns)
	}
	return ValidateRequestedColumnsAgainstActiveSchema(viewMeta, requested, activeNames, latestColumns)
}

func (s *Service) validateRecordActiveSchema(ctx context.Context, viewMeta *pb.View, datasetID string, requested []string, latestColumns []*pb.ViewColumn) error {
	if !NeedsActiveSchemaValidation(viewMeta, requested) {
		return nil
	}
	activeNames := ViewColumnNames(viewMeta.GetColumns())
	if len(activeNames) == 0 {
		...
		activeNames = RecordRowsColumnNames(rows)
	}
	return ValidateRequestedColumnsAgainstActiveSchema(viewMeta, requested, activeNames, latestColumns)
}
...
func RecordRowsColumnNames(rows []*pb.RecordRow) []string {
