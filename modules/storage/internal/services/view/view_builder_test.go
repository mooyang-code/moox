...
func TestBuilderJoinsColumnsFromMultipleDatasets(t *testing.T) {
	...
	require.NoError(t, facts.WriteRows(ctx, []*pb.DataRow{{
		...
		Columns: []*pb.ColumnValue{testutil.DoubleValue("ma20", 7.9)},
...
func TestBuilderJoinsRowsWithAmbiguousDimensionTextSeparately(t *testing.T) {
	...
	require.NoError(t, facts.WriteRows(ctx, []*pb.DataRow{
		writeFact("kline", map[string]string{"a": "b&c=d"}, testutil.DoubleValue("close", 1)),
		writeFact("kline", map[string]string{"a": "b", "c": "d"}, testutil.DoubleValue("close", 2)),
		writeFact("factor", map[string]string{"a": "b&c=d"}, testutil.DoubleValue("ma20", 10)),
		writeFact("factor", map[string]string{"a": "b", "c": "d"}, testutil.DoubleValue("ma20", 20)),
	}, pb.WriteMode_WRITE_MODE_UPSERT))
...
func newBuilderFixture(t *testing.T, ctx context.Context) builderFixture {
	...
	return builderFixture{meta: meta, facts: facts, views: views, builder: builder, root: root}
}
