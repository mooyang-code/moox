	...
	require.NoError(t, store.InsertRows(ctx, "view_result_crypto_kline_filter", []*pb.QueryViewRow{
		{
			...
			Values: []*pb.ColumnValue{
				...
				quantstore.DoubleValue("volume", 100),
			},
		},
		{
			SubjectId: "AR-USDT",
			DataTime:  "2026-06-15T00:00:00+08:00",
			Values: []*pb.ColumnValue{
				quantstore.DoubleValue("close", 3.2),
				quantstore.DoubleValue("volume", 200),
			},
		},
		{
			SubjectId: "OP-USDT",
			DataTime:  "2026-06-15T00:00:00+08:00",
			Values: []*pb.ColumnValue{
				quantstore.DoubleValue("close", 10.5),
				quantstore.DoubleValue("volume", 50),
			},
		},
	}))
