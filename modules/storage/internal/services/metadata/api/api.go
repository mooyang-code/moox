// Package api 提供给前端web页面的api接口（兼容wuji协议，方便业务使用wuji缓存本api结果）
package api
...
func (a *APIHandle) Register(handler SchemaHandler) {
	...
	a.handlers[handler.SchemaID()] = handler
}

// RegisterStandardHTTPHandlers 注册标准http接口
func RegisterStandardHTTPHandlers(s *server.Server) {
	router := mux.NewRouter()
	router.HandleFunc("/metaapi/{schemaid}", func(w http.ResponseWriter, r *http.Request) {
	...
	thttp.RegisterNoProtocolServiceMux(s.Service("trpc.storage.metadata.stdhttp"), router)
}
...
func handleAPIRequest(w http.ResponseWriter, r *http.Request) error {
	...
	if !ok || schemaID == "" {
		return errs.New(400, "请求错误:未提供有效的schemaid")
	}

	// 根据schemaID获取对应的表处理器
	handler, ok := GetAPIHandleInstance().handlers[schemaID]
	if !ok {
		return errs.New(404, "未找到处理器: "+schemaID)
	}
