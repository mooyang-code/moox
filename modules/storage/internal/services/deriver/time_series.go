...
func (s *Service) processTimeSeriesBatch(ctx context.Context, keys []*pb.TimeSeriesKey) error {
	...
	if s == nil || s.reader == nil || s.metadataReader == nil || s.views == nil {
		return errors.New("deriver time-series processor requires reader, metadata reader and view writer")
	}
	...
	for key, datasetRows := range grouped {
		...
		for _, item := range views {
			...
			mapped, ok, err := TimeSeriesRowsForView(ctx, item, columns, datasetRows, s.readTimeSeriesProjectionRow)
			if err != nil {
				return err
			}
			if !ok {
