...
func (s *Service) processRecordBatch(ctx context.Context, keys []*pb.RecordKey) error {
	...
	if s == nil || s.reader == nil || s.metadataReader == nil || s.search == nil {
		return errors.New("deriver record processor requires reader, metadata reader and record indexer")
	}
	...
	for key, datasetRows := range grouped {
		...
		for _, item := range views {
			...
			projected, ok, err := RecordRowsForView(ctx, item, columns, datasetRows, s.readRecordProjectionRow)
			if err != nil {
				return err
			}
			if !ok {
