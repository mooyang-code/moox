...
func TestViewMetricsExposeAggregateRuntimeMetricsWithFixedLabels(t *testing.T) {
	...
	metrics.ObserveViewOutputWatermark("crypto", "view_crypto_spot_kline_1m_factor", "dataset_spot_kline_1m_factor", "1m", time.Date(2026, 8, 20, 14, 0, 0, 0, time.UTC))
	metrics.ObserveViewOutputWatermark("crypto", "view_crypto_spot_kline_1m_factor", "dataset_spot_kline_1m_factor", "1m", time.Date(2026, 8, 20, 13, 0, 0, 0, time.UTC))
	assert.Equal(t, float64(time.Date(2026, 8, 20, 14, 0, 0, 0, time.UTC).Unix()), testutil.ToFloat64(metrics.viewOutputWatermark.WithLabelValues("crypto", "view_crypto_spot_kline_1m_factor", "dataset_spot_kline_1m_factor", "1m")))
