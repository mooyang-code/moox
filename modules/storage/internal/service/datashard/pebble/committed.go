...
func (s *Store) WriteRowsWithCommittedMessage(ctx context.Context, rows []*pb.ShardRow) error {
	if len(rows) == 0 {
		return nil
	}
	...
	if rows[0].GetKey().GetDataKind() == pb.DataKind_DATA_KIND_TIME_SERIES {
		...
		for _, row := range rows {
			...
			event.Writes = append(event.Writes, &pb.TimeSeriesRowWrite{Operation: pb.RowWriteOperation_ROW_WRITE_OPERATION_MERGE, Row: public, SourceShardId: s.shardID})
		}
		...
		for _, row := range rows {
			...
			event.Writes = append(event.Writes, &pb.RecordRowWrite{Operation: pb.RowWriteOperation_ROW_WRITE_OPERATION_MERGE, Row: public, SourceShardId: s.shardID})
		}
	...
	return s.WriteRowsWithOutbox(ctx, rows, &contracts.OutboxEntry{Data: mustMarshalMessage(msg), CreatedAt: now})
}

func validateRowBatchIdentity(rows []*pb.ShardRow) error {
	if len(rows) == 0 {
		return nil
	}
	for _, row := range rows {
...
func (s *Store) DeleteRowsWithCommittedMessage(ctx context.Context, keys []*pb.ShardKey) error {
	if len(keys) == 0 {
		return nil
	}
	...
	if keys[0].GetDataKind() == pb.DataKind_DATA_KIND_TIME_SERIES {
		...
		for _, key := range keys {
			subject, freq, _, err := rowkey.ParseTimeSeriesDataKey(key.GetKey())
			if err != nil {
				return err
			}
			dimensions, err := s.dimensionsForDelete(key)
			if err != nil {
				return err
			}
			event.Writes = append(event.Writes, &pb.TimeSeriesRowWrite{Operation: pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE, Row: &pb.TimeSeriesRow{Key: &pb.TimeSeriesKey{
				SpaceId: key.GetSpaceId(), DatasetId: key.GetDatasetId(), SubjectId: subject, Freq: freq, Dimensions: dimensions, DataTime: key.GetVersion(),
			}}})
		}
		...
		return s.deleteRows(ctx, keys, &contracts.OutboxEntry{Data: mustMarshalMessage(msg), CreatedAt: now})
	}
	...
	for _, key := range keys {
		event.Writes = append(event.Writes, &pb.RecordRowWrite{Operation: pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE, Row: &pb.RecordRow{Key: &pb.RecordKey{
			SpaceId: key.GetSpaceId(), DatasetId: key.GetDatasetId(), RecordId: rowkey.ParseRecordDataKey(key.GetKey()), Version: publicRecordVersion(key.GetVersion()),
		}}})
	}
	raw, err := proto.MarshalOptions{Deterministic: true}.Marshal(event)
	if err != nil {
...
func (s *Store) dimensionsForDelete(key *pb.ShardKey) (map[string]string, error) {
	if key == nil || key.GetDataKind() != pb.DataKind_DATA_KIND_TIME_SERIES {
		return nil, nil
	}
...
func validateKeyBatchIdentity(keys []*pb.ShardKey) error {
	if len(keys) == 0 {
		return nil
	}
	for _, key := range keys {
...
func primaryTimeSeriesRow(row *pb.ShardRow) (*pb.TimeSeriesRow, error) {
	if row == nil || row.GetKey() == nil {
		return nil, errors.New("time-series row key is required")
	}
	subject, freq, _, err := rowkey.ParseTimeSeriesDataKey(row.GetKey().GetKey())
	if err != nil {
		return nil, err
	}
	dimensions := map[string]string{}
	...
	return &pb.TimeSeriesRow{Key: &pb.TimeSeriesKey{
		...
		RemovedColumnNames: append([]string(nil), row.GetRemovedColumnNames()...), RemovedColumns: cloneColumnRemovals(row.GetRemovedColumns()), SourceShardId: row.GetSourceShardId(), SourceSequence: row.GetSourceSequence()}, nil
}

func primaryRecordRow(row *pb.ShardRow) (*pb.RecordRow, error) {
	if row == nil || row.GetKey() == nil {
		return nil, errors.New("record row key is required")
	}
	return &pb.RecordRow{Key: &pb.RecordKey{
		SpaceId: row.GetKey().GetSpaceId(), DatasetId: row.GetKey().GetDatasetId(), RecordId: rowkey.ParseRecordDataKey(row.GetKey().GetKey()),
...
func publicRecordVersion(version string) string {
	if strings.TrimSpace(version) == rowkey.EmptyVersion {
		return ""
	}
	return version
}
