...
func (s *Store) WriteRowsWithOutbox(ctx context.Context, rows []*pb.ShardRow, entry *contracts.OutboxEntry) error {
...
func (s *Store) stageOutbox(batch *cpebble.Batch, entry *contracts.OutboxEntry) error {
	seq, err := s.nextOutboxSequence()
	if err != nil {
		return err
	}
	return s.stageOutboxAt(batch, entry, seq)
}

func (s *Store) stageOutboxAt(batch *cpebble.Batch, entry *contracts.OutboxEntry, seq uint64) error {
	entry.Sequence = seq
	msg := &messagepb.MooxMessage{}
	if err := proto.Unmarshal(entry.Data, msg); err != nil {
		return fmt.Errorf("decode outbox message: %w", err)
...
func setCommittedPayloadSequence(msg *messagepb.MooxMessage, sequence uint64) error {
	...
	case timeSeriesRowsCommittedType:
		...
		for _, write := range event.GetWrites() {
			...
			write.SourceShardId, write.SourceSequence = event.GetShardId(), sequence
			if write.GetRow() != nil {
				write.Row.SourceShardId, write.Row.SourceSequence = event.GetShardId(), sequence
				for _, removal := range write.Row.GetRemovedColumns() {
					if removal != nil {
						removal.SourceShardId, removal.SourceSequence = event.GetShardId(), sequence
					}
				}
			}
		}
