...
func encodeRowKey(row *pb.ShardRow) string {
	return encodeShardKey(row.GetKey())
}

func encodeShardKey(key *pb.ShardKey) string {
...
func encodeKeyPrefix(key *pb.ShardKey) string {
	return strings.Join([]string{
		kindPrefix(key.GetDataKind()),
		escape(key.GetSpaceId()),
		escape(key.GetDatasetId()),
...
func keyBounds(key *pb.ShardKey, versionRange *pb.VersionRange) ([]byte, []byte) {
