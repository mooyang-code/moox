...
func (s *Service) currentTimeSeriesRows(ctx context.Context, keys []*pb.TimeSeriesKey) ([]*pb.TimeSeriesRow, error) {
	...
	return s.readTimeSeriesRowMapperRows(ctx, queryKeys)
}

func (s *Service) readTimeSeriesRowMapperRows(ctx context.Context, keys []*pb.TimeSeriesKey) ([]*pb.TimeSeriesRow, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	...
	if rsp == nil {
		return nil, errors.New("read time-series projection row returned nil retinfo")
	}
