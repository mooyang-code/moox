...
func (s *Service) currentRecordRows(ctx context.Context, keys []*pb.RecordKey) ([]*pb.RecordRow, error) {
	...
	if rsp == nil {
		return nil, errors.New("read record rows returned nil retinfo")
	}
...
func (s *Service) readRecordRowMapperRows(ctx context.Context, keys []*pb.RecordKey) ([]*pb.RecordRow, error) {
	if len(keys) == 0 {
		return nil, nil
	}
	...
	if rsp == nil {
		return nil, errors.New("read record projection row returned nil retinfo")
	}
	if err := retInfoError(rsp.GetRetInfo()); err != nil {
		return nil, err
	}
