...
func normalizeSubscriberOptions(opts SubscriberOptions) (SubscriberOptions, error) {
	...
	// A ViewBuilder applies one source-shard sequence lane. Parallel delivery
	// would let a later message establish the lane before an earlier message
	// arrives, so transport concurrency is deliberately serialized here.
	opts.MaxInFlight = 1
	return opts, nil
}
...
func validateRowsCommittedMessage(msg *messagepb.MooxMessage, expectedShardID string) error {
	...
	case defaultTimeSeriesRowsCommittedType:
		...
		if err := validateTimeSeriesWrites(event); err != nil {
			return err
		}
	case defaultRecordRowsCommittedType:
		...
		payloadShard, payloadSpace, payloadDataset, payloadSequence = event.GetShardId(), event.GetSpaceId(), event.GetDatasetId(), event.GetSequence()
		if err := validateRecordWrites(event); err != nil {
			return err
		}
	default:
...
func validateTimeSeriesWrites(event *pb.TimeSeriesRowsCommitted) error {
	if event == nil || len(event.GetWrites()) == 0 {
		return errors.New("time-series rows committed payload must contain writes")
	}
	for index, write := range event.GetWrites() {
		if write == nil || write.GetRow() == nil || write.GetRow().GetKey() == nil {
			return fmt.Errorf("time-series rows committed write %d must contain a row key", index)
		}
		if write.GetOperation() != pb.RowWriteOperation_ROW_WRITE_OPERATION_MERGE && write.GetOperation() != pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE {
			return fmt.Errorf("time-series rows committed write %d has invalid operation", index)
		}
		key := write.GetRow().GetKey()
		if key.GetSpaceId() != event.GetSpaceId() || key.GetDatasetId() != event.GetDatasetId() {
			return fmt.Errorf("time-series rows committed write %d key identity does not match payload", index)
		}
		if write.GetOperation() == pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE && (len(write.GetRow().GetColumns()) != 0 || len(write.GetRow().GetAttributes()) != 0 || len(write.GetRow().GetRemovedColumns()) != 0 || len(write.GetRow().GetRemovedColumnNames()) != 0) {
			return fmt.Errorf("time-series rows committed DELETE write %d contains row data", index)
		}
	}
	return nil
}

func validateRecordWrites(event *pb.RecordRowsCommitted) error {
	if event == nil || len(event.GetWrites()) == 0 {
		return errors.New("record rows committed payload must contain writes")
	}
	for index, write := range event.GetWrites() {
		if write == nil || write.GetRow() == nil || write.GetRow().GetKey() == nil {
			return fmt.Errorf("record rows committed write %d must contain a row key", index)
		}
		if write.GetOperation() != pb.RowWriteOperation_ROW_WRITE_OPERATION_MERGE && write.GetOperation() != pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE {
			return fmt.Errorf("record rows committed write %d has invalid operation", index)
		}
		key := write.GetRow().GetKey()
		if key.GetSpaceId() != event.GetSpaceId() || key.GetDatasetId() != event.GetDatasetId() {
			return fmt.Errorf("record rows committed write %d key identity does not match payload", index)
		}
		if write.GetOperation() == pb.RowWriteOperation_ROW_WRITE_OPERATION_DELETE && (len(write.GetRow().GetColumns()) != 0 || len(write.GetRow().GetAttributes()) != 0 || len(write.GetRow().GetRemovedColumns()) != 0 || len(write.GetRow().GetRemovedColumnNames()) != 0) {
			return fmt.Errorf("record rows committed DELETE write %d contains row data", index)
		}
	}
	return nil
}
...
// payload kind.
func (b *SubscriberBus) SubscribeRowsCommitted(ctx context.Context, handler RowsCommittedHandler) (Subscription, error) {
	if handler == nil {
	...
	timeSub, err := b.SubscribeTimeSeriesRowsCommitted(ctx, func(handlerCtx context.Context, event *pb.TimeSeriesRowsCommitted) error {
		return handler(handlerCtx, &RowsCommittedEvent{TimeSeries: event})
	})
	if err != nil {
		return nil, err
	}
	recordSub, err := b.SubscribeRecordRowsCommitted(ctx, func(handlerCtx context.Context, event *pb.RecordRowsCommitted) error {
		return handler(handlerCtx, &RowsCommittedEvent{Record: event})
	})
	if err != nil {
		_ = timeSub.Close()
		return nil, err
	}
	return &combinedSubscription{items: []Subscription{timeSub, recordSub}}, nil
}

func (b *SubscriberBus) consumerRef(_ bool) jetstream.ConsumerRef {
	return jetstream.ConsumerRef{
		Stream: b.opts.StreamName, Durable: "storage_view_rows_committed_v1", FilterSubject: RowsCommittedSubjectWildcard(b.subjectPrefix()),
		AckWait: b.opts.AckWait, MaxDeliver: b.opts.MaxDeliver, MaxAckPending: b.opts.MaxAckPending,
	}
}

func (b *SubscriberBus) SubscribeTimeSeriesRowsCommitted(ctx context.Context, handler TimeSeriesRowsCommittedHandler) (Subscription, error) {
	if handler == nil {
		return noopSubscription{}, nil
	}
	b.mu.Lock()
...
func (b *SubscriberBus) SubscribeRecordRowsCommitted(ctx context.Context, handler RecordRowsCommittedHandler) (Subscription, error) {
	if handler == nil {
		return noopSubscription{}, nil
	}
	b.mu.Lock()
...
type combinedSubscription struct{ items []Subscription }
