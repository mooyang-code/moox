...
func testTimeSeriesMessage(payload []byte) *messagepb.MooxMessage {
	...
	if len(event.GetWrites()) == 0 {
		event.Writes = []*pb.TimeSeriesRowWrite{{
			Operation: pb.RowWriteOperation_ROW_WRITE_OPERATION_MERGE,
			Row: &pb.TimeSeriesRow{Key: &pb.TimeSeriesKey{
				SpaceId: event.GetSpaceId(), DatasetId: event.GetDatasetId(), SubjectId: "subject-1", Freq: "1m", DataTime: "2026-01-01T00:00:00Z",
			}},
		}}
	}
	event.Sequence = 1
