...
func TestServiceStartValidatesRequiredDependencies(t *testing.T) {
	engine := newRecordingViewIndexEngine("bleve")
	base := Options{
		Events:   eventconsumer.NewMemoryBus(),
...
func TestServiceStartSubscribeAndCloseRoundTrip(t *testing.T) {
	bus := eventconsumer.NewMemoryBus()
	svc := NewService(Options{
		Events:     bus,
		Reader:     &buildingGuardReader{},
		Metadata:   newBuildingGuardMetadata(),
...
func (s *blockingSubscriber) SubscribeRowsCommitted(ctx context.Context, _ eventconsumer.RowsCommittedHandler) (eventconsumer.Subscription, error) {
	close(s.entered)
	<-ctx.Done()
	return nil, ctx.Err()
}

func (s *blockingSubscriber) SubscribeTimeSeriesRowsCommitted(ctx context.Context, _ eventconsumer.TimeSeriesRowsCommittedHandler) (eventconsumer.Subscription, error) {
	close(s.entered)
	<-ctx.Done()
	return nil, ctx.Err()
}

func (*blockingSubscriber) SubscribeRecordRowsCommitted(context.Context, eventconsumer.RecordRowsCommittedHandler) (eventconsumer.Subscription, error) {
...
func TestReadRecordRowMapperRowsReturnsRows(t *testing.T) {
	key := &pb.RecordKey{
	...
	rows, err := svc.readRecordRowMapperRows(context.Background(), []*pb.RecordKey{key})
	require.NoError(t, err)
