...
func (r *Resolver) Resolve(ctx context.Context, spaceID string, datasetID string, subjectID string) (*pb.ShardTarget, error) {
	...
	for _, route := range routes {
		...
		if !supportedHashRule(route.GetHashRule()) {
			return nil, fmt.Errorf("unsupported primary store hash rule %q on route %s", route.GetHashRule(), route.GetRouteId())
		}
...
func (r *Resolver) selectHashedRoute(ctx context.Context, spaceID string, datasetID string, partitionKey string, candidates []routeCandidate) (*pb.PrimaryStoreRoute, error) {
	...
	for _, candidate := range candidates {
		if !supportedHashRule(candidate.route.GetHashRule()) {
			return nil, fmt.Errorf("unsupported primary store hash rule %q on route %s", candidate.route.GetHashRule(), candidate.route.GetRouteId())
		}
	}
...
func (r *Resolver) ResolveDatasetTargets(ctx context.Context, spaceID string, datasetID string) ([]*pb.ShardTarget, error) {
	if spaceID == "" || datasetID == "" {
		return nil, fmt.Errorf("space_id and dataset_id are required")
	}
	routes, err := r.listDatasetRoutes(ctx, spaceID, datasetID)
...
func (r *Resolver) targetForRoute(ctx context.Context, spaceID string, datasetID string, route *pb.PrimaryStoreRoute) (*pb.ShardTarget, error) {
	if route == nil {
	...
	shardID, err := shardIdentity(node, device)
	if err != nil {
		return nil, err
	}
...
func shardIdentity(node *pb.PrimaryStoreNode, device *pb.Device) (string, error) {
	if device != nil && device.GetAttributes() != nil && strings.TrimSpace(device.GetAttributes()["shard_id"]) != "" {
		return strings.TrimSpace(device.GetAttributes()["shard_id"]), nil
	}
	if node != nil && node.GetAttributes() != nil && strings.TrimSpace(node.GetAttributes()["shard_id"]) != "" {
		return strings.TrimSpace(node.GetAttributes()["shard_id"]), nil
	}
	return "", fmt.Errorf("storage node %s and device %s must define shard_id", node.GetNodeId(), device.GetDeviceId())
}
...
func (r *Resolver) resolvePrimaryDevice(ctx context.Context, nodeID string) (*pb.Device, error) {
	...
	if len(active) > 1 {
		return nil, fmt.Errorf("storage node %s has %d active pebble devices; shard routing requires exactly one", nodeID, len(active))
	}
	if len(active) == 1 {
		return active[0], nil
	}
