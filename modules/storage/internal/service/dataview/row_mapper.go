//go:build legacy_storage

package view

import (
	"context"
	"encoding/json"
	"errors"
	"strings"

	pb "github.com/mooyang-code/moox/modules/storage/proto/storagegen"
	"google.golang.org/protobuf/proto"
)

const projectionReadBatchSize = 500

type TimeSeriesRowMapperReader func(context.Context, []*pb.TimeSeriesKey) ([]*pb.TimeSeriesRow, error)

type RecordRowMapperReader func(context.Context, []*pb.RecordKey) ([]*pb.RecordRow, error)

// TimeSeriesRowsForView projects fact rows into the columns exposed by a view.
func TimeSeriesRowsForView(
	ctx context.Context,
	item *pb.View,
	columns []*pb.ViewColumn,
	rows []*pb.TimeSeriesRow,
	readRowMapperRows TimeSeriesRowMapperReader,
) ([]*pb.TimeSeriesRow, bool, error) {
	if item == nil || !IsProjectableTimeSeriesView(item, columns) {
		return nil, false, nil
	}
	primaryDatasetID := item.GetPrimaryDatasetId()
	datasetIDs := ViewRowMapperDatasets(primaryDatasetID, columns)
	type projectionGroup struct {
		template      *pb.TimeSeriesKey
		rowsByDataset map[string]*pb.TimeSeriesRow
	}
	groups := make(map[string]*projectionGroup, len(rows))
	order := make([]string, 0, len(rows))
	for _, row := range rows {
		if row == nil || row.GetKey() == nil {
			continue
		}
		grainKey := TimeSeriesRowMapperGrainKey(row.GetKey())
		group := groups[grainKey]
		if group == nil {
			group = &projectionGroup{template: row.GetKey(), rowsByDataset: make(map[string]*pb.TimeSeriesRow)}
			groups[grainKey] = group
			order = append(order, grainKey)
		}
		group.rowsByDataset[row.GetKey().GetDatasetId()] = row
	}
	for _, datasetID := range datasetIDs {
		var missing []*pb.TimeSeriesKey
		for _, grainKey := range order {
			group := groups[grainKey]
			if group.rowsByDataset[datasetID] != nil {
				continue
			}
			key := proto.Clone(group.template).(*pb.TimeSeriesKey)
			key.DatasetId = datasetID
			missing = append(missing, key)
		}
		if len(missing) == 0 {
			continue
		}
		if readRowMapperRows == nil {
			continue
		}
		for start := 0; start < len(missing); start += projectionReadBatchSize {
			end := min(start+projectionReadBatchSize, len(missing))
			readRows, err := readRowMapperRows(ctx, missing[start:end])
			if err != nil {
				return nil, true, err
			}
			for _, read := range readRows {
				if read == nil || read.GetKey() == nil {
					continue
				}
				if read.GetKey().GetDatasetId() != datasetID {
					return nil, true, errors.New("time-series projection batch returned a row from the wrong dataset")
				}
				if group := groups[TimeSeriesRowMapperGrainKey(read.GetKey())]; group != nil {
					group.rowsByDataset[datasetID] = read
				}
			}
		}
	}
	out := make([]*pb.TimeSeriesRow, 0, len(order))
	for _, grainKey := range order {
		group := groups[grainKey]
		primaryRow := group.rowsByDataset[primaryDatasetID]
		if primaryRow == nil {
			continue
		}
		out = append(out, &pb.TimeSeriesRow{
			Key:            proto.Clone(primaryRow.GetKey()).(*pb.TimeSeriesKey),
			Columns:        ProjectColumnsForView(primaryDatasetID, columns, group.rowsByDataset),
			Attributes:     CloneStringMap(primaryRow.GetAttributes()),
			RemovedColumns: ProjectColumnRemovalsForView(primaryDatasetID, columns, group.rowsByDataset),
			SourceShardId:  primaryRow.GetSourceShardId(), SourceSequence: primaryRow.GetSourceSequence(),
		})
	}
	return out, true, nil
}

// FilteredTimeSeriesRowsForView applies view.filter_json before projecting
// fact rows into a TimeSeries view.
func FilteredTimeSeriesRowsForView(
	ctx context.Context,
	item *pb.View,
	columns []*pb.ViewColumn,
	rows []*pb.TimeSeriesRow,
	readRowMapperRows TimeSeriesRowMapperReader,
) ([]*pb.TimeSeriesRow, bool, error) {
	if item == nil || !IsProjectableTimeSeriesView(item, columns) {
		return nil, false, nil
	}
	filtered, err := filterRowsByViewJSON(item, rows)
	if err != nil {
		return nil, true, err
	}
	return TimeSeriesRowsForView(ctx, item, columns, filtered, readRowMapperRows)
}

// FilteredTimeSeriesSourceRowsForView applies the view predicate without
// requiring the primary dataset row. ViewBuilder uses it for secondary-source
// fragments, which must be merged without rereading unchanged sources.
func FilteredTimeSeriesSourceRowsForView(item *pb.View, rows []*pb.TimeSeriesRow) ([]*pb.TimeSeriesRow, bool, error) {
	if item == nil {
		return nil, false, nil
	}
	filtered, err := filterRowsByViewJSON(item, rows)
	if err != nil {
		return nil, true, err
	}
	return filtered, true, nil
}

// RecordRowsForView projects record rows into the columns exposed by a view.
func RecordRowsForView(
	ctx context.Context,
	item *pb.View,
	columns []*pb.ViewColumn,
	rows []*pb.RecordRow,
	readRowMapperRows RecordRowMapperReader,
) ([]*pb.RecordRow, bool, error) {
	if item == nil || !IsProjectableRecordView(item, columns) {
		return nil, false, nil
	}
	primaryDatasetID := item.GetPrimaryDatasetId()
	datasetIDs := ViewRowMapperDatasets(primaryDatasetID, columns)
	type projectionGroup struct {
		template      *pb.RecordKey
		rowsByDataset map[string]*pb.RecordRow
	}
	groups := make(map[string]*projectionGroup, len(rows))
	order := make([]string, 0, len(rows))
	for _, row := range rows {
		if row == nil || row.GetKey() == nil {
			continue
		}
		grainKey := RecordRowMapperGrainKey(row.GetKey())
		group := groups[grainKey]
		if group == nil {
			group = &projectionGroup{template: row.GetKey(), rowsByDataset: make(map[string]*pb.RecordRow)}
			groups[grainKey] = group
			order = append(order, grainKey)
		}
		group.rowsByDataset[row.GetKey().GetDatasetId()] = row
	}
	for _, datasetID := range datasetIDs {
		var missing []*pb.RecordKey
		for _, grainKey := range order {
			group := groups[grainKey]
			if group.rowsByDataset[datasetID] != nil {
				continue
			}
			key := proto.Clone(group.template).(*pb.RecordKey)
			key.DatasetId = datasetID
			missing = append(missing, key)
		}
		if len(missing) == 0 || readRowMapperRows == nil {
			continue
		}
		for start := 0; start < len(missing); start += projectionReadBatchSize {
			end := min(start+projectionReadBatchSize, len(missing))
			readRows, err := readRowMapperRows(ctx, missing[start:end])
			if err != nil {
				return nil, true, err
			}
			for _, read := range readRows {
				if read == nil || read.GetKey() == nil {
					continue
				}
				if read.GetKey().GetDatasetId() != datasetID {
					return nil, true, errors.New("Record projection batch returned a row from the wrong dataset")
				}
				if group := groups[RecordRowMapperGrainKey(read.GetKey())]; group != nil {
					group.rowsByDataset[datasetID] = read
				}
			}
		}
	}
	out := make([]*pb.RecordRow, 0, len(order))
	for _, grainKey := range order {
		group := groups[grainKey]
		primaryRow := group.rowsByDataset[primaryDatasetID]
		if primaryRow == nil {
			continue
		}
		out = append(out, &pb.RecordRow{
			Key:            proto.Clone(primaryRow.GetKey()).(*pb.RecordKey),
			Columns:        ProjectRecordColumnsForView(primaryDatasetID, columns, group.rowsByDataset),
			Attributes:     CloneStringMap(primaryRow.GetAttributes()),
			RemovedColumns: ProjectRecordColumnRemovalsForView(primaryDatasetID, columns, group.rowsByDataset),
			SourceShardId:  primaryRow.GetSourceShardId(), SourceSequence: primaryRow.GetSourceSequence(),
		})
	}
	return out, true, nil
}

func cloneColumnRemovals(values []*pb.ColumnRemoval) []*pb.ColumnRemoval {
	out := make([]*pb.ColumnRemoval, 0, len(values))
	for _, value := range values {
		if value != nil {
			out = append(out, proto.Clone(value).(*pb.ColumnRemoval))
		}
	}
	return out
}

// ProjectColumnRemovalsForView carries source tombstones into their view
// column names. A source row's legacy removal names use that row's snapshot
// stamp, while durable removals retain their own stamp.
func ProjectColumnRemovalsForView(primaryDatasetID string, columns []*pb.ViewColumn, rowsByDataset map[string]*pb.TimeSeriesRow) []*pb.ColumnRemoval {
	return projectColumnRemovals(primaryDatasetID, columns, func(datasetID string) ([]*pb.ColumnRemoval, []string, string, uint64) {
		row := rowsByDataset[datasetID]
		if row == nil {
			return nil, nil, "", 0
		}
		return row.GetRemovedColumns(), row.GetRemovedColumnNames(), row.GetSourceShardId(), row.GetSourceSequence()
	})
}

// ProjectRecordColumnRemovalsForView is the record-row counterpart of
// ProjectColumnRemovalsForView.
func ProjectRecordColumnRemovalsForView(primaryDatasetID string, columns []*pb.ViewColumn, rowsByDataset map[string]*pb.RecordRow) []*pb.ColumnRemoval {
	return projectColumnRemovals(primaryDatasetID, columns, func(datasetID string) ([]*pb.ColumnRemoval, []string, string, uint64) {
		row := rowsByDataset[datasetID]
		if row == nil {
			return nil, nil, "", 0
		}
		return row.GetRemovedColumns(), row.GetRemovedColumnNames(), row.GetSourceShardId(), row.GetSourceSequence()
	})
}

func projectColumnRemovals(primaryDatasetID string, columns []*pb.ViewColumn, source func(string) ([]*pb.ColumnRemoval, []string, string, uint64)) []*pb.ColumnRemoval {
	out := make([]*pb.ColumnRemoval, 0)
	for _, viewColumn := range columns {
		datasetID := ViewColumnOriginDataset(primaryDatasetID, viewColumn)
		sourceName := ViewColumnSourceName(datasetID, viewColumn)
		removals, legacyNames, rowShardID, rowSequence := source(datasetID)
		for _, removal := range removals {
			if removal == nil || removal.GetColumnName() != sourceName {
				continue
			}
			mapped := proto.Clone(removal).(*pb.ColumnRemoval)
			mapped.ColumnName = viewColumn.GetColumnName()
			out = append(out, mapped)
		}
		for _, name := range legacyNames {
			if name != sourceName {
				continue
			}
			out = append(out, &pb.ColumnRemoval{ColumnName: viewColumn.GetColumnName(), SourceShardId: rowShardID, SourceSequence: rowSequence})
		}
	}
	return dedupeColumnRemovals(out)
}

func dedupeColumnRemovals(values []*pb.ColumnRemoval) []*pb.ColumnRemoval {
	seen := make(map[string]struct{}, len(values))
	out := make([]*pb.ColumnRemoval, 0, len(values))
	for _, value := range values {
		if value == nil || value.GetColumnName() == "" {
			continue
		}
		if _, ok := seen[value.GetColumnName()]; ok {
			continue
		}
		seen[value.GetColumnName()] = struct{}{}
		out = append(out, value)
	}
	return out
}

// IsProjectableTimeSeriesView reports whether the view can be projected from fact rows.
func IsProjectableTimeSeriesView(item *pb.View, columns []*pb.ViewColumn) bool {
	if item == nil || item.GetPrimaryDatasetId() == "" {
		return false
	}
	for _, column := range columns {
		if column.GetOriginType() != pb.ColumnOriginType_COLUMN_ORIGIN_TYPE_DATASET_COLUMN {
			return false
		}
		datasetID := ViewColumnOriginDataset(item.GetPrimaryDatasetId(), column)
		if datasetID == "" {
			return false
		}
	}
	return true
}

// IsProjectableRecordView reports whether the view can be projected from record rows.
func IsProjectableRecordView(item *pb.View, columns []*pb.ViewColumn) bool {
	if item == nil || item.GetPrimaryDatasetId() == "" {
		return false
	}
	for _, column := range columns {
		if column.GetOriginType() != pb.ColumnOriginType_COLUMN_ORIGIN_TYPE_DATASET_COLUMN {
			return false
		}
		if ViewColumnOriginDataset(item.GetPrimaryDatasetId(), column) == "" {
			return false
		}
	}
	return true
}

// ProjectColumnsForView projects time-series row columns into view column names.
func ProjectColumnsForView(primaryDatasetID string, columns []*pb.ViewColumn, rowsByDataset map[string]*pb.TimeSeriesRow) []*pb.ColumnValue {
	valuesByDataset := make(map[string]map[string]*pb.ColumnValue, len(rowsByDataset))
	for datasetID, row := range rowsByDataset {
		values := make(map[string]*pb.ColumnValue, len(row.GetColumns()))
		for _, column := range row.GetColumns() {
			values[column.GetColumnName()] = column
		}
		valuesByDataset[datasetID] = values
	}
	out := make([]*pb.ColumnValue, 0, len(columns))
	for _, viewColumn := range columns {
		datasetID := ViewColumnOriginDataset(primaryDatasetID, viewColumn)
		sourceName := ViewColumnSourceName(datasetID, viewColumn)
		source, ok := valuesByDataset[datasetID][sourceName]
		if !ok {
			if timeSeriesRowHasRemoval(rowsByDataset[datasetID], sourceName) {
				continue
			}
			out = append(out, &pb.ColumnValue{ColumnName: viewColumn.GetColumnName(), ValueType: viewColumn.GetValueType()})
			continue
		}
		copied := proto.Clone(source).(*pb.ColumnValue)
		copied.ColumnName = viewColumn.GetColumnName()
		if copied.ValueType == pb.FieldValueType_FIELD_VALUE_TYPE_UNSPECIFIED {
			copied.ValueType = viewColumn.GetValueType()
		}
		out = append(out, copied)
	}
	return out
}

// ProjectRecordColumnsForView projects record row columns into view column names.
func ProjectRecordColumnsForView(primaryDatasetID string, columns []*pb.ViewColumn, rowsByDataset map[string]*pb.RecordRow) []*pb.ColumnValue {
	valuesByDataset := make(map[string]map[string]*pb.ColumnValue, len(rowsByDataset))
	for datasetID, row := range rowsByDataset {
		values := make(map[string]*pb.ColumnValue, len(row.GetColumns()))
		for _, column := range row.GetColumns() {
			values[column.GetColumnName()] = column
		}
		valuesByDataset[datasetID] = values
	}
	out := make([]*pb.ColumnValue, 0, len(columns))
	for _, viewColumn := range columns {
		datasetID := ViewColumnOriginDataset(primaryDatasetID, viewColumn)
		sourceName := ViewColumnSourceName(datasetID, viewColumn)
		source, ok := valuesByDataset[datasetID][sourceName]
		if !ok {
			if recordRowHasRemoval(rowsByDataset[datasetID], sourceName) {
				continue
			}
			out = append(out, &pb.ColumnValue{ColumnName: viewColumn.GetColumnName(), ValueType: viewColumn.GetValueType()})
			continue
		}
		copied := proto.Clone(source).(*pb.ColumnValue)
		copied.ColumnName = viewColumn.GetColumnName()
		if copied.ValueType == pb.FieldValueType_FIELD_VALUE_TYPE_UNSPECIFIED {
			copied.ValueType = viewColumn.GetValueType()
		}
		out = append(out, copied)
	}
	return out
}

func timeSeriesRowHasRemoval(row *pb.TimeSeriesRow, name string) bool {
	if row == nil {
		return false
	}
	for _, removal := range row.GetRemovedColumns() {
		if removal != nil && removal.GetColumnName() == name {
			return true
		}
	}
	for _, removed := range row.GetRemovedColumnNames() {
		if removed == name {
			return true
		}
	}
	return false
}

func recordRowHasRemoval(row *pb.RecordRow, name string) bool {
	if row == nil {
		return false
	}
	for _, removal := range row.GetRemovedColumns() {
		if removal != nil && removal.GetColumnName() == name {
			return true
		}
	}
	for _, removed := range row.GetRemovedColumnNames() {
		if removed == name {
			return true
		}
	}
	return false
}

// ViewRowMapperDatasets returns the datasets needed to project a view.
func ViewRowMapperDatasets(primaryDatasetID string, columns []*pb.ViewColumn) []string {
	seen := make(map[string]bool, len(columns)+1)
	out := make([]string, 0, len(columns)+1)
	add := func(datasetID string) {
		if datasetID == "" || seen[datasetID] {
			return
		}
		seen[datasetID] = true
		out = append(out, datasetID)
	}
	add(primaryDatasetID)
	for _, column := range columns {
		add(ViewColumnOriginDataset(primaryDatasetID, column))
	}
	return out
}

// ViewColumnOriginDataset returns the source dataset for a view column.
func ViewColumnOriginDataset(primaryDatasetID string, column *pb.ViewColumn) string {
	if column.GetOriginType() == pb.ColumnOriginType_COLUMN_ORIGIN_TYPE_DATASET_COLUMN {
		if idx := strings.LastIndex(column.GetOriginId(), "."); idx > 0 {
			return column.GetOriginId()[:idx]
		}
	}
	return primaryDatasetID
}

// ViewColumnSourceName returns the source column name for a view column.
func ViewColumnSourceName(datasetID string, column *pb.ViewColumn) string {
	originID := column.GetOriginId()
	prefix := datasetID + "."
	if strings.HasPrefix(originID, prefix) {
		return strings.TrimPrefix(originID, prefix)
	}
	if idx := strings.LastIndex(originID, "."); idx >= 0 {
		return originID[idx+1:]
	}
	if originID != "" {
		return originID
	}
	return column.GetColumnName()
}

// TimeSeriesRowMapperGrainKey returns the view projection dedupe key for a time-series row key.
func TimeSeriesRowMapperGrainKey(key *pb.TimeSeriesKey) string {
	dimensions, _ := json.Marshal(key.GetDimensions())
	return strings.Join([]string{
		key.GetSpaceId(),
		key.GetSubjectId(),
		key.GetFreq(),
		key.GetDataTime(),
		string(dimensions),
	}, "\x00")
}

// RecordRowMapperGrainKey returns the view projection dedupe key for a record row key.
func RecordRowMapperGrainKey(key *pb.RecordKey) string {
	return strings.Join([]string{
		key.GetSpaceId(),
		key.GetRecordId(),
		key.GetVersion(),
	}, "\x00")
}

// CloneStringMap clones a string map.
func CloneStringMap(in map[string]string) map[string]string {
	if len(in) == 0 {
		return nil
	}
	out := make(map[string]string, len(in))
	for key, value := range in {
		out[key] = value
	}
	return out
}
