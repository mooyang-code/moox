...
func (s *Service) publishSubjectReady(ctx context.Context, view viewRef, indexID, datasetID string, rows []*pb.RowFieldUpsert, origins map[*pb.RowFieldUpsert]*eventpb.EventMessage) error {
	if len(origins) == 0 {
		return nil
	}
	s.mu.RLock()
	...
	for _, row := range rows {
		...
		origin := origins[row]
		if origin == nil || origin.GetEventId() == "" || origin.GetOccurredAt() == nil || schema.SchemaHash == "" {
		...
		eventID := stableViewEventID("source-subject-ready", view.spaceID, view.viewID, indexID, contract, datasetID, origin.GetEventId(), key.GetSubjectId(), key.GetFreq(), key.GetDataTime(), key.GetSeriesTag())
		...
		ready := &storagepb.ViewSourceSubjectReady{
			...
			ActiveIndexId: indexID, InputContractVersion: contract, SourceEventId: origin.GetEventId(), ReadyAt: readyAt,
		}
