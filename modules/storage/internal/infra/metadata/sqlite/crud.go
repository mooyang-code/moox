...
func (s *Store) UpsertView(ctx context.Context, item *pb.View) (*pb.View, error) {
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO t_views (c_space_id, c_view_id, c_name, c_description, c_primary_dataset_id, c_dataset_ids_json, c_grain_keys_json, c_filter_json, c_engine, c_record_view_mode, c_retention_window, c_active_index_id, c_view_version, c_active_view_version, c_active_columns_json, c_active_schema_hash, c_active_coverage_start, c_active_coverage_end, c_status, c_attrs_json)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(c_space_id, c_view_id) DO UPDATE SET
			...
			c_record_view_mode = excluded.c_record_view_mode,
			...
			c_status = excluded.c_status,
			c_attrs_json = excluded.c_attrs_json
	`, next.GetSpaceId(), next.GetViewId(), next.GetName(), next.GetDescription(), next.GetPrimaryDatasetId(), datasetIDs, grainKeys, defaultJSON(next.GetFilterJson()), next.GetEngine(), next.GetRecordViewMode(), next.GetRetentionWindow(), next.GetActiveIndexId(), next.GetViewVersion(), next.GetActiveViewVersion(), activeColumns, next.GetActiveSchemaHash(), next.GetActiveCoverageStart(), next.GetActiveCoverageEnd(), next.GetStatus(), raw)
	if err != nil {
		return nil, err
	}
	if shapeChanged {
...
func viewIndexShapeChanged(existing *pb.View, next *pb.View) bool {
	...
	if existing.GetRecordViewMode() != next.GetRecordViewMode() {
		return true
	}
