...
func (s *ViewStore) Apply(ctx context.Context, tableName string, batch viewindex.ViewIndexApplyBatch) error {
	...
	for _, write := range batch.RowWrites {
		...
		row := &pb.TimeSeriesRow{Key: write.Key.TimeSeriesKey, Columns: write.Columns, Attributes: write.Attributes, AttributesToDelete: write.AttributesToDelete, RemovedColumnNames: write.RemovedColumnNames}
	...
	if err := deleteRowsByPrimaryKey(ctx, tx, quoted, append(deleteRows, upsertRows...)); err != nil {
		return rollback(err)
	}
	if len(merged) > 0 {
