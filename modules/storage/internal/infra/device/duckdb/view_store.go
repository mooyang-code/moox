...
func rowMatchesFilter(row *pb.TimeSeriesRow, filter *pb.FilterExpr) (bool, error) {
	...
	if fn, field, token, ok := parseFunctionFilter(filter.GetExpr()); ok {
		rowValue, exists := rowColumnValue(row, field)
		switch fn {
		case "is_empty":
			return !exists || factvalue.String(rowValue) == "", nil
		case "is_not_empty":
			return exists && factvalue.String(rowValue) != "", nil
		}
		if !exists {
			return fn == "not_contains", nil
		}
		expected := filterValue(token, filter.GetArgs())
		if expected == nil {
			return false, fmt.Errorf("unsupported filter value %q", token)
		}
