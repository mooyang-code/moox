...
- NATS is reachable from Storage and Factor, and stream `MOOX_STORAGE` includes `moox.storage.dataset.rows.upserted.v1.>`.
...
4. Confirm Factor consumes `moox.storage.dataset.rows.upserted.v1.>` events.
