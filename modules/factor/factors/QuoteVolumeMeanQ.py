"""Rolling percentile rank of rolling quote volume mean."""
...
def _windows(params):
    ...
    if isinstance(values, (int, float, str)):
        values = [values]
    if not values:
        raise ValueError("QuoteVolumeMeanQ requires window or windows")
...
def _names(params, windows):
    ...
    if names is not None:
        if len(names) != len(windows):
            raise ValueError("QuoteVolumeMeanQ output_names length must match windows")
        return list(names)
    return [f"quote_volume_mean_q_{window}" for window in windows]


def compute(df, params):
    windows = _windows(params)
    if any(window <= 0 for window in windows):
        raise ValueError("QuoteVolumeMeanQ windows must be positive")
