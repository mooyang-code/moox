...
func TestApplySchemaAllowsUnusedLegacyTablesAndCreatesMissingOnes(t *testing.T) {
	...
	require.NoError(t, db.db.Exec(`
		...
		CREATE TABLE t_factor_period_barriers (
	...
	var extra int
	require.NoError(t, db.db.Raw(`SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 't_factor_period_barriers'`).Scan(&extra).Error)
