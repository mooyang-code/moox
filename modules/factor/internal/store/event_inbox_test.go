...
func TestEventInboxPersistsAcrossRestartAndCommitsDuplicateIDsIdempotently(t *testing.T) {
	...
	event := &storagepb.DatasetRowsUpserted{SpaceId: "crypto", DatasetId: "kline"}
	...
	claimed, err = first.ClaimPendingEvent(ctx, "message-1", &storagepb.DatasetRowsUpserted{SpaceId: "wrong"}, now.Add(time.Minute))
	if err != nil {
		t.Fatal(err)
	}
	if claimed {
	...
	var loaded int
	if err := restarted.LoadPendingEvents(ctx, func(id string, got *storagepb.DatasetRowsUpserted, receivedAt time.Time) error {
		loaded++
	...
	loaded = 0
	if err := restarted.LoadPendingEvents(ctx, func(string, *storagepb.DatasetRowsUpserted, time.Time) error {
		loaded++
		return nil
	}); err != nil {
		t.Fatal(err)
