...
func TestPeriodMetricsExposeViewDrivenSignals(t *testing.T) {
	...
	m.BeginBatches("prices-view", "1m", 3)
	m.ObserveBatch("prices-view", "1m", "complete", 2, 4, 100*time.Millisecond)
	m.ObserveBatch("prices-view", "1m", "degraded", 1, 2, 200*time.Millisecond)
	m.BeginBatches("prices-view", "1m", 0)
	...
	require.Equal(t, float64(3), testutil.ToFloat64(m.batchTotal.WithLabelValues("prices-view", "1m", "complete")))
	require.Equal(t, float64(1), testutil.ToFloat64(m.batchTotal.WithLabelValues("prices-view", "1m", "degraded")))
	require.Equal(t, float64(8), testutil.ToFloat64(m.batchFactors.WithLabelValues("prices-view", "1m")))
