...
var ErrOutputOwnershipConflict = errors.New("output row is already owned by another source series")

// OutputManifest records the dynamic result RowKeys owned by one binding task.
type OutputManifest struct {
	SourceSeriesTag       string    `gorm:"column:c_source_series_tag;primaryKey"`
	FilterSourceSeriesTag bool      `gorm:"column:c_filter_source_series_tag;primaryKey"`
	BindingID             string    `gorm:"column:c_binding_id;primaryKey"`
	BindingGeneration     string    `gorm:"column:c_binding_generation;primaryKey"`
	CleanupTaskJSON       string    `gorm:"column:c_cleanup_task_json"`
