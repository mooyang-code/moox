...
func TestSourceSeriesManifestsPreserveOtherPartitionOutputs(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, access.patchReqs, 2, "B must not clear A's output")
	_, err = client.WriteFactorPatch(ctx, &b, result("output-a"))
	require.ErrorContains(t, err, "another source series")
	require.Len(t, access.patchReqs, 2, "conflicts must fail before remote writes")
	_, err = client.WriteFactorPatch(ctx, &b, &engine.FactorResult{})
	require.NoError(t, err)
	require.Len(t, access.patchReqs, 3)
	require.Equal(t, "output-b", access.patchReqs[2].GetRow().GetKey().GetTimeSeries().GetSeriesTag())
