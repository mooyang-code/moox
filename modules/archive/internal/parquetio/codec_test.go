...
func TestWideParquetRoundTripPreservesTypesAndRows(t *testing.T) {
	...
	zero := int64(0)
	closed := false
	rows := []domain.ArchiveRow{{Partition: domain.PartitionKey{SpaceID: "crypto", DatasetID: "spot_kline_1h", SubjectID: "BTC-USDT", Freq: "1h", SeriesTag: "venue:binance", Month: "202606"}, DataTime: time.Date(2026, 6, 1, 0, 0, 0, 123, time.UTC), Attributes: map[string]string{"source": "binance"}, WrittenAt: time.Date(2026, 6, 1, 0, 0, 1, 0, time.UTC), Columns: map[string]domain.Scalar{"trade_num": {Type: storagepb.FieldValueType_FIELD_VALUE_TYPE_INT, Int: &zero}, "closed": {Type: storagepb.FieldValueType_FIELD_VALUE_TYPE_BOOL, Bool: &closed}}}}
...
func TestWideParquetRoundTripPreservesJSON(t *testing.T) {
	...
	rows := []domain.ArchiveRow{{Partition: domain.PartitionKey{SpaceID: "crypto", DatasetID: "spot_kline_1h", SubjectID: "BTC-USDT", Freq: "1h", Month: "202606"}, DataTime: time.Date(2026, 6, 1, 0, 0, 0, 0, time.UTC), Columns: map[string]domain.Scalar{"payload": {Type: storagepb.FieldValueType_FIELD_VALUE_TYPE_JSON, JSON: &jsonValue}}}}
