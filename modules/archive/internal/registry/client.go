...
func BuildArchiveFile(deviceID string, key domain.PartitionKey, manifest domain.Manifest, sealed bool, cosState domain.COSState) *storagepb.ArchiveFile {
	...
	return &storagepb.ArchiveFile{SpaceId: key.SpaceID, ArchiveFileId: StableArchiveFileID(key), DatasetId: key.DatasetID, DeviceId: deviceID, PartitionKey: fmt.Sprintf("%s/%s/%s", key.Freq, domain.EncodeIdentity(key.SubjectID), key.Month), FileUri: fileURL, FileFormat: "parquet", MinTime: manifest.MinTime.UTC().Format(time.RFC3339Nano), MaxTime: manifest.MaxTime.UTC().Format(time.RFC3339Nano), RowCount: manifest.RowCount, ContentHash: manifest.SHA256, Columns: append([]string(nil), manifest.Columns...), Status: status, CreatedAt: time.Now().UTC().Format(time.RFC3339Nano), UpdatedAt: time.Now().UTC().Format(time.RFC3339Nano), Attributes: map[string]string{"manifest": string(raw)}}
}
