	...
	raw, err := yaml.Marshal(map[string]any{
		"archive": map[string]any{
			...
			"sources": map[string]any{
				"crypto": map[string]any{"datasets": []string{"dataset_spot_kline_1h"}},
			},
		},
	})
	require.NoError(t, err)
...
func TestRunStatusReportsJournalState(t *testing.T) {
	...
	_, err = store.Append(context.Background(), domain.EventBatch{
		MessageID: "m1",
		Rows:      []domain.RowPatch{{Partition: domain.PartitionKey{SpaceID: "crypto", DatasetID: "dataset_spot_kline_1h", SubjectID: "BTC", Freq: "1h", Month: "202601"}}},
	})
	require.NoError(t, err)
...
func TestRunBackfillRequiresConfirm(t *testing.T) {
	...
	err := run(context.Background(), []string{
		...
		"--space", "crypto", "--dataset", "dataset_spot_kline_1h", "--subject", "BTC-USDT",
...
func TestRunCompactMaterializesDirtyRows(t *testing.T) {
	...
	_, err = store.Append(context.Background(), domain.EventBatch{
		...
		Rows: []domain.RowPatch{{
			Partition: domain.PartitionKey{SpaceID: "crypto", DatasetID: "dataset_spot_kline_1h", SubjectID: "BTC-USDT", Freq: "1h", SeriesTag: "venue:binance", Month: "202601"},
...
func TestRunCompactSeriesTagPresenceSelectsExactIncludingEmpty(t *testing.T) {
	...
	for i, tag := range []string{"", "venue:binance"} {
		_, err = store.Append(t.Context(), domain.EventBatch{
			...
			Rows: []domain.RowPatch{{
				Partition: domain.PartitionKey{SpaceID: "crypto", DatasetID: "dataset_spot_kline_1h", SubjectID: "BTC-USDT", Freq: "1h", SeriesTag: tag, Month: "202601"},
...
func TestRunVerifyChecksParquetFiles(t *testing.T) {
	cfgPath, paths := writeTestConfig(t)
	key := domain.PartitionKey{SpaceID: "crypto", DatasetID: "dataset_spot_kline_1h", SubjectID: "BTC-USDT", Freq: "1h", SeriesTag: "venue:binance", Month: "202601"}
