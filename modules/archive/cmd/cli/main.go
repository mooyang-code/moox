...
func parseArgs(args []string) (cliConfig, error) {
	...
	fs.Var(optionalStringFlag{target: &cfg.SeriesTag}, "series-tag", "exact series tag; omit to match all tags")
...
func run(ctx context.Context, args []string, out io.Writer) error {
	...
	case "sync-cos":
		return runSyncCOS(ctx, cfg, appCfg, out)
	case "status":
...
func runSyncCOS(ctx context.Context, cli cliConfig, cfg *config.Config, out io.Writer) error {
	...
	if err := (cosstore.Syncer{
		...
		Root:     cfg.Archive.RootDir, Prefix: cfg.Archive.COS.Prefix, Workers: cfg.Archive.COS.Workers,
		SyncOpenPartitions: cfg.Archive.COS.SyncOpenPartitions, SeriesTag: cli.SeriesTag,
...
func runBackfill(ctx context.Context, cli cliConfig, cfg *config.Config, out io.Writer) error {
	...
	plan := backfill.Plan{SpaceID: cli.Space, DatasetID: cli.Dataset, SubjectID: cli.Subject, Freq: cli.Freq, SeriesTag: cli.SeriesTag, Start: cli.Start, End: cli.End, Confirm: cli.Confirm}
	if !cli.Confirm {
...
func runCompact(ctx context.Context, cli cliConfig, cfg *config.Config, out io.Writer) error {
	...
	defer store.Close()
	if err := w.WriteDirtyMatching(ctx, 100000, cli.SeriesTag); err != nil {
		return err
	}
	return json.NewEncoder(out).Encode(map[string]any{"ok": true})
}
...
func runVerify(ctx context.Context, cli cliConfig, cfg *config.Config, out io.Writer) error {
	...
	err := filepath.WalkDir(cfg.Archive.RootDir, func(path string, entry os.DirEntry, walkErr error) error {
		...
		if cli.Space != "" && cli.Space != key.SpaceID || cli.Dataset != "" && cli.Dataset != key.DatasetID || cli.Subject != "" && cli.Subject != key.SubjectID || cli.Freq != "" && cli.Freq != key.Freq || cli.SeriesTag != nil && *cli.SeriesTag != key.SeriesTag || cli.Month != "" && cli.Month != key.Month {
			return nil
		}
		...
		if _, err := parquetio.Validate(path, key, generation); err != nil {
			return err
		}
		checked++
		return nil
...
type optionalStringFlag struct{ target **string }
