...
func TestMarketPipelineE2E_SourceFirstFallbackAndIdempotentRetry(t *testing.T) {
	...
	pipe := pipeline.KlinePipeline{Provider: primary, Gate: providers.StaticGate{Permit: providers.RequestPermit{Allowed: true}}, Store: store, SourceDatasetID: "primary_kline", UnifiedDatasetID: "spot_kline", Resolver: pipeline.QualityResolver{Policy: pipeline.QualityPolicy{ProviderPriority: []marketdata.ProviderID{"primary", "fallback"}, AuthoritativeSingleSource: true}, Now: func() time.Time { return fixedE2ETime }}}
	...
	if got := store.unified[len(store.unified)-1]; got.ProviderID != "primary" || got.Close.String() != "10" {
...
func (s *e2EStore) Candidates(context.Context, string, marketdata.Frequency, time.Time) ([]marketdata.ProviderKline, error) {
	return append([]marketdata.ProviderKline(nil), s.source...), nil
}
func (s *e2EStore) Unified(_ context.Context, _ string, _ marketdata.Frequency, _ time.Time) (*marketdata.ResolvedKline, error) {
	if len(s.unified) == 0 {
		return nil, nil
	}
