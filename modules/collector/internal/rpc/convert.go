...
func toPBRule(rule domain.TaskRule) *pb.TaskRule {
	...
	return &pb.TaskRule{
		...
		Feed:            rule.Feed,
		InstrumentTypes: stringsFromJSONString(rule.InstrumentTypes),
		Frequencies:     stringsFromJSONString(rule.Frequencies),
		HistoryStart:    rule.HistoryStart,
		HistoryEnd:      rule.HistoryEnd,
		SubjectFilters:  stringsFromJSONString(rule.SubjectFilters),
		ExchangeFilters: stringsFromJSONString(rule.ExchangeFilters),
		ScheduleSpec:    structFromJSONString(rule.ScheduleSpec),
	}
}
...
func fromPBRule(rule *pb.TaskRule) domain.TaskRule {
	...
	return domain.TaskRule{
		...
		MarketID:        rule.GetMarketId(),
		Feed:            rule.GetFeed(),
		InstrumentTypes: jsonStringFromStrings(rule.GetInstrumentTypes()),
		Frequencies:     jsonStringFromStrings(rule.GetFrequencies()),
		HistoryStart:    rule.GetHistoryStart(),
		HistoryEnd:      rule.GetHistoryEnd(),
		SubjectFilters:  jsonStringFromStrings(rule.GetSubjectFilters()),
		ExchangeFilters: jsonStringFromStrings(rule.GetExchangeFilters()),
		ScheduleSpec:    jsonStringFromStruct(rule.GetScheduleSpec()),
	}
}
