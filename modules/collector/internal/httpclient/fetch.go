...
func sendSingleRequest(ctx context.Context, url string, httpClient *http.Client, respData *[]byte) error {
	body := []byte("{}")
	// 创建 HTTP 请求
	req, err := runtimeapp.NewSignedRequestWithContext(ctx, "POST", url, body, runtimeapp.DefaultAuthConfig())
	if err != nil {
...
func getScheduledDomains() []string {
	...
	return runtimeapp.LocalAppConfig.DNSProxy.ScheduledDomains
}
