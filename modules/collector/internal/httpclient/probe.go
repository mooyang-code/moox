...
func getProbeConfig(domain string) *runtimeapp.ProbeConfig {
	// 确保本地配置已初始化
	if runtimeapp.LocalAppConfig == nil {
		runtimeapp.InitLocalAppConfig()
	}

	// 如果没有 DNSProxy 配置，返回 nil（使用默认 TCP 探测）
	if runtimeapp.LocalAppConfig.DNSProxy == nil {
		return nil
	}

	// 查找匹配的域名配置
	for i := range runtimeapp.LocalAppConfig.DNSProxy.ProbeConfigs {
		cfg := &runtimeapp.LocalAppConfig.DNSProxy.ProbeConfigs[i]
...
// probeIP 统一探测入口
func probeIP(ctx context.Context, ip, domain string, cfg *runtimeapp.ProbeConfig) (latency int64, available bool) {
...
func probeHTTPS(ctx context.Context, ip, domain string, cfg *runtimeapp.ProbeAPIConfig) (latency int64, available bool) {
	// 默认值
	...
	// 设置 User-Agent
	req.Header.Set("User-Agent", "moox-collector-probe/1.0")
