...
func executeMarketKlineJobItem(ctx context.Context, item nodeRuntime.JobItem) (nodeRuntime.Result, error) {
	...
	summary, err := pipe.Run(ctx, buildFetchKlinesRequest(params, product, instrument, frequency, subjectID, providerSymbol, start, end))
	result := nodeRuntime.Result{Summary: map[string]any{"market_id": stringValue(params, "market_id"), "provider_id": string(providerID), "fetched_rows": summary.FetchedRows, "source_rows": summary.SourceRows, "unified_rows": summary.UnifiedRows, "request_count": summary.RequestCount, "complete": summary.Complete, "next_cursor": summary.NextCursor}}
	if err != nil {
...
func buildFetchKlinesRequest(params map[string]any, product marketdata.ProductType, instrument marketdata.InstrumentType, frequency marketdata.Frequency, subjectID, providerSymbol string, start, end time.Time) providers.FetchKlinesRequest {
	return providers.FetchKlinesRequest{MarketID: marketdata.MarketID(stringValue(params, "market_id")), ExchangeID: marketdata.ExchangeID(stringValue(params, "exchange_id")), ProductType: product, InstrumentType: instrument, Frequency: frequency, Subjects: []providers.ProviderSubject{{SubjectID: subjectID, ProviderSymbol: providerSymbol}}, StartTime: start, EndTime: end, Limit: intValue(params, "limit"), Cursor: stringValue(params, "cursor")}
}
