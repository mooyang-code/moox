...
type Bootstrap struct {
	config    *runtimeapp.AppConfig
	services  *Services
	startTime time.Time // 启动时间
}

// New 创建新的启动器
func New(cfg *runtimeapp.AppConfig) *Bootstrap {
	if cfg == nil {
...
func (b *Bootstrap) Initialize(ctx context.Context) error {
	...
	// 1. 加载配置
	cfg, err := runtimeapp.LoadConfigs(b.config)
	if err != nil {
...
// GetConfig 获取配置实例
func (b *Bootstrap) GetConfig() *runtimeapp.AppConfig {
	return b.config
}
