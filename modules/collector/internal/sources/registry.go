...
type CollectorDescriptor struct {
	...
	DataType    string    // 数据类型，如 "kline"、"symbol"
...
func (r *CollectorRegistry) Register(descriptor *CollectorDescriptor) error {
	...
	key := r.generateKey(descriptor.Source, descriptor.Market, descriptor.DataType)
...
// Get 获取采集器
func (r *CollectorRegistry) Get(source, market, dataType string) (Collector, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *CollectorRegistry) generateKey(source, market, dataType string) string {
	...
	return fmt.Sprintf("%s:%s:%s", source, market, dataType)
}
...
// Market 设置市场
func (b *CollectorBuilder) Market(market, marketCN string) *CollectorBuilder {
	b.descriptor.Market = market
	b.descriptor.MarketCN = marketCN
	return b
}
...
func (b *CollectorBuilder) Register() error {
	if b.descriptor.Market == "" {
		b.descriptor.Market = "spot"
	}
