...
func testAggregator(t *testing.T) (*Aggregator, *gorm.DB) {
	t.Helper()
	...
	return &Aggregator{Store: store.New(db)}, db
}

func TestWritePointIsIdempotentAndSourceScoped(t *testing.T) {
	a, db := testAggregator(t)
	...
	db.Table("t_strategy_performance_points").Where("c_binding_id=? AND c_performance_source=?", "b1", "paper").Count(&points)
	db.Table("t_strategy_performance_daily").Where("c_binding_id=? AND c_performance_source=?", "b1", "paper").Count(&daily)
	if points != 1 || daily != 1 {
		t.Fatalf("points=%d daily=%d", points, daily)
	}
	var nav string
	db.Table("t_strategy_performance_points").Select("c_nav").Where("c_binding_id=?", "b1").Scan(&nav)
	if nav != "1.02" {
		t.Fatalf("nav=%q", nav)
	}
}
