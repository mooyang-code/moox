...
EVENTBUS_CREDENTIAL_FILE="${MOOX_FACTOR_EVENTBUS_CREDENTIAL_FILE:-${HOME}/.config/moox/eventbus/factor-merge-eventbus.yaml}"
...
require_file "${ROOT}/bin/moox-factor-merge"
...
[[ -n "${MOOX_EVENTBUS_NATS_URL}" ]] || {
  echo "factor-merge EventBus URL is missing from ${EVENTBUS_CREDENTIAL_FILE}" >&2
  exit 1
}
if [[ "${MOOX_FACTOR_MERGE_ALLOW_LOCAL_EVENTBUS:-}" != "1" ]]; then
  case "${MOOX_EVENTBUS_NATS_URL}" in
    tls://127.0.0.1:*|tls://localhost:*|tls://[::1]:*|nats://127.0.0.1:*|nats://localhost:*)
      echo "factor-merge must use the public tls:// EventBus URL from factor-merge-eventbus.yaml" >&2
      exit 1
      ;;
  esac
  [[ "${MOOX_EVENTBUS_NATS_URL}" == tls://* ]] || {
    echo "factor-merge EventBus URL must be tls://" >&2
    exit 1
  }
fi
...
mkdir -p "${ROOT}/data/factor-merge" "${ROOT}/logs/factor-merge" "${ROOT}/run"
...
for _ in $(seq 1 20); do
  ...
  leftover="$(pgrep -f "${ROOT}/bin/moox-factor-merge" || true)"
  if [[ -n "${leftover}" ]]; then
...
LOG_FILE="${ROOT}/logs/factor-merge/stdout.log"

export MOOX_FACTOR_MERGE_DB_PATH="${MOOX_FACTOR_MERGE_DB_PATH:-${ROOT}/data/factor-merge/merge.db}"
...
export MOOX_FACTOR_STORAGE_RPC_KEY_ID="${MOOX_FACTOR_STORAGE_RPC_KEY_ID:-merge}"
export MOOX_FACTOR_STORAGE_RPC_HMAC_KEY_FILE="${SECRETS_DIR}/gateway-merge.key"
...
with open(log_path, "ab") as log:
    proc = subprocess.Popen(
        [os.path.join(root, "bin/moox-factor-merge"), "-config=config/merge-app.yaml", "-conf=config/merge-trpc.yaml"],
...
if ! ps -p "${pid}" >/dev/null 2>&1; then
  echo "factor-merge failed to start; see ${LOG_FILE}" >&2
  tail -80 "${LOG_FILE}" >&2 || true
  exit 1
fi
echo "factor-merge started pid=${pid}"
