...
MOOX_EVENTBUS_ENABLE_TLS=1 MOOX_STORAGE_VIEW_DUCKDB_MEMORY_LIMIT=256MB "${ROOT}/scripts/deploy/deploy-moox.sh" \
  --target localhost \
  --dir "${DEPLOY_DIR}" \
  --stage "${STAGE_DIR}" \
  --skip-build \
...
MOOX_EVENTBUS_ENABLE_TLS=1 MOOX_STORAGE_VIEW_DUCKDB_MEMORY_LIMIT=256MB "${ROOT}/scripts/deploy/deploy-moox.sh" --target localhost --dir "${DEPLOY_DIR}" --stage "${STAGE_DIR}" --skip-build --with-storage-node --no-start --no-web-host --no-cloudnode --no-collector --no-factor --no-strategy --no-trade --no-monitor "${GATEWAY_ARGS[@]}" >/dev/null
...
assert_grep 'source "\$\{ROOT\}/config/runtime.env"' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'sign_health_request' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'check_eventbus_tls_material' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'storage-eventbus.yaml' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'metrics-publisher.yaml' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'server-key.pem' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'internal-admin.yaml' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'MOOX_EVENTBUS_NATS_TLS_CA_FILE' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'openssl x509' "${DEPLOY_DIR}/healthcheck.sh"
assert_grep 'openssl pkey' "${DEPLOY_DIR}/healthcheck.sh"
for mapping in \
...
assert_grep 'maintenance.lock' "${ROOT}/scripts/runtime/moox-storage-view-watchdog.sh"
assert_grep 'start\.sh" storage-view 8>&- 9>&-' "${ROOT}/scripts/runtime/moox-storage-view-watchdog.sh"
...
assert_grep 'MOOX_STORAGE_VIEW_DUCKDB_MEMORY_LIMIT=\$\{quoted_storage_view_duckdb_memory_limit\}' "${ROOT}/scripts/deploy/deploy-moox.sh"
...
if MOOX_CONTROL_ROOT="${CONTROL_AUTH_ROOT}" \
  ...
  "${ROOT}/scripts/deploy/deploy-moox.sh" --target localhost --dir "${GUARD_DEPLOY_DIR}" --stage "${STAGE_DIR}" \
