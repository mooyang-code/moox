...
Environment:
  STORAGE_CGO_ENABLED   Passed through to scripts/build.sh
...
cp "${ROOT}/scripts/publish-release-binaries.sh" "${artifact_dir}/deploy/"
...
\`\`\`bash
./scripts/publish-release-binaries.sh \\
...
if ((PUBLISH_LATEST)); then
  ...
  cp "${ROOT}/scripts/publish-release-binaries.sh" "${OUTPUT_ROOT}/deploy/"
