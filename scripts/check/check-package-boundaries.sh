...
go test scripts/check/check-event-topology-declarations.go scripts/check/check-event-topology-declarations_test.go >/dev/null
while IFS= read -r match; do
	violations+=("${match}: Registry-owned event topology must not be redeclared in a business module")
done < <(go run scripts/check/check-event-topology-declarations.go modules)
