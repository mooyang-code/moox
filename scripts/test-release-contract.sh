...
grep -q 'deploy/caddy/Caddyfile.public' "${ROOT}/scripts/release.sh"
grep -q 'deploy/caddy/Caddyfile.public.no-admin' "${ROOT}/scripts/release.sh"
grep -q 'caddy-managed.sh" start --deploy-dir "${ROOT}" 9>&-' "${ROOT}/scripts/deploy-moox.sh"
grep -q 'Caddyfile.next" 8>&-' "${ROOT}/scripts/deploy-moox.sh"
grep -q '"${DEPLOY_DIR}/start.sh" 8>&-' "${ROOT}/scripts/deploy-moox.sh"
