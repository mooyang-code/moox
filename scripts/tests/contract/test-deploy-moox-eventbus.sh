...
grep -q 'trade_eventbus_preflight' "${ROOT}/scripts/deploy-moox.sh"
grep -q 'eventbus-check' "${ROOT}/scripts/deploy-moox.sh"
...
grep -Fq 'systemctl --user is-enabled moox-eventbus.service' "${ROOT}/scripts/deploy-moox.sh"
grep -Fq 'systemctl --user disable moox-eventbus.service' "${ROOT}/scripts/deploy-moox.sh"
