...
"${ROOT}/scripts/moox-log-rotate.sh" --root "${TMP_ROOT}" --max-size-mb 1 --backup-count 2
...
if "${ROOT}/scripts/moox-log-rotate.sh" --root "${TMP_ROOT}" --max-size-mb 0 --backup-count 2 2>/dev/null; then
