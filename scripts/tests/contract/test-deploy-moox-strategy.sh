...
trade_start_line=$(grep -n '^      start_trade$' "${ROOT}/scripts/deploy-moox.sh" | head -1 | cut -d: -f1)
strategy_start_line=$(grep -n '^      start_strategy$' "${ROOT}/scripts/deploy-moox.sh" | head -1 | cut -d: -f1)
[[ -n "${trade_start_line}" && -n "${strategy_start_line}" && ${trade_start_line} -lt ${strategy_start_line} ]]
grep -q 'wait_http "http://127.0.0.1:11210/readyz"' "${ROOT}/scripts/deploy-moox.sh"
