...
export function buildSpotLiquidationPlan(
  account: Account,
  balances: Balance[],
  ...
  balances.forEach(balance => {
    ...
    if (instruments.length > 0 && !instrument) {
      const dustTarget = buildSpotDustTarget(
        account,
        balance,
        currency,
        quantity,
        `未找到 ${candidateSymbol} 现货交易对，尝试转为 ${DUST_TARGET_CURRENCY}`
      );
    ...
    if (instrument && !isTradableInstrument(instrument)) {
      const dustTarget = buildSpotDustTarget(
        ...
        currency,
        quantity,
        `交易对不可交易，尝试转为 ${DUST_TARGET_CURRENCY}`
      );
      if (dustTarget) targets.push(dustTarget);
      else hiddenCount += 1;
      return;
    ...
    if (!normalized.tradeable) {
      const dustTarget = buildSpotDustTarget(
        ...
        currency,
        quantity,
        `低于交易所最小下单量，尝试转为 ${DUST_TARGET_CURRENCY}`
      );
      if (dustTarget) targets.push(dustTarget);
      else hiddenCount += 1;
      return;
    ...
    if (!meetsMinNotional(normalized.quantity, instrument)) {
      const dustTarget = buildSpotDustTarget(
        ...
        currency,
        quantity,
        `低于 ${quoteCurrency} 最小成交额，尝试转为 ${DUST_TARGET_CURRENCY}`
      );
      if (dustTarget) targets.push(dustTarget);
      else hiddenCount += 1;
      return;
...
export function buildSpotLiquidationTargets(
  account: Account,
  balances: Balance[],
  instruments: Instrument[] = []
): LiquidationTarget[] {
