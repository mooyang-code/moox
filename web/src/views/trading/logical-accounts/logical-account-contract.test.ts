...
describe("策略账户子视图契约", () => {
  it("使用策略账户和实际交易账户中文概念并支持内嵌布局", () => {
    ...
    expect(source).toContain("新建策略账户");
    expect(source).toContain("策略账户详情");
    expect(source).toContain("实际交易账户");
    expect(source).toContain("多个实际账户按优先级执行，当前账户容量不足时自动切换下一账户。");
    expect(source).not.toContain("<h2>逻辑账户</h2>");
    expect(source).not.toContain("物理交易账户");
  });

  it("保留策略账户执行控制和成员操作 API", () => {
    for (const call of [
      "pauseLogicalAccount",
