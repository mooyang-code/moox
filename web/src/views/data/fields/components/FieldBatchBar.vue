<template>
  <div v-if="selectedCount" class="batch-bar">
    <span
      >已选择 <strong>{{ selectedCount }}</strong> 个字段</span
    >
    <a-divider direction="vertical" />
    <a-button size="small" :disabled="loading" @click="moveVisible = true"
      ><template #icon><icon-folder /></template>移动字段组</a-button
    >
    <a-button size="small" :loading="loading" @click="$emit('apply', { target_status: 'active' })"
      ><template #icon><icon-check-circle /></template>启用</a-button
    >
    <a-popconfirm content="确认停用所选字段？" @ok="$emit('apply', { target_status: 'disabled' })">
      <a-button size="small" status="warning" :disabled="loading"
        ><template #icon><icon-minus-circle /></template>停用</a-button
      >
    </a-popconfirm>
