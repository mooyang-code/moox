  ...
  >
    <a-form ref="formRef" :model="form" layout="vertical">
      ...
      <div class="form-grid">
        ...
        <a-form-item class="span-2" field="description" label="描述"><a-textarea v-model="form.description" :auto-size="{ minRows: 2, maxRows: 4 }" /></a-form-item>
      </div>

      <a-divider orientation="left">数据定义</a-divider>
      <div class="form-grid">
        <a-form-item field="value_type" label="值类型" required>
          <a-select v-model="form.value_type"><a-option v-for="item in fieldValueTypeOptions" :key="item.value" :value="item.value">{{ item.label }}</a-option></a-select>
        </a-form-item>
        <a-form-item field="unit" label="单位"><a-input v-model="form.unit" placeholder="例如 CNY、percent" /></a-form-item>
        <a-form-item class="span-2" field="validation_rule_json" label="校验规则 JSON" :validate-status="jsonError ? 'error' : undefined" :help="jsonError">
          <a-textarea v-model="form.validation_rule_json" class="code-input" :auto-size="{ minRows: 3, maxRows: 7 }" @blur="validateJSON" />
        </a-form-item>
        <a-form-item class="span-2" field="write_example" label="写入示例"><a-textarea v-model="form.write_example" :auto-size="{ minRows: 2, maxRows: 5 }" /></a-form-item>
      </div>
...
watch(() => props.visible, (visible) => {
  ...
  Object.assign(form, emptyField(), props.field ? { ...props.field, validation_rule_json: jsonText(props.field.validation_rule_json) } : {});
  jsonError.value = '';
