<template>
  <a-modal :visible="visible" width="520px" :title="editing ? '编辑字段组' : '新增字段组'" :ok-loading="saving" @cancel="$emit('close')" @ok="submit">
    <a-form :model="form" layout="vertical">
      <a-form-item field="parent_group_id" label="父字段组">
        <a-select v-model="form.parent_group_id" allow-clear placeholder="留空表示一级字段组">
          <a-option v-for="root in roots" :key="root.group_id" :value="root.group_id" :disabled="root.group_id === form.group_id">{{ root.name }}</a-option>
        </a-select>
      </a-form-item>
      <div class="form-grid">
        <a-form-item field="group_id" label="字段组 ID" required><a-input v-model="form.group_id" :disabled="editing" placeholder="例如 market_quote" /></a-form-item>
        ...
        <a-form-item field="status" label="启用状态"><a-switch v-model="form.status" checked-value="active" unchecked-value="disabled" /></a-form-item>
      </div>
    </a-form>
  </a-modal>
</template>
...
function emptyGroup(): FieldGroup {
  return { space_id: props.spaceID, group_id: '', name: '', description: '', parent_group_id: props.parentID, sort_order: 0, status: 'active' };
}
