  ...
  >
    <template #columns>
      ...
      <a-table-column title="字段组" data-index="group_id" :width="190">
        <template #cell="{ record }">
          <a-tooltip :content="groupPath(groups, record.group_id)"
            ><span class="ellipsis">{{ groupPath(groups, record.group_id) || "-" }}</span></a-tooltip
          >
        </template>
      </a-table-column>
      <a-table-column title="单位" data-index="unit" :width="100"
        ><template #cell="{ record }">{{ record.unit || "-" }}</template></a-table-column
      >
      ...
      <a-table-column title="操作" :width="70" align="center" fixed="right">
        <template #cell="{ record }">
          <a-tooltip content="编辑字段"
            ><a-button size="mini" type="text" @click.stop="$emit('edit', record)"><icon-edit /></a-button
          ></a-tooltip>
        </template>
      </a-table-column>
    </template>
