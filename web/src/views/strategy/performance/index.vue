<template>
  <a-card title="策略表现" :bordered="false">
    <div class="performance-toolbar">
      <a-radio-group v-model="source" type="button" @change="load"
        ><a-radio v-for="item in sourceOptions" :key="item" :value="item">{{ item }}</a-radio></a-radio-group
      >
      <span class="as-of">数据时间：{{ performance?.as_of || "-" }}</span>
    </div>
    <a-alert v-if="performance?.summary?.status === 'insufficient_data'" type="warning" show-icon
      >数据不足，暂不计算绩效。</a-alert
    >
