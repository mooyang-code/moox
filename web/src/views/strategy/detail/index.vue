<template>
  <div class="strategy-page">
    <div class="moox-inner">
      <div class="page-head">
        <div><h2>{{ overview?.definition?.strategy_id || '策略详情' }}</h2><span>{{ overview?.definition?.version || '' }} · {{ overview?.binding?.binding_id || bindingId }}</span></div>
        <a-space><a-button @click="goBack">返回</a-button><a-button :loading="store.loading" @click="refresh">刷新</a-button></a-space>
      </div>
      <a-alert v-if="store.error" type="error" show-icon class="top-alert">{{ store.error }}</a-alert>
      <a-grid :cols="2" :col-gap="12" :row-gap="12">
        <a-grid-item><a-card title="策略信息" :bordered="false"><a-descriptions :column="2" size="small"><a-descriptions-item label="策略 ID">{{ overview?.definition?.strategy_id || '-' }}</a-descriptions-item><a-descriptions-item label="版本">{{ overview?.definition?.version || '-' }}</a-descriptions-item><a-descriptions-item label="源码 Hash">{{ overview?.definition?.source_hash || '-' }}</a-descriptions-item><a-descriptions-item label="Binding">{{ overview?.binding?.binding_id || '-' }}</a-descriptions-item><a-descriptions-item label="Space">{{ overview?.binding?.space_id || '-' }}</a-descriptions-item><a-descriptions-item label="频率">{{ overview?.binding?.freq || '-' }}</a-descriptions-item></a-descriptions></a-card></a-grid-item>
        <a-grid-item><HealthPanel :health="overview?.health" /></a-grid-item>
      </a-grid>
      <div class="operation-panel"><OperationPanel :binding-id="bindingId" :status="overview?.binding?.status" :current-mode="overview?.health?.mode" @changed="refresh" /></div>
...
function goBack() { router.push({ name: 'strategy-running' }); }
onMounted(() => { refresh(); store.startPolling(refresh, 5000); });
