<template>
  <div class="master-detail">
    <aside class="host-rail" aria-label="监控主机列表">
      ...
      >
        <div><strong>{{ row.displayName }}</strong><span>{{ row.displayAddress }}</span></div>
        <span class="rail-state" :class="row.state">{{ stateText(row) }}</span>
        <dl><div><dt>CPU</dt><dd>{{ percent(row.cpuUsage) }}</dd></div><div><dt>内存</dt><dd>{{ percent(row.memoryUsage) }}</dd></div><div><dt>文件系统</dt><dd>{{ percent(row.filesystemUsage) }}</dd></div></dl>
      </button>
    </aside>
