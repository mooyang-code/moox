    ...
    <div class="moox-inner">
      ...
      >
        <div class="terminal-modal-body">
          <SshTerminal v-if="terminalHostId" :initial-host-id="terminalHostId" disconnect-on-unmount />
        </div>
      </a-modal>
...
const monitorByHostId = computed(() =>
  Object.fromEntries(rows.value.filter(row => row.ssh?.id !== undefined && row.monitor).map(row => [row.ssh!.id, row.monitor]))
);
...
.host-workbench-page > .moox-inner {
  min-height: 100%;
}
.workbench-content {
  min-width: 0;
...
.workbench-content :deep(.moox-page) {
  height: auto;
  padding: 0;
  overflow: visible;
  background: transparent;
}
.workbench-content :deep(.moox-page > .moox-inner) {
  min-height: 0;
  padding: 0;
  border: 0;
  border-radius: 0;
...
.terminal-modal-body :deep(.ssh-terminal-page) {
  height: 100%;
}
