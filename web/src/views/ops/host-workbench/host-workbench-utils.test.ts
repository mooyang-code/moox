...
describe('mergeHostWorkbenchRows', () => {
  it('matches monitor and ssh rows by exact address before unique name', () => {
    const rows = mergeHostWorkbenchRows(
      [{ host_id: 'agent-1', host_name: 'prod-a', address: '10.0.0.1', status: 'online', timestamp: '', cpu: {} as any, memory: {} as any, filesystems: [], disks: [], networks: [], storage_available: true }],
  ...
  it('does not fuzzy match ambiguous names', () => {
    const rows = mergeHostWorkbenchRows(
      [{ host_id: 'agent-1', host_name: 'prod', address: 'agent-1', status: 'offline', timestamp: '', cpu: {} as any, memory: {} as any, filesystems: [], disks: [], networks: [], storage_available: true }],
      [{ id: 1, name: 'prod', address: '10.0.0.1' }, { id: 2, name: 'prod', address: '10.0.0.2' }] as any,
    );
