...
function monitor(overrides: Partial<HostMetrics> = {}): HostMetrics {
  return {
    ...
    filesystems: [{ device: '/dev/vda1', mountpoint: '/', fs_type: 'ext4', total: 100, used: 67, available: 33, percent: 67, percent_available: true, read_only: false }],
    disks: [],
    networks: [{ device: 'eth0', operstate: 'up', rx_speed: 100, tx_speed: 40, receive_errors_total: 0, transmit_errors_total: 0, rate_available: true }],
...
describe('buildHostMonitorRows', () => {
  it('matches a monitor agent to an SSH host by exact unique name', () => {
    const rows = buildHostMonitorRows([monitor({ host_name: '腾讯云-122' })], [ssh({ name: '腾讯云-122', address: '106.53.107.122' })]);
  ...
  it('derives summaries, attention, and aggregate network rates', () => {
    const rows = buildHostMonitorRows([
      monitor({
        ...
        filesystems: [
          { device: '/dev/a', mountpoint: '/', fs_type: 'ext4', total: 100, used: 30, available: 70, percent: 30, percent_available: true, read_only: false },
          { device: '/dev/b', mountpoint: '/data', fs_type: 'ext4', total: 100, used: 78, available: 22, percent: 78, percent_available: true, read_only: false },
        ],
        networks: [
          { device: 'eth0', operstate: 'up', rx_speed: 100, tx_speed: 40, receive_errors_total: 0, transmit_errors_total: 0, rate_available: true },
          { device: 'eth1', operstate: 'up', rx_speed: 20, tx_speed: 10, receive_errors_total: 0, transmit_errors_total: 0, rate_available: true },
        ],
      }),
    ], []);
