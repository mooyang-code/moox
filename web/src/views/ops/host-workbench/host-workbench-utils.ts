...
export function mergeHostWorkbenchRows(monitors: HostMetrics[], sshHosts: SSHHost[], sessions: SessionInfo[] = []): HostWorkbenchRow[] {
  ...
  for (const monitor of monitors) {
    ...
    if (!ssh) {
      const sameName = sshHosts.filter((host) => host.id !== undefined && !usedSSH.has(host.id) && normalize(host.name) === normalize(monitor.host_name));
    ...
    rows.push({ key: `monitor:${monitor.host_id}`, monitor, ssh, sessions: ssh?.id !== undefined ? sessionsByHost.get(ssh.id) || [] : [], match });
  }

  for (const host of sshHosts) {
    if (host.id !== undefined && usedSSH.has(host.id)) continue;
    rows.push({ key: `ssh:${host.id}`, ssh: host, sessions: host.id !== undefined ? sessionsByHost.get(host.id) || [] : [], match: 'unmatched' });
  }
  return rows;
}
