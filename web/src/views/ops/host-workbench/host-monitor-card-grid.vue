  ...
  <section class="monitor-card-grid" aria-label="主机监控卡片">
    ...
    >
      ...
      <div class="metric-grid">
        <div>
          <span>CPU</span><strong>{{ percent(row.cpuUsage) }}</strong>
        </div>
        <div>
          <span>内存</span><strong>{{ percent(row.memoryUsage) }}</strong>
        </div>
        <div>
          <span>文件系统</span><strong>{{ percent(row.filesystemUsage) }}</strong>
        </div>
      </div>
      <div class="progress-list">
        ...
        <a-progress
          :percent="ratio(row.filesystemUsage)"
          :show-text="false"
          size="small"
          :color="progressColor(row.filesystemUsage)"
        />
      </div>
...
const progressColor = (value: number | null) =>
  value !== null && value >= 80 ? "#dc2626" : value !== null && value >= 60 ? "#d97706" : "#16a34a";
