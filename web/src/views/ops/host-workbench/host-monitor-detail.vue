<template>
  <section v-if="row" class="monitor-detail" aria-label="主机监控详情">
    ...
    <template v-else>
      <a-alert v-if="row.state !== 'online'" type="warning" show-icon class="detail-alert"
        >主机当前离线，设备明细来自最后一次成功上报。</a-alert
      >
      ...
      <div class="detail-overview">
        ...
        <section class="device-section">
          ...
          <dl>
            <div>
              <dt>CPU 核心</dt>
              <dd>{{ row.monitor.cpu.cores || "--" }}</dd>
            </div>
            <div>
              <dt>内存容量</dt>
              <dd>{{ formatBytes(row.monitor.memory.total) }}</dd>
            </div>
            <div>
              <dt>文件系统</dt>
              <dd>{{ row.monitor.filesystems.length }}</dd>
            </div>
            <div>
              <dt>磁盘 / 网卡</dt>
              <dd>{{ row.monitor.disks.length }} / {{ row.monitor.networks.length }}</dd>
            </div>
          </dl>
        </section>
      </div>
            ...
            <table class="detail-table filesystem-table">
              ...
              <tbody>
                <tr v-for="item in row.monitor.filesystems" :key="`${item.device}:${item.mountpoint}`">
                  ...
                  <td :title="item.fs_type">{{ item.fs_type || "--" }}</td>
                  <td :title="item.percent_available ? `${item.percent.toFixed(1)}%` : '--'">
                    {{ item.percent_available ? `${item.percent.toFixed(1)}%` : "--" }}
                  </td>
                  <td :title="formatBytes(item.total)">{{ formatBytes(item.total) }}</td>
                </tr>
                <tr v-if="!row.monitor.filesystems.length">
                  <td colspan="5" class="empty-cell">暂无文件系统数据</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table class="detail-table disk-table">
              ...
              <tbody>
                <tr v-for="item in row.monitor.disks" :key="item.device">
                  <td :title="item.device">{{ item.device || "--" }}</td>
                  <td :title="item.rate_available ? formatBytesPerSecond(item.read_bytes_per_second) : '--'">
                    {{ item.rate_available ? formatBytesPerSecond(item.read_bytes_per_second) : "--" }}
                  </td>
                  <td :title="item.rate_available ? formatBytesPerSecond(item.write_bytes_per_second) : '--'">
                    {{ item.rate_available ? formatBytesPerSecond(item.write_bytes_per_second) : "--" }}
                  </td>
                  <td :title="item.rate_available ? item.read_iops.toFixed(1) : '--'">
                    {{ item.rate_available ? item.read_iops.toFixed(1) : "--" }}
                  </td>
                  <td :title="item.rate_available ? item.write_iops.toFixed(1) : '--'">
                    {{ item.rate_available ? item.write_iops.toFixed(1) : "--" }}
                  </td>
                  <td :title="item.rate_available ? `${item.utilization_percent.toFixed(1)}%` : '--'">
                    {{ item.rate_available ? `${item.utilization_percent.toFixed(1)}%` : "--" }}
                  </td>
                </tr>
                <tr v-if="!row.monitor.disks.length">
                  <td colspan="6" class="empty-cell">暂无磁盘数据</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            <table class="detail-table network-table">
              ...
              <tbody>
                <tr v-for="item in row.monitor.networks" :key="item.device">
                  <td :title="item.device">{{ item.device || "--" }}</td>
                  <td :title="item.operstate">
                    <span class="operstate" :class="item.operstate">{{ item.operstate }}</span>
                  </td>
                  <td :title="item.rate_available ? formatBytesPerSecond(item.rx_speed) : '--'">
                    {{ item.rate_available ? formatBytesPerSecond(item.rx_speed) : "--" }}
                  </td>
                  <td :title="item.rate_available ? formatBytesPerSecond(item.tx_speed) : '--'">
                    {{ item.rate_available ? formatBytesPerSecond(item.tx_speed) : "--" }}
                  </td>
                  <td :title="String(item.receive_errors_total + item.transmit_errors_total)">
                    {{ item.receive_errors_total + item.transmit_errors_total }}
                  </td>
                </tr>
                <tr v-if="!row.monitor.networks.length">
                  <td colspan="5" class="empty-cell">暂无网络数据</td>
                </tr>
              </tbody>
            </table>
          </div>
...
async function loadHistory() {
  ...
  try {
    ...
    historyNotice.value = !response.storage_available
      ? "历史存储暂不可用"
      : response.data_gap
        ? "当前时间范围存在历史数据缺口"
        : "";
