<template>
  <div class="rule-editor">
      ...
      <div class="form-grid">
        ...
        <a-form-item label="状态"
          ><a-switch v-model="draft.enabled"
            ><template #checked>启用</template><template #unchecked>停用</template></a-switch
          ></a-form-item
        >
        <a-form-item label="条件连接"
          ><a-radio-group v-model="draft.connector" type="button"
            ><a-radio :value="LogicalOperator.AND">AND</a-radio><a-radio :value="LogicalOperator.OR">OR</a-radio></a-radio-group
          ></a-form-item
        >
        <a-form-item label="评估间隔（秒）"
          ><a-input-number v-model="draft.evaluation_interval_seconds" :min="1" :max="86400"
        /></a-form-item>
        <a-form-item label="连续触发次数"
          ><a-input-number v-model="draft.consecutive_trigger_count" :min="1" :max="100"
        /></a-form-item>
        <a-form-item label="连续恢复次数"
          ><a-input-number v-model="draft.consecutive_recovery_count" :min="1" :max="100"
        /></a-form-item>
        <a-form-item class="wide" label="通知 Webhook"
          ><a-select v-model="draft.webhook_ids" multiple allow-clear placeholder="选择通知通道"
            ><a-option v-for="webhook in enabledWebhooks" :key="webhook.webhook_id" :value="webhook.webhook_id">{{
              webhook.name || webhook.webhook_id
            }}</a-option></a-select
          ></a-form-item
        >
        <a-form-item class="wide" label="说明"
          ><a-textarea v-model="draft.description" :auto-size="{ minRows: 2, maxRows: 4 }"
        /></a-form-item>
      </div>
    </a-form>
    ...
    <div class="preview-head">
      <strong>保存前预览</strong><a-button size="small" :loading="previewLoading" @click="previewRule">运行预览</a-button>
    </div>
      ...
      <template #columns>
        ...
        <a-table-column title="结果" :width="100"
          ><template #cell="{ record }"
            ><a-tag :color="record.has_data === false ? 'orange' : record.result ? 'red' : 'green'">{{
              record.has_data === false ? "无数据" : record.result ? "触发" : "正常"
            }}</a-tag></template
          ></a-table-column
        >
        <a-table-column title="值 / 阈值" :width="150"
          ><template #cell="{ record }"
            >{{ formatNumber(record.value) }} / {{ formatNumber(record.threshold) }}</template
          ></a-table-column
        >
    ...
    <div v-if="preview" class="preview-result">
      最终结果：<a-tag :color="preview.result ? 'red' : 'green'">{{ preview.result ? "触发" : "正常" }}</a-tag>
    </div>
...
function newCondition(id: string): MetricCondition {
  return {
    condition_id: id,
    query: {
      selector: { service_name: "", metric_name: "", matchers: [] },
      time_reducer: TimeReducer.CURRENT,
      window_seconds: 0,
      series_reducer: SeriesReducer.MAX
    },
    compare: CompareOperator.GT,
    threshold: 0,
    no_data_policy: NoDataPolicy.KEEP_STATE
  };
}
function cloneRule(rule?: MetricRule) {
  return JSON.parse(JSON.stringify(rule || newRule())) as MetricRule;
}
function renumber() {
  draft.value.conditions = draft.value.conditions
    .slice(0, MAX_CONDITIONS)
    .map((condition, index) => ({ ...condition, condition_id: String.fromCharCode(65 + index) }));
}
function addCondition() {
  if (draft.value.conditions.length < MAX_CONDITIONS)
    draft.value.conditions.push(newCondition(String.fromCharCode(65 + draft.value.conditions.length)));
}
...
function patchCondition(index: number, patch: MetricConditionPatch) {
  ...
  draft.value.conditions[index] = {
    ...current,
    ...patch,
    query: { ...current.query, ...query, selector: { ...current.query.selector, ...selector } }
  } as MetricCondition;
  renumber();
}
function metricOptionsFor(service: string) {
  return metricNames.value[service] || [];
}
async function loadServices() {
  try {
    const response = await metricMonitorApi.listMetricServices({ page: { page: 1, size: 200 } });
    serviceOptions.value = [
      ...new Set((response.services || []).map(item => item.service_name).filter(Boolean) as string[])
    ].sort();
...
function validateRule() {
  ...
  if (draft.value.consecutive_trigger_count < 1 || draft.value.consecutive_recovery_count < 1)
    return "连续触发和恢复次数必须为正数";
  if (draft.value.evaluation_interval_seconds < 1) return "评估间隔必须为正数";
  for (const condition of draft.value.conditions) {
    if (!condition.query.selector.service_name || !condition.query.selector.metric_name)
      return `条件 ${condition.condition_id} 的服务和指标不能为空`;
    if (condition.query.time_reducer !== TimeReducer.CURRENT && (condition.query.window_seconds || 0) < 1)
      return `条件 ${condition.condition_id} 的时间窗口必须为正数`;
  }
  return "";
}
async function previewRule() {
  ...
  try {
    preview.value = (await metricMonitorApi.previewMetricRule(draft.value)).evaluation;
  } catch (reason) {
    notifyError(reason, "预览失败");
  } finally {
...
async function saveRule() {
  ...
  try {
    ...
    Message.error(reason instanceof Error ? reason.message : "规则保存失败");
  } finally {
    saving.value = false;
  }
}
...
.rule-editor {
  padding-bottom: var(--moox-space-6);
}
.form-grid {
  display: grid;
