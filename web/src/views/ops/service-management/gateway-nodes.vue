  ...
  <div class="gateway-nodes">
    ...
    >
      <template #columns>
            ...
            <a-tooltip :content="hashTooltip(record)">
              <div class="hash-state">
                ...
                <div class="hash-values">
                  <code>预期 {{ shortHash(record.route_hash) }}</code
                  ><code>应用 {{ shortHash(record.applied_route_hash) }}</code>
                </div>
              </div>
            </a-tooltip>
          </template>
        ...
        <a-table-column title="最近错误" :width="180" ellipsis tooltip>
          <template #cell="{ record }"
            ><span :class="{ 'error-text': record.last_error }">{{ record.last_error || "--" }}</span></template
          >
        </a-table-column>
            ...
            <a-space :size="4">
              <a-tooltip content="查看路由">
                <a-button
                  size="mini"
                  type="text"
                  :aria-label="`查看 ${record.name || record.node_id} 路由`"
                  @click="openRoutes(record)"
                >
    ...
    >
      <div class="routes-meta">
        <span
          >节点：<strong>{{ routeSnapshot.node_id }}</strong></span
        >
...
function resetForm() {
  Object.assign(form, { node_id: "", host_id: 0, name: "", public_address: "", status: "enabled" });
}
function openCreate() {
  editing.value = false;
  editingNodeID.value = "";
...
async function submit() {
  ...
  const result = await runModalSubmission(
    () => {
      const error =
        !form.node_id.trim() || !form.name.trim() || !form.host_id || !form.public_address.trim()
...
function hostLabel(hostID: number) {
  ...
  return host ? `${host.name} (${host.address})` : hostID ? `#${hostID}` : "--";
}
function onlineLabel(lastSeenAt?: string) {
  return gatewayNodeOnlineState(lastSeenAt).label;
}
function onlineColor(lastSeenAt?: string) {
  return { online: "green", offline: "red", never: "gray" }[gatewayNodeOnlineState(lastSeenAt).state];
}
function hashLabel(node: GatewayNode) {
  return gatewayHashState(node.route_hash, node.applied_route_hash).label;
}
function hashColor(node: GatewayNode) {
  return gatewayHashState(node.route_hash, node.applied_route_hash).state === "synced" ? "green" : "orange";
}
function hashTooltip(node: GatewayNode) {
  return `预期：${node.route_hash || "--"}\n已应用：${node.applied_route_hash || "--"}`;
}
...
.routes-meta {
  ...
  align-items: center;
  flex-wrap: wrap;
  gap: var(--moox-space-2) var(--moox-space-5);
  margin-bottom: 14px;
...
@media (max-width: 1280px) {
  ...
  .gateway-nodes :deep(td:nth-child(8)) {
    display: none;
  }
}
@media (max-width: 900px) {
  ...
  .gateway-nodes :deep(td:nth-child(6)) {
    display: none;
  }
}
@media (max-width: 600px) {
  .gateway-nodes :deep(th:nth-child(7)),
  .gateway-nodes :deep(td:nth-child(7)) {
    display: none;
  }
}
@media (max-width: 420px) {
  .gateway-nodes :deep(th:nth-child(4)),
  .gateway-nodes :deep(td:nth-child(4)) {
    display: none;
  }
}
</style>
