...
export const memoryUsageAvailable = (totalBytes: number | undefined, metricPresent: boolean): boolean =>
  metricPresent && (totalBytes ?? 0) > 0;

export const filesystemUsageAvailable = (
  device: string | undefined,
  mountpoint: string | undefined,
  totalBytes: number | undefined
): boolean => Boolean(device || mountpoint || (totalBytes ?? 0) > 0);

export const metricValueAvailable = (status: string, available: boolean): boolean => status === "online" && available;

export const historyHasRenderableData = (points: HistoryAvailabilityValue[]): boolean =>
  points.some(point => point.cpu_available || point.memory_available || point.disk_available);
