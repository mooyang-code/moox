...
describe('installSpaceAwareSignedClient', () => {
  it('registers the Space header interceptor after signing so Axios executes it first', () => {
    const handlers: Array<(config: InternalAxiosRequestConfig) => InternalAxiosRequestConfig | Promise<InternalAxiosRequestConfig>> = [];
    const client = {
      interceptors: {
        request: { use: (handler: (config: InternalAxiosRequestConfig) => InternalAxiosRequestConfig | Promise<InternalAxiosRequestConfig>) => handlers.push(handler) },
