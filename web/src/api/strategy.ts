...
export function normalizePerformance(input: { groups?: StrategyPerformance[] } | StrategyPerformance): { groups: StrategyPerformance[] } {
...
export async function listRunningStrategies(params: RunningStrategyFilter = {}): Promise<PageResult<RunningStrategySummary>> {
  const rsp = await callControl<{ page: PageRequest; space_id?: string; status?: string; mode?: string; strategy_id?: string }, { items?: RunningStrategySummary[]; total?: number; page?: number; page_size?: number }>(
    'strategy',
    'ListRunningStrategies',
    { page: { page: params.page ?? 1, page_size: params.page_size ?? 20 }, space_id: params.space_id, status: params.status, mode: params.mode, strategy_id: params.strategy_id },
  );
...
export async function listStrategyRuns(binding_id: string, params: PageRequest & { from?: string; to?: string } = {}): Promise<PageResult<StrategyRun>> {
	const rsp = await callControl<{ binding_id: string; page: PageRequest; range: { from: string; to: string } }, { items?: StrategyRun[]; total?: number; page?: number; page_size?: number }>(
    'strategy',
    'ListStrategyRuns',
    { binding_id, page: { page: params.page ?? 1, page_size: params.page_size ?? 20 }, range: { from: params.from ?? '', to: params.to ?? '' } },
  );
  return { items: rsp.items ?? [], page: { total: rsp.total ?? 0, page: rsp.page, page_size: rsp.page_size } };
}

export async function listStrategyTargets(run_id: string, params: PageRequest = {}): Promise<PageResult<TargetWeight>> {
  const rsp = await callControl<{ run_id: string; page: PageRequest }, { targets?: TargetWeight[]; total?: number; page?: number; page_size?: number }>(
    'strategy',
    'ListStrategyTargets',
    { run_id, page: { page: params.page ?? 1, page_size: params.page_size ?? 100 } },
  );
...
export async function getStrategyHealth(binding_id: string): Promise<StrategyHealth> {
  const rsp = await callControl<{ binding_id: string }, { health?: StrategyHealth }>('strategy', 'GetStrategyHealth', { binding_id });
...
export async function getStrategyPerformance(
  ...
  const rsp = await callControl<{ binding_id: string; performance_source: PerformanceSource; interval: string; range: { from: string; to: string } }, StrategyPerformance>(
    'strategy',
    'GetStrategyPerformance',
    { binding_id, performance_source: source, interval: params.interval ?? 'auto', range: { from: params.from ?? '', to: params.to ?? '' } },
  );
