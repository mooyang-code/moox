...
export async function saveSigningSession(input: { sessionId: string; rawKeyHex: string; expiresAt: number }): Promise<void> {
  const key = await crypto.subtle.importKey("raw", signingKeyBytes(input.rawKeyHex), { name: "HMAC", hash: "SHA-256" }, false, [
    "sign"
  ]);
...
export async function signRequest(
  key: CryptoKey,
  input: { method: string; path: string; body: string; headers?: SigningHeaders; timestamp: number; nonce: string }
): Promise<string> {
