...
assert(!route.includes('path: "/ops/metric-monitor"'), "retired metric monitor route must be absent");
assert(
  menu.includes('"ops-services"') && !menu.includes('menu("0607"'),
  "metric monitor must be grouped under service management"
);
assert(
  api.includes("ListMetricServices") && api.includes("ListMetricNames") && api.includes("ListMetricSeries"),
  "metric catalog RPC methods are missing"
);
...
assert(
  dashboard.includes("暂无已上报服务") && dashboard.includes("没有匹配的指标序列"),
  "empty catalog/no matching series states are missing"
);
assert(
  dashboard.includes("陈旧") && dashboard.includes("部分指标最新值加载失败") && dashboard.includes("部分历史指标加载失败"),
  "stale/partial states are missing"
);
assert(!dashboard.includes("historyError"), "unused history error state must stay removed");
assert(
  chart.includes("seriesField:") && chart.includes("series") && chart.includes("new VChart"),
  "VChart series mapping is missing"
);
assert(editor.includes("previewMetricRule") && editor.includes("保存前预览"), "structured rule preview is missing");
assert(
  condition.includes("标签条件") &&
    condition.includes("timeReducerOptions") &&
    condition.includes("compareOptions") &&
    condition.includes("noDataOptions"),
  "condition selectors are incomplete"
);
assert(editor.includes("LogicalOperator.AND") && editor.includes("LogicalOperator.OR"), "AND/OR connector control is missing");
assert(
  editor.includes("MAX_CONDITIONS = 8") && editor.includes("String.fromCharCode(65"),
  "flat A-H condition bounds are missing"
);
assert(
  editor.includes("consecutive_trigger_count") && editor.includes("consecutive_recovery_count"),
  "consecutive trigger/recovery controls are missing"
);
assert(editor.includes("webhook_ids") && editor.includes("multiple"), "multi-webhook selector is missing");
assert(
  !/Create(?:Monitor)?Target|Add(?:Monitor)?Target|scrape_target|target_url/i.test(api + dashboard + editor),
  "manual monitoring target API/control must not be present"
);
assert(!/promql|free.?text|expression/i.test(editor + condition), "free-text metric DSL must not be present in the rule editor");
assert(
  dashboard.includes('class="latest-panel"') && dashboard.includes('class="chart-panel"'),
  "metric monitor must expose latest and history panels"
);
